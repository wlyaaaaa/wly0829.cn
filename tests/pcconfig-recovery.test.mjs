...
test("accepted first H backup, current completed Cold and unverified machine restore remain distinct", () => {
  ...
  assert.match(technical, /H.*2026-09-05.*首次.*验收.*2026-09-09 06:55:23.*Cold.*complete.*21:59.*(?:不在|不可读)/s);
  ...
  assert.match(recoveryGaps, /06:55:23Z Cold回执complete.*主机H不在/);
  assert.match(recoveryGaps, /不保证后续G增量已入H.*不证明新机恢复/);
...
test("the System recovery explanation keeps the same ordinary/protected distinction", () => {
  ...
  assert.ok(node.detail.includes("9 月 9 日 06:55 UTC 完成") && node.detail.includes("本轮主机 H 不可用"));
  assert.match(node.detail, /后来新增的内容.*下一次备份/);
