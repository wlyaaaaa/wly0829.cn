...
test("the shared enhancement and search indices stay within their measured review thresholds", async () => {
  ...
  assert.match(runtimeSource, /if \(document\.querySelector\("\[data-computer-access\]"\)\) \{[\s\S]*?if \(window\.location\.origin !== "https:\/\/wly0829\.cn" \|\| keepLegacyResults\) import\("\.\.\/app\/computer-access-client\.jsx"\)/, "P6 island loads only on its initial route; legacy website origin redirects first");
