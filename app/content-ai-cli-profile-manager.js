...
export const aiCliProfileManagerProject = {
  ...
  plainExample: "我可以说：“在这个项目里用官方 Codex；先告诉我实际会启动哪个程序、会清掉哪些第三方变量，不要改官方登录。若要改用 Qwen3.8，就另起一个精确 Profile；先跑 Doctor，不要自动发送付费请求，也不要失败后换模型。”",
  ...
  usageExamples: [
    { moduleSlug: "profiles-launch", ask: "用官方 Codex 打开这个项目，但先确认不会被第三方 OPENAI_BASE_URL 劫持。", effect: "profile show、native 与 Doctor 先说明数据去向和将清除的父环境；start 再启动官方原生 CLI，不改全局登录。" },
    { moduleSlug: "profiles-launch", ask: "我已经在 OpenClaw 里配过 DeepSeek，先预览能导入什么；确认后只生成 Codex Flash/Pro、Claude Flash 和 OI Flash，别碰 Qwen。", effect: "脚本先核对 api.deepseek.com 来源并默认零写入；显式 -Apply 后生成四个 Profile，Key 立即进入 DPAPI，同名项没有 -Force 不覆盖。" },
    { moduleSlug: "profiles-launch", ask: "把这个不用的用户 Profile 删掉；先告诉我是否会删最后一个秘密引用，不要影响官方登录。", effect: "remove 默认确认；若它是最后一个 SecretRef 引用，只删除本工具的对应 DPAPI 密文，内置模板和上游官方登录保留。" },
    { moduleSlug: "engines-providers", ask: "这次精确用 Qwen3.8 Max Workspace；失败就报错，不要换 Qwen3.7、Token Plan 或本地模型。", effect: "只允许 codex-qwen3-8-max-paygo 的模型、Responses、地域、endpoint 和有效 xhigh 组合；任何覆盖或 fallback 都拒绝。" },
    { moduleSlug: "secrets-isolation", ask: "复用同一个 DeepSeek Key，但不要让它出现在参数、日志或别的终端里。", effect: "同凭据域只复用 SecretRef，DPAPI 在当前用户下解封并仅注入目标进程；native/eject 继续脱敏。" },
    { moduleSlug: "doctor-validation", ask: "先检查这个 Profile 为什么不能用，不要现在发送付费请求。", effect: "Doctor 返回依赖、配置、秘密引用、服务和证据状态；只有我随后明确选择 --live 才进入真实模型调用。" },
    { moduleSlug: "recoverable-runs", ask: "这个 Codex 文件任务中断了；只有还能恢复同一个任务身份才继续，否则明确告诉我需要从头重跑。", effect: "status 对账事件和控制器，resume 回验全部身份；任何新任务身份或证据链损坏都返回不支持恢复，不合并旧 partial。" },
    { moduleSlug: "local-proxies", ask: "先看看本地 Qwen 和两个 ChatGPT→Claude 代理现在是否真的可用，不要自动启动或升级。", effect: "分别核对 Ollama/LocalGpuBroker、批准制品、进程和 127.0.0.1 监听；未运行或版本未批准只报告限制。" },
    { moduleSlug: "install-recovery", ask: "把当前 main 的修复装到本机；如果以后卸载，先保留 Profile 和秘密，不要把本地退出说成远程 OAuth 已撤销。", effect: "安装器先做退役预检和候选验证，再原子切换；普通卸载保留用户数据，purge 和远程撤销分别处理。" }
  ],
  evidenceLayers: [
...
export const aiCliProfileManagerModules = [
  {
    ...
    example: "“在 D:\\项目\\演示 用 codex-official 启动；先告诉我实际程序和会清掉哪些变量。”native 只展示脱敏计划，确认后 start 才创建进程。",
  ...
  },
  {
    ...
    example: "“只用 qwen3.7-max-2026-06-08 的北京 Workspace 按量 Responses（响应式模型协议）；我的 max 可以映射 xhigh，但不要接受通用 alias（模型别名）或 fallback（自动回退）。”",
  ...
  },
  {
    ...
    example: "“DeepSeek Flash 和 Pro 使用同一凭据域；复用 Key，但不要打印、复制到另一个 Provider，也别改我的官方 Codex 登录。”",
  ...
  },
  {
    ...
    example: "“检查 codex-qwen3-8-max-paygo 现在缺什么；不要先扣费。如果静态都对，再由我决定是否做 text 或 agent。”",
  ...
  },
  {
    ...
    example: "“在这个隔离工作区后台跑任务；进程断了最多恢复三次。若 model、Provider、权限或任务身份变了就停，不要把旧输出并进新结果。”",
  ...
  },
  {
    ...
    example: "“只检查 qwen-main-v1 与 cliproxy 当前状态；不要启动、下载新版或改端口，也不要在不可用时转到云端。”",
  ...
  },
  {
    ...
    example: "“把 main 的最后修复装上；若遇到未知旧文件就先停。以后普通卸载保留我的 Profile 和密文，只有我明确 purge 才删本工具数据。”",
