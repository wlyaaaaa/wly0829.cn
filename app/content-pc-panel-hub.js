export const pcPanelHubProject = {
  ...
  summary: "PC Panel Hub 把 Windows 工作站分成两块职责不同的常驻副屏：480×1920 TURZX 机箱屏每秒呈现硬件、网络、磁盘、FPS 和前台状态；2288×1048 HS2 曲面 OLED 以动态壁纸为底层，透明事件浮层只在需要时显示媒体、任务、手机事件和硬件告警。同一个长期看门狗协调刷新、睡眠、关机和故障恢复。",
  why: "主屏上的监控窗口会挡工作，也容易在问题发生前被关掉；实体副屏又可能出现“任务仍在运行、主机仍在写串口，但画面已经冻结”的假健康。这个项目把数据可信度、渲染、传输、实体观察和恢复分层，使我既能随时看见关键状态，也能知道哪一层真的被证明。",
  plainExample: "例如机箱屏时钟不再跳动，我不先重启整台电脑。系统先确认目标串口仍是唯一健康设备、只有一个发送进程、最新 heartbeat（心跳）确实包含发送动作、1 Hz（每秒一次）节拍和定期全帧基线；必要时只重建本项目链路。恢复后仍要看实体像素是否继续更新，不能用主机日志替代眼睛。",
  ...
  operatingFlow: [
    ...
    { title: "分层验收并按需刷新网页", detail: "Source、Tests、Runtime、Demo、Protocol 与 Owner observation 分别记录；本轮本人观察只确认动态壁纸已经生效，其余实体情景仍保留缺口。只有正式源发布会使页面判断实质错误时，才异步刷新本页。" }
  ],
  components: [
  ...
  usageExamples: [
    ...
    { ask: "我要给网页补副屏截图", effect: "优先使用合成 demo 和软件渲染预览并标明证据等级；只有无私人正文且经过实际值审查的实体照片才可作为实机证据。" }
  ],
  evidenceLayers: [
