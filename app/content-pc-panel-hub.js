...
export const pcPanelHubProject = {
  ...
  summary: "PC Panel Hub 让 TURZX 480×1920 USB 机箱屏和 LIAN LI HS2 2288×1048 曲面 OLED 水冷屏各司其职：前者持续显示 Windows 主机的硬件、帧率、网络、磁盘和后台状态，后者在动态壁纸上临时显示媒体、手机、任务和告警。显示器身份或主屏归属真正变化后，单一看门狗还会在当前显示路径绑定健康、变化稳定且冷却允许时，请现有 Wallpaper Engine 做一次有界渲染重绑。公开源码同时保留从干净克隆到本机双屏绑定的重建路线，使换机或重装不必靠旧路径和记忆拼安装。",
  ...
  readerStates: {
    ...
    unavailable: "设备、串口、L-Connect、本地指标、唯一当前显示路径绑定、Wallpaper Engine 控制客户端或交互桌面不可用时，停止相应动作并标为 Unknown（未验证）；不抢串口、不重置 USB 树、不改壁纸配置，也不把旧预览冒充当前画面。"
  },
  productPrinciples: [
  ...
  responsibilities: [
    ...
    "协调两块屏在启动、睡眠、唤醒、关机、显示拓扑变化和进程故障时的安全状态，并在唯一当前显示路径绑定健康时恢复动态壁纸渲染",
...
export const pcPanelHubModules = [
  ...
  },
  {
    ...
    value: "负责恢复小屏的看门狗自己异常退出时，原来的隐藏启动入口也能把它接回；明确停止整条链则不会自动反弹。刚登录时，误入水冷屏的普通应用可先搬回安全屏，不必等完整HS2浮层验证。后续异常只修受影响链路：串口有熔断、HS2模式有epoch（周期）边界，动态壁纸只在真实活动显示路径变化稳定后重绑一次。 物理主屏存在时，若普通窗口整个跑进远程虚拟屏，或大窗口有一部分跨过去，现有看门狗会将可见内容放回主屏工作区，过大的窗口收窄到可用范围。主屏或VDD身份不唯一就不动；主屏确实离线时允许VDD继续承载窗口，不改分辨率、HDR、捕获配置或屏幕排列。",
    ...
    example: "远程虚拟屏退出又回来后，HS2 浮层位置没错，动态壁纸却像一张海报一样不动了。看门狗会先等显示拓扑稳定并确认当前主屏和唯一HS2身份正确、参与桌面的MTT后端健康，再让原来的壁纸重绑一次；条件少一项就不折腾屏幕，只保留静态背景并写清原因。",
    result: "9月7日TURZX已在看门狗异常退出后由同一入口自动接回，本人确认小屏正常刷新；人工停止也已验证不会反弹。壁纸分支另有自己的条件：符合当前显示路径绑定、稳定和冷却时应恢复原有Wallpaper Engine场景，并记录Baseline、Stabilizing、WaitForHealth、Cooldown、Rebind或分派失败。条件不全则等待并保留静态背景；本次没有真实触发stop/play或完成HS2壁纸实体恢复。",
    readerStates: {
      pass: "首次健康观察只建立壁纸拓扑基线；之后指纹真的变化、稳定满 30 秒、主屏/HS2路径健康且参与桌面的MTT后端健康且离上次分派至少 900 秒时，只分派一次 stop/play 并记录结果。任务、精确进程、heartbeat、控制器模式与绑定仍分别验收。",
      problem: "拓扑仍在变化或任一绑定不健康时等待；串口假活进入熔断；Wallpaper Engine 分派失败时消费本事件、保留现有配置并停止紧密重试，避免把一次恢复变成循环闪屏。",
      unavailable: "控制器、HS2端点、当前会话 wallpaper64 路径、同目录 wallpaper32 或 explorer shell 缺失时不执行壁纸控制；MTT仅在参与活动桌面时要求唯一健康后端。需要物理处理则明确要求先关机断电；需要恢复壁纸时仍可使用 Wallpaper Engine 自身用户控制，系统静态背景继续是视觉降级。"
    },
    decisionImpact: [
