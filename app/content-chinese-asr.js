export const chineseAsrProject = {
  ...
  summary: "ChineseASR（中文自动语音识别）日常 quick（快速）使用 SenseVoiceSmall，strict（严格）使用 Qwen3-ASR-1.7B 主引擎加 SenseVoiceSmall 对照，把微信语音、会议、口述笔记和长录音变成可搜索、定位和复核的文字，并保留原始输出、时间位置、分歧与失败原因。",
  why: "中文录音最危险的问题不是单纯“识别错一个字”，而是模型在静音、噪声、方言、专有名词或长音频切片处生成看似通顺的错误内容。普通转写结果往往没有输入指纹、模型版本、时间位置和失败证据，事后既不知道错在哪里，也无法判断重跑是否真的更可靠。",
  plainExample: "例如我说“把这段两小时的中文会议录音转成可以复核的纪要”。系统先确认文件、音频格式和所需模式，再把长录音按连续时间线切片；严格模式由 Qwen3-ASR 主转写、SenseVoice 对照，逐段记录两路差异和风险。任务可以断点续跑，最后交付正文、逐段时间位置、原始模型结果、审计文件和需要人工回听的清单，而不是只给一段无法追溯的文字。",
  ...
  exclusions: [
    ...
    "不保存或公开私人录音、转写正文、声纹向量、模型权重和 API Key（接口密钥）",
  ...
  usageExamples: [
    { ask: "把这段微信语音转成文字。", effect: "先走本地 quick 或 strict；返回正文和原始结果，普通请求不会触发云上传。" },
    { ask: "这段会议很重要，尽量降低幻觉。", effect: "使用 strict 双模型，保留两路分歧、风险标记和需要回听的句段；必要时再显式选择证据级路线。" },
    { ask: "把两小时录音处理完，中断后别从头来。", effect: "生成连续 chunk 清单，按输入和配置身份断点续跑，只补缺失或失效片段。" },
    { ask: "把这个文件夹的录音都转写。", effect: "批量入口复用已加载模型，逐文件生成独立结果与失败状态，不用一个文件失败拖垮全部。" },
    { ask: "告诉我哪一段可能是我说的。", effect: "结合 person:self 声学线索、声道、联系人、对话角色和句义；证据冲突或单声道歧义时保持 unknown。" },
    { ask: "这段录音是不是完全没人说话？", effect: "只有执行完成、覆盖完整且有规范负向证据时才返回 no_speech_detected；空文本或失败保持 indeterminate。" },
    { ask: "模型卡住了，我要不要再提交一次？", effect: "先查 job 状态和 job key；任务仍运行时继续观察，超时或失败按已有任务恢复，不盲目复制重任务。" }
  ],
  evidenceLayers: [
