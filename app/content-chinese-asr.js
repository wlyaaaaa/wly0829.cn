...
export const chineseAsrProject = {
  ...
  plainExample: "例如我说“把这段两小时会议录音转成可复核文字，供后续整理纪要；中断后别从头来，以后换电脑也要能恢复”。它会按连续分段保留进度，交付正文、原始结果、风险与恢复入口；安装侧另用依赖锁、离线 wheelhouse（轮包仓库）和模型回执重建环境。只有依赖、模型身份和真实 smoke（冒烟验证）分别通过后，才把新环境称为可用。",
