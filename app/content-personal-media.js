...
const personalMediaProject = {
  ...
  plainExample: "例如手机里多了二十张旅行照片和几段视频。连接后先排除系统与应用私有边界，把新共享文件逐项写入 E 盘本次副本和 G 盘异卷副本，回读 bytes 与 SHA-256 通过就立即告诉我可以拔线。随后在一张联系表里判断画面意义、质量、精确/高度近重复、类别和时间，只把 keeper 归入当前照片/视频根，并同步手机恢复包和 upload=0 云候选。",
  result: `我得到的不是一个模糊相册，而是一组能回到原件的结果：检索候选带着日期、地点、类别和匹配依据；精选入口直接展示 ${current.selectedImageRows.toLocaleString("zh-CN")} 张值得主动重看的照片和 ${current.selectedVideoRows} 个视频；分类决定说明保留或退出原因；手机保全说明双盘是否完成、手机是否清空；恢复包说明当前可写回范围；云候选只说明等待人工批准的内容。`,
  readerStates: {
