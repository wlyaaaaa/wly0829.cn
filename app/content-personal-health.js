...
export const personalHealthModules = [
  {
    ...
    example: "询问“现有材料能不能支持这个判断”时，先看现行底色是否已经包含关键事实。只有新报告、纠正、来源冲突、完整性问题或一个会改变答案的源级缺口，才读取 SOURCES 并打开最小相关原件。",
  ...
  },
  {
    ...
    example: "本人明确点击刷新。入口先尝试离线消费已完成交接；只有确实没有待处理结果时才调用固定 Secret Broker 一次。Secret Broker 返回 rebind、异常回执、过大输出或超时，整条路线停止，不调用 capture。",
  ...
  },
  {
    ...
    example: "步数记录存在且结构完整，但最近窗口覆盖不足。系统显示“需要复核：覆盖不足”，同时写明“不能证明活动不足或身体异常”；字段不会进入下游判断。",
  ...
  },
  {
    ...
    example: "医生建议一项重大治疗，AI发现现有资料无法解释为什么只有这一种选择。AI不能宣判医生错误，也不能让用户擅自停药；它应列出缺失依据和合理替代，帮助向原医生追问，必要时让独立专科医生复核同一份原始资料。",
