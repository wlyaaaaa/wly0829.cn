...
export const personalHealthProject = {
  ...
  usageEntry: "直接向 personal-health 提出健康问题；只有本人明确说更新设备记录时才走前台 Fitbit Air 刷新，现有记录可按指定日期离线核对。",
  usageInputs: ["当前想弄清的健康问题", "新的症状描述、报告或医生意见（仅在现有依据不足或本人要补充时）", "明确刷新时想看的日期范围（如指定）"],
  title: "个人健康证据与安全决策",
  ...
  dataSources: {
    "title": "资料从哪里来，这次会怎样用",
    "intro": "本项目可使用本人描述、选定报告与医生意见，以及 Fitbit Air 经手机 Google Health 同步的设备记录。下表说的是代码可取的资料类型，不是本次个人健康记录。来源、账号授权、日期与质量仍要在真正刷新时核对；设备家族标记不能单独证明每条数据都来自同一具体机型。",
    "rows": [
      {
        "source": "本人描述、报告与医生意见",
        "data": "本人愿意说明的感受、目标、现实负担，以及选定报告所见和医生解释。",
        "result": "普通问题先用已整理的现行依据，确实缺关键事实才读相关原件；分清观察、解释、建议与未知。"
      },
      {
        "source": "Fitbit Air / Google Health：日常活动",
        "data": "步数、距离、活动分钟、活跃区间、久坐时段和热量六类记录。",
        "result": "默认只展开步数与活动分钟的日期覆盖和变化；没记录不等于当天没有活动。"
      },
      {
        "source": "Fitbit Air / Google Health：运动与体能",
        "data": "已记录运动、游泳趟数与几类心肺能力估计，共五类。",
        "result": "默认只汇总真正记录到的运动次数与日期；其余先保全，不自动写成健康判断。"
      },
      {
        "source": "Fitbit Air / Google Health：睡眠",
        "data": "睡眠时长、入睡与起床时段，以及主睡眠、小睡或未分类记录。",
        "result": "比较所选日期范围内的睡眠与覆盖，说明重叠、缺失和分类不明。"
      },
      {
        "source": "Fitbit Air / Google Health：心率变化",
        "data": "心率、静息心率与心率变异性等四类记录。",
        "result": "可以保全原始记录；默认只说有无记录与覆盖，不展开高频正文或据此诊断。"
      },
      {
        "source": "Fitbit Air / Google Health：血氧、呼吸与皮温",
        "data": "血氧、呼吸及睡眠皮温变化等五类记录。",
        "result": "可以保全但默认不纳入健康判断；皮温变化不是体温计测得的核心体温。"
      },
      {
        "source": "账号历史范围与运动附件",
        "data": "账号资料帮助确认可请求的历史起点；有相应运动记录时，还可保全运动轨迹附件。",
        "result": "它们不算进上述设备字段，也不能证明当前设备在线或自动产生运动分析。"
      }
    ],
    "note": "默认摘要只解释睡眠、步数、活动分钟和已记录运动。其他记录即使保存了，也要另过字段质量和问题相关性门；通用接口有某类字段，不等于本项目已经采集。"
  },
  readerStates: {
  ...
  exclusions: [
    "不诊断、不决定用药，不替代医生的查体、诊断和处方；医生的建议也应让本人理解依据与选择。",
    "不因为专业意见可能有局限就推定恶意，更不鼓励擅自停药。",
    "不一味迎合危险选择；发现急症或高风险做法会明确劝阻并给现实下一步。",
    "不做身体总分、疾病预测器或个人健康画像，也不为回答普通问题建立另一套数据库和服务。",
    "不设持续设备同步、每日打卡、后台监测或自动提醒。",
    "没有记录、尚未测量、真实零值和正常状态各不相同，不合并成一个结论。",
    "设备导入和摘要不会自动改写当前健康依据；需要健康项目负责人核对后才采用。",
    "公开网页不展示本人健康事实；代码与合成测试也不证明当前账号、设备或个人情况。"
  ],
  glossary: [
  ...
  operatingFlow: [{"title": "先问清想解决什么", "detail": "普通问题先用已整理的当前依据，缺某日数据时按字段和日期窄查，不因每次提问重扫全部报告。"}, {"title": "明确要求时才更新设备", "detail": "一次前台取回 Fitbit Air 经手机 Google Health 同步的记录；先保全原始数据，再查日期、分页与质量，失败沿同一清单续作。"}, {"title": "只用够作决定的证据", "detail": "按当前问题取睡眠、步数、活动等相关字段；缺失或质量不足明确不据此判断身体异常。"}, {"title": "交回选择与边界", "detail": "说明事实、未知、可选行动及何时寻求专业帮助；医生、设备和 AI 的说法都需要依据，非紧急选择由本人作出。"}],
  ...
  evolution: [
    { date: "2026-08-24", title: "早期健康方案退出，现役健康协作接管", commit: "ace3596–a222a85", result: "早期方案不再写入；现役项目承接普通问答、证据维护和本人明确发起的设备更新。同期建立最小当前依据与只在本机受保护入口使用的账号授权。" },
    { date: "2026-08-24—08-25", title: "从一次设备保全，走到可续跑的当前健康底色", commit: "0f42703–98f514a", result: "普通问题先使用已经核对的当前依据；设备原始记录可以保全、按一日一项窄查，中断后只继续缺失部分。" },
    { date: "2026-08-25", title: "从有数据，走到知道哪些数据可用于判断", commit: "50131b3–377cbe9", result: "每项设备记录先看来源、覆盖和质量：有些可用于这次问题，有些只能保存，有些必须停用；健康项目负责人最后审阅。" },
    { date: "2026-08-30—08-31", title: "刷新只有完整、核过并审过，才进入当前状态", commit: "48d5a5b", result: "一次前台更新要先证明资料取齐、文件未变，再由健康项目负责人决定是否采用；失败不自动改写当前依据，也不转成后台任务。" }
  ],
...
export const personalHealthModules = [
  {
    ...
    usageEntry: "直接描述当前健康问题，先用已整理的现行依据回答。",
    usageInputs: ["当前健康问题", "新的本人描述或更正（如有）"],
    productFlow: [{"title": "系统核对并处理", "detail": "先取相关底色；只有缺少会改变判断的细节，才回到一份报告或某日设备字段核对。"}, {"title": "交付与接续", "detail": "交回依据、未知与下一步；旧材料多不代表比当前更可信，来源冲突先标明。"}],
    shortTitle: "平时怎么回答",
    ...
    readerStatus: "已有先用处理后信息回答、必要时回原件的方法；本页没有读取个人底色，不能代替一次实际健康咨询。"
  },
  {
    slug: "protected-foreground-refresh",
    usageEntry: "明确说“更新这次 Fitbit Air 记录”，由现有前台入口执行。",
    usageInputs: ["本人明确的这次设备刷新请求", "想看的日期范围（如不同于默认）"],
    productFlow: [{"title": "系统核对并处理", "detail": "只在本次前台通过受保护凭据入口取回 Google Health 数据，凭据不落在普通文件或命令行。"}, {"title": "交付与接续", "detail": "交回本次导出、缺页和后续离线验真状态；连接或授权失败就停，不暗中设后台同步。"}],
    shortTitle: "主动更新设备",
    ...
    result: "得到一次由本人明确发起的前台更新结果：取回了什么、哪些日期还缺、下一步能否离线核对。账号或权限出了问题就说明并停止，不换账号或在后台反复尝试；长期凭据仍留在受保护入口。",
    readerStates: {
      "pass": "本次设备记录取回并核对完整后，再转入本机离线验真；一次请求返回不等于全部完成。",
      "problem": "若上一批已经成功取回但尚未整理，先在本机完成它，不重复访问账号。",
      "unavailable": "账号需重新绑定、回执异常或本次运行超时就停止，保留已有材料并说明接续条件。"
    },
    stateLabels: evidenceStateLabels,
    decisionImpact: [
    ...
    readerStatus: "设备更新、凭据保护和中断处理已有实现及合成测试；当前账号与真实设备数据更新未在本轮验证。"
  },
  {
    slug: "raw-preservation-resume",
    usageEntry: "有设备更新清单中断时，要求沿这同一批继续。",
    usageInputs: ["继续上次未完成设备更新的请求", "有多批时指出要继续哪一批"],
    productFlow: [{"title": "系统核对并处理", "detail": "先保存取回的原始页和运动附件，增量覆盖上次未完整的中国日起；已完整历史不反复下载。"}, {"title": "交付与接续", "detail": "逐页核对后交回完整与缺失范围；中断继续同一清单，不另猜一个文件夹或吞掉旧原件。"}],
    shortTitle: "中断后继续",
    ...
    why: "有一批文件躺在目录里，不代表那一天所有记录都拿齐了。只按最近文件夹续跑，容易漏页、重复统计或误把残缺数据说成完整。",
    example: "几百页设备记录下载到一半断了。下次继续时，系统先核对已经完成的日期和页面是否仍属于同一次导出；对得上就从下一页接着来，对不上就停下说明冲突，不从头重下，也不把半成品冒充完整数据。",
    result: "已有完整历史与这次新增的原始记录都保留下来，能知道哪些日期和页面真的取齐。中断时从同一批的缺口继续；未完成当天由新记录补齐，不覆盖旧原件。",
    readerStates: {
      "pass": "日期、页面与原始文件都能一一对上时，才把这次导出记为完整；旧完整日期与新日期衔接。",
      "problem": "中途断开但已保存部分仍可核对时，保留它们并从同一批未完成处继续。",
      "unavailable": "文件、顺序或内容身份对不上时，在再访问设备账号前停下，不换目录凑成成功。"
    },
    stateLabels: evidenceStateLabels,
    decisionImpact: [
    ...
    readerStatus: "已有逐页保存、只补新日期和同次续跑的能力；离线合成测试通过，本轮未导入真实个人数据。"
  },
  {
    slug: "offline-decision-brief",
    usageEntry: "问近两周变化，或明确指定已采集的完整中国日期范围。",
    usageInputs: ["要比较的睡眠、步数或活动等问题", "想看的日期范围（如指定）"],
    productFlow: [{"title": "系统核对并处理", "detail": "先离线核对原始页与覆盖，再按所选范围汇总相关字段；指定长范围就按该范围自身的质量判断。"}, {"title": "交付与接续", "detail": "交回变化、缺记录和可用性；保存了数据不等于这一字段足以支持健康结论。"}],
    shortTitle: "筛选可用信息",
    ...
    result: "得到所选日期里睡眠、步数、活动或已记录运动的变化、覆盖和缺口，知道每项记录目前能不能用于眼前问题。只问某一天时只读那天已保存的字段；查到记录不等于能作医学判断。",
    readerStates: {
      "pass": "所选日期和字段的来源、完整性与质量都过关时，才交健康项目负责人用于当前问题；单日查询只说明这一天保存了什么。",
      "problem": "重叠、缺页或覆盖不足会明确列出。某日没查到记录不等于真实数值为零，也不重新下载整段历史。",
      "unavailable": "选中的导出或文件无法核对时，先停在证据层，不根据残缺摘要解释身体状况。"
    },
    stateLabels: evidenceStateLabels,
    decisionImpact: [
    ...
    readerStatus: "已有离线按日期查记录、检查覆盖和整理简报的功能；本轮只保留合成验收，没有读取个人数据或得出健康结论。"
  },
  {
    slug: "evidence-three-state",
    usageEntry: "查看一次健康摘要时，要求说明每项证据能不能用于决定。",
    usageInputs: ["要判断的健康问题或想看的设备字段"],
    productFlow: [{"title": "系统核对并处理", "detail": "逐字段区分可用于当前问题、仅有库存记录、或因质量与冲突被阻断，不把状态合成身体分数。"}, {"title": "交付与接续", "detail": "交回可采用项与缺口；被阻断的数据留作原件，不进入当前建议。"}],
    shortTitle: "证据三态",
    ...
    readerStates: {
      "pass": "可用于当前问题：来源和质量都核对过，仍不代表身体正常或已得到诊断。",
      "problem": "仅已保存或需要复核：资料存在，但覆盖、结构或来源不足以支持这次判断。",
      "unavailable": "暂不能用：入口或原始记录无法核对；保留未知并说明怎么补证据。"
    },
    stateLabels: evidenceStateLabels,
    decisionImpact: [
    ...
    readerStatus: "已有区分可判断、待复核与证据不足的状态；它评价的是资料，不是身体正常、健康得分或诊断。"
  },
  {
    slug: "health-owner-boundary",
    usageEntry: "给出健康疑问或选项，请 AI 帮忙核对依据与真实取舍。",
    usageInputs: ["本人当前目标与现实限制", "尚未记录而会改变判断的症状或专业意见（如有）"],
    productFlow: [{"title": "系统核对并处理", "detail": "AI整理相互支持或冲突的证据，比较收益、风险、负担和退出条件；红旗情形明确提示及时求助。"}, {"title": "交付与接续", "detail": "交回可复查的选择依据和不确定性；不把 AI、设备或医生写成无需解释的终局权威。"}],
    shortTitle: "谁做决定",
    ...
    readerStatus: "已有由领域审阅决定采用哪些证据的方法；实际医疗决定不在本轮验收，紧急情况仍回到现实医疗。"
  }
];
