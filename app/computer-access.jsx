...
function ReadingSources({ sample, compact = false, note = null }) {
  ...
  return <FloatingDetails className="ca-source-details" label={compact ? "来源" : "查看读数来源与时间"}>{note && <p className="ca-footnote">{note}</p>}<dl>{sources.map(([field, source]) => <div key={field}><dt>{labels[field]}</dt><dd>{source.source || "来源未知"} · {source.status === "unknown" ? "暂无读数" : source.status === "stale" ? "读数已过期" : source.status === "unavailable" ? "来源不可用" : "已读取"}<br />{source.observed_at_unix ? `读取于 ${beijingTime(source.observed_at_unix)}` : "读取时间未提供"}</dd></div>)}</dl></FloatingDetails>;
}
...
function Hardware({ hardware = {}, host = {}, cached = false, grafana, connected, onRetry, loading }) {
  ...
  return <section className="ca-card ca-resources" aria-labelledby="ca-hardware-title">
    <div className="ca-section-title"><div className="ca-resource-heading"><h2 id="ca-hardware-title">系统资源状态{cached && <small className="ca-cached-label">上次采样 · 等待连接确认</small>}</h2><nav className="ca-resource-links" aria-label="电脑与监控入口"><a href="/mcp/"><SiModelcontextprotocol size={15} />连接电脑</a><GrafanaStatus service={grafana} connected={connected} onRetry={onRetry} loading={loading} compact /></nav></div><div className="ca-system-line"><span>{windowsLabel}</span><span><Desktop size={14} />{({ locked: "已锁屏", unlocked: "未锁屏", no_session: "无交互桌面" })[host.screen_state] || "屏幕状态未知"}</span><span>已运行 {Number.isFinite(host.uptime_seconds) ? minutesLabel(Math.floor(host.uptime_seconds / 60)) : "未知"}</span><span className="ca-dpc-meta">系统DPC占用 <strong>{reading(hardware.health?.dpc_usage_percent, "%", 2)}</strong><ReadingSources sample={hardware.health} compact /></span><FloatingDetails className="ca-boot-details" label="详情" title="系统详情"><p>{host.windows_version || "Windows版本未知"}<br />本轮开机：{beijingTime(host.boot_time_unix)}。各项读数保留实际来源与读取时间。</p></FloatingDetails></div></div>
    <p className="ca-resource-readtime">{resourceReadLabel(hardware)}</p>
    <div className="ca-resource-grid">
      <article className="ca-hardware-card"><div className="ca-hardware-heading"><span className="ca-icon"><Cpu size={25} /></span><div><h3>处理器</h3><p className="ca-spec-line"><span>{reading(cpu.model)}</span><span>{reading(cpu.cores)} 核 · {reading(cpu.threads)} 线程</span></p></div></div><div className="ca-primary-reading"><strong>{reading(cpu.usage_percent, "%")}</strong></div><Meter used={cpu.usage_percent} label="CPU使用率" /><HardwareMetrics sample={cpu} /><DisplayCard display={hardware.display} /><ReadingSources sample={cpu} compact note={`${hardwareBasis(cpu.frequency_basis, "frequency")}；电压口径：${reading(cpu.voltage_basis)}`} /></article>
      ...
      <article className="ca-hardware-card"><div className="ca-hardware-heading"><span className="ca-icon"><Memory size={25} /></span><div><h3>内存</h3><p className="ca-spec-line">{[memorySpecs.standard, memorySpecs.modules, memorySpecs.manufacturer, memorySpecs.timings].filter(Boolean).map((part, index) => <span key={index}>{part}</span>)}</p></div></div><div className="ca-primary-reading ca-memory-reading"><span>已用</span><strong>{capacity(memory.used_bytes)}</strong></div><Meter used={memory.used_bytes} total={memory.usable_total_bytes} label="物理内存占用" /><Facts items={[["已安装", capacity(memory.installed_total_bytes)], ["Windows可用物理总量", capacity(memory.usable_total_bytes)], ["当前剩余", capacity(memory.available_bytes)]]} /><div className="ca-commit"><div><span>虚拟内存（已提交）</span><strong>{capacity(memory.committed_bytes)} / {capacity(memory.commit_limit_bytes)}</strong></div><Meter used={memory.committed_bytes} total={memory.commit_limit_bytes} label="内存提交占用" /><small>已提交 / 提交上限，非分页文件磁盘占用。</small></div><ReadingSources sample={memory} compact note={`内存型号：${reading(memory.part_number)}；${memorySpecs.timings || "时序未读取"}`} /></article>
    </div>
...
function NetworkCard({ network = {} }) {
  const metrics = [["↑ 上传", rate(network.upload_bytes_per_second), "upload"], ["↓ 下载", rate(network.download_bytes_per_second), "download"], ["延迟", reading(network.latency_ms, " ms", 2), "latency"], ["抖动", reading(network.jitter_ms, " ms", 2), "jitter"], ["丢包", reading(network.packet_loss_percent, "%", 2), "loss"]];
  return <section className="ca-card ca-network-card" aria-label="网络状态"><div className="ca-network-strip"><div className="ca-network-name"><WifiHigh size={19} /><h3>网络</h3></div><div className="ca-network-device"><strong>{reading(network.model)}</strong><small>{networkConnection(network)}</small></div><dl className="ca-network-quality">{metrics.map(([label, value, kind]) => <div className={`ca-metric ca-metric-${kind}`} key={kind}><dt>{label}</dt><dd>{value}</dd></div>)}</dl><div className="ca-network-meta"><SampleTime sample={network} /><ReadingSources sample={network} compact note={hardwareBasis(network.interface_basis || network.basis, "network")} /></div></div></section>;
}
...
export default function ComputerAccess({ initialRead = null }) {
  ...
  return <div className="ca-page">{connection === "offline" && <section className="ca-offline-panel" aria-label="电脑连接说明"><div><Desktop size={24} /><h2>{connectionProblem === "service" ? "连接服务暂时异常" : "暂时无法连接电脑"}</h2></div><p>{connectionProblem === "service" ? "服务器返回了异常响应，当前无法读取电脑状态；这不表示电脑已经关机。" : "可能是网络、休眠、关机或连接服务问题，目前无法确认原因。"} 已有授权仍按原期限处理，页面不会重放办理操作。</p><p>如需远程开机或恢复连接，可使用已配置的 ToDesk、网易UU 或小米智能插座3路径，再回来重新检查。请先确认实际情况，避免仅凭连接失败反复断电。</p><button className="ca-refresh" type="button" disabled={refreshing} onClick={() => reader.current?.read({ refresh: true })}>重新检查连接</button></section>}<div className="ca-workspace"><div className="ca-overview">
    <header className="ca-intro"><h1>授权与状态</h1><span className={`ca-connection ca-connection-${connection}`}><span className="ca-dot" />{connection === "online" ? "主机在线" : connection === "connecting" ? "正在连接主机" : connectionProblem === "service" ? "连接服务异常" : "暂时无法连接电脑"}</span><span className="ca-updated"><ReadIndicator busy={refreshing || connection === "connecting"}>{refreshing || connection === "connecting" ? "正在读取…" : lastRead || observed ? `${connection === "online" ? "读取完成" : "上次读取"} ${beijingTime(lastRead || observed, true)}` : "尚未读取"}</ReadIndicator></span><div className="ca-top-actions"><button className="ca-refresh" type="button" disabled={busy || Boolean(actionBusy) || refreshing || connection === "connecting"} aria-busy={refreshing || connection === "connecting"} onClick={() => reader.current?.read({ refresh: true })}><ArrowClockwise size={17} />{refreshing || connection === "connecting" ? "刷新中…" : connection === "offline" ? "重新检查" : "刷新状态"}</button><button className="ca-refresh ca-screen-lock" title="仅锁交互桌面，保留资料和无限制授权原期限" type="button" disabled={!actionEnabled("windows") || snapshot?.public_actions?.lock_windows !== true} onClick={() => lock("windows")}><Desktop size={17} />锁定Windows</button></div></header>
