...
function Hardware({ hardware = {}, host = {}, cached = false }) {
  ...
  return <section className="ca-card ca-resources" aria-labelledby="ca-hardware-title">
    <div className="ca-section-title"><h2 id="ca-hardware-title">系统资源状态{cached && <small className="ca-cached-label">上次采样 · 等待连接确认</small>}</h2><div className="ca-system-line"><span>{host.windows_version || "Windows版本未知"}</span><span><Desktop size={14} />{({ locked: "已锁屏", unlocked: "未锁屏", no_session: "无交互桌面" })[host.screen_state] || "屏幕状态未知"}</span><span>已运行 {Number.isFinite(host.uptime_seconds) ? minutesLabel(Math.floor(host.uptime_seconds / 60)) : "未知"}</span><details className="ca-boot-details"><summary>本轮开机与更新时间</summary><p>本轮开机：{beijingTime(host.boot_time_unix)}。下方每组保留实际读取时间；读取成功不等于传感器样本时间已知。</p></details></div></div>
...
export default function ComputerAccess() {
  ...
  useEffect(() => {
    ...
    }, (_, { refresh }) => { if (mounted.current) { setRefreshing(false); setConnection("offline"); if (refresh) notify("本次读取未完成，保留上次观测；当前状态未知。", "warning"); } });
    let timer;
  ...
  return <div className="ca-page"><div className="ca-workspace"><div className="ca-overview">
    ...
    <div className="ca-grants"><GrantCard title="个人资料" grant={snapshot?.personal_data} now={now} current={connection === "online"} purpose="personal_data" enabled={available && !busy && !actionBusy && (!request || requestTerminal)} onChoose={choose} description="本机与已认证的电脑连接共用" endEnabled={canEndGrant(snapshot?.personal_data, now) && actionEnabled("personal-data") && snapshot?.public_actions?.lock_data === true} onEnd={() => lock("personal-data")} /><GrantCard title="无限制授权" grant={snapshot?.unrestricted} now={now} current={connection === "online"} purpose="unrestricted" enabled={available && !busy && !actionBusy && (!request || requestTerminal)} onChoose={choose} description="全局授权，与个人资料分别计时" endEnabled={canEndGrant(snapshot?.unrestricted, now) && actionEnabled("unrestricted") && snapshot?.public_actions?.end_unrestricted === true} onEnd={() => lock("unrestricted")} /></div>
      ...
      <Hardware hardware={snapshot?.hardware} host={host} cached={connection !== "online" && Boolean(snapshot?.hardware)} />
      <p className="ca-update-note">仅前台每60秒读取，回到前台立即检查；状态刷新不延长授权。</p>
    </div><NetworkCard network={snapshot?.hardware?.network} /></div>
    {toast && <div className={`ca-toast ca-toast-${toast.tone}`} role="status"><p>{toast.message}</p><button type="button" aria-label="关闭提示" onClick={() => setToast(null)}>×</button></div>}
