...
const vaultToolSnapshot = createProjectSnapshot({
  ...
  label: "默认保留原件，改密保留另一层，恢复与远端结果能够逐项核对",
  boundary: "本轮从2.3.1正式来源重建产品差分，保留以前真实算法的虚构样例；没有读取真实密码或私人库，没有执行私人远端上传、物理断电或新机恢复。",
  metrics: [
  ...
  facts: [
    ...
    { label: "当前来源", value: "公开工具仓库 wlyaaaaa/vault-tool，维护目录 E:\\Projects\\Tools\\vault-tool；正式版本2.3.1，来源提交57ad50b87e0c38dce7c16b96068c0f5d88d75e3c。它与私人Key密文仓库分开，源码更新不表示用户的真实密文已经迁移。", hero: false },
    ...
    { label: "入口实际区别", value: "菜单添加在内存中合并本次打开层与新输入，未解锁槽位保持密文字节不变；直接encrypt仍只处理明确source/，不能当自动追加。两种方式默认保留输入。info、doctor、assess、plan无论文本或JSON均只读，旧明文清理由独立clean-plaintext --confirm处理。", hero: false },
    { label: "来源回归与运行环境", value: "2.3.1来源验收记录为137项核心测试及30项子用例，通过虚构文件、虚构密码与隔离目录验证事务、双槽、原件保留、导出、资源预算、精确计划和新副本重建。本网页未重跑该源码全套；下列9月3日实际算法样例继续保留原日期和范围。", hero: false },
...
export const vaultToolProject = {
  order: 13,
  slug: "vault-tool",
  technicalSections: [{"title":"来源读取与精确判定","paragraphs":["输入来自本人明确选中的文件及本次本地操作。普通原件、暂存目录、密文、密钥材料和远端副本分别处理，不把整个电脑变成保险库。","电脑上选定的文件和子目录：文本文档、清单、图片、PDF 或任意二进制文件。引导式入口把指定路径复制到 source/，打包保留相对目录；原路径副本与 source/ 暂存不是同一件事。；得到经过写入和读回验证的vault.enc；source/及原路径文件默认保留。清理输入是另行明确选择，只删除已归档且仍与本次快照完全一致的对象，新增、变化或无法确认的文件保留。","已有的 vault.enc：先读格式和参数；只有在本人本地提供相应凭据后才解出内容。追加需要打开旧库，纯结构检查不需要密码。；可查看的选定文本、明确提取的附件，或经确认维护后的新库；不能从“格式有效”推断内容一定能解开。","本人在本地输入的密码和选定密钥文件：密码与密钥文件字节共同参与密钥派生；密钥文件不是指向原文件的快捷方式，换一个内容相近的文件也不等价。；本机内完成解锁或加密，AI 只得到不含秘密的状态。密码或所需文件丢失就无法恢复，不存在云端找回按钮。","明确提供的诱饵材料或封面图片：诱饵材料用于另一个密码对应的内容层；图片载体把已有密文追加在封面字节后，不修改像素来藏字。；双密码容器或可重新提取密文的图片文件；二者都不能代替密文备份和真实恢复检查。","明确指定、现场确认私有的 GitHub 目标：普通备份上传选定密文。另有独立的远端 README 保护动作，只在本地辅助进程内读取选定版本并加密；不是日常状态读取。；密文提交与读回结论；README 保护会同时更新当前说明，但不改写旧历史，也不代表已合并原有保险库。","网站公开展示的是工具、配置类别、虚构样例与验证结论；本轮没有读取或展示本人真实库里的文件名、正文、密码、密钥文件或个人恢复材料。"]}],
  usageEntry: "在已接通本机 vault-workflow 的 AI 对话中说清要检查、加密、查看或备份哪份明确文件；实际解锁只在本人本地窗口完成。",
  usageInputs: ["要处理的文件或库","想做的动作及保留或替换意图","需要解锁时本人在本地掌握的凭据"],
  title: "vault-tool",
  kicker: "文件由自己加密，取回和维护都保留退路",
  route: "/projects/vault-tool",
  visibility: "公开仓库",
  statusTone: "mixed",
  cardStatus: "默认保留原件；改密、导出与失败恢复各自回读",
  cardStatusTone: "pass",
  ...vaultToolSnapshot,
  ...
  summary: "把明确选中的文件和文件夹装进一份加密文件，需要时由我在本机输入密码，查看文字、寻找文件或只取出要用的附件。新建和追加默认保留原件；改密码只改当前打开的一层，另一层保持原样。容量不够或需要更换密钥文件时，先生成可验证的新副本，不拿唯一旧库试错。AI 可以检查状态和准备操作，不代管密码，也不能找回遗失的解锁条件。",
  ...
  dataSources: {"title":"要给它什么，文件会去哪里","intro":"只处理本人明确选中的文件和本次动作；原件、暂存文件、密文与恢复凭据分开说明，不扫描整台电脑。","rows":[{"source":"选中的文件和目录","data":"可以是文档、图片、PDF 或其他文件；加入前核对目标和同名关系。","result":"得到可读回的新密文。原件和准备阶段的明文默认保留，清理要另行明确。"},{"source":"已有加密库","data":"先识别格式；要追加或看内容时，由本人在本地输入对应凭据。","result":"可以取得选定文字或文件，或形成经核对的新库；只看格式不能证明能解开。"},{"source":"本人持有的密码和密钥文件","data":"解锁条件由本人本地提供；系统不会替本人找回或托管。","result":"AI 只收到动作状态。丢失必要凭据时不能凭网页恢复内容。"},{"source":"双密码材料或封面图片","data":"按明确需要分别指定两层内容，或选一张图片承载已有密文。","result":"分别试开两层或比对提取密文；图片外观不证明内容可解。"},{"source":"已确认的私人远端","data":"普通备份只送选定密文，远端说明文件的保护是另一项明确动作。","result":"读回密文字节；密码、密钥文件和真实恢复仍要另外保全与验证。"}],"note":"公开页面只解释工具与虚构验证，不展示真实私库正文或凭据。"},
  productPrinciples: [
    ...
    {
      "title": "先保住原件，再谈清理与扩容",
      "detail": "生成密文不会顺手消灭输入；容量或共享凭据需要变化时，先做独立新副本。只有本人明确选择清理，才核对本次确已归档且没有变化的文件，不能用“加密成功”授权清掉别的内容。"
    },
  ],
  responsibilities: ["把本人选中的文件放进加密库，明确是新建、追加还是替换。","在本机查看或搜索选定文字；只有外部软件需要文件时才按名称提取。","说明密码、密钥文件和旧格式对以后恢复有什么影响。","改密或升级时保留旧库恢复位置，并分别说明暂存明文和备份还在哪里。","按明确选择建立双密码内容层，或让图片带一份已有密文；容量和恢复条件不满足就停止。","让 AI 做不含原文的检查和建议，实际解锁、查看与编辑留在本人本地界面。","明确要求时把密文放入已确认的私人远端，读回相同字节；说明上传和真正恢复的差别。"],
  exclusions: [
  ...
  glossary: [
    ...
    { term: "metadata-only（仅元数据）", meaning: "只读格式、尺寸、参数与目标状态，不解密、不列私人文件名、不执行建议。文本和JSON输出都保持只读，--json只改变表达方式。" }
  ],
  ...
  operatingFlow: [{"title":"先核对文件与目标库","detail":"AI 检查指定原件、库结构和本次动作；旧库成员与同名关系须等本人本地解锁后才能确认，避免把追加误做成替换。"},{"title":"本人只在本地解锁","detail":"需要打开内容时由本人在本机窗口提供密码和密钥文件；工具处理已解锁的范围，AI 只取得操作状态。"},{"title":"核对结果与恢复条件","detail":"新密文要能按本次凭据取回预期内容；原件、旧库、备份和远端字节分别核对，不把其中一步写成全部恢复。"}],
  components: [
  ...
  usageExamples: [
    ...
    { moduleSlug: "files-encryption", ask: "把这三份文件加进原来的库，其他文件不要丢。", effect: "用明确添加入口，在内存中合并新文件和当前解锁层；同名关系先说明，其他未解锁槽位保持原密文字节，原件默认保留。直接encrypt仍不是追加入口。" },
    ...
    { moduleSlug: "maintenance-recovery", ask: "换一个密码，失败时原来的库还能用。", effect: "验证旧凭据后只重加密当前打开的VAULT03槽位，保留原KDF和另一未解锁槽。需要换共享密钥文件或扩容时另建新副本，原库保留；失败交回真实恢复位置。" },
    ...
    { moduleSlug: "ai-local-interface", ask: "只检查这份加密库的结构和环境，别打开内容。", effect: "对我指定的那份库读取结构、参数和恢复环境，不弹密码框、不解密、不清理旧文件；文本或JSON都只读。结构通过仍不证明密码正确或内容已经恢复。" },
    ...
    {
      "moduleSlug": "maintenance-recovery",
      "ask": "这份库快放不下了，换一份更大的，但旧库先别动。",
      "effect": "先检查空间、格式和所需凭据，再建立独立新副本并读回。提供两个已知密码时分别核对两层；新副本成功前旧库保持原样，没有密码的层不会被猜测重建。"
    },
  ],
  evidenceLayers: [
  ...
  operationalEntrypoints: [
    ...
    { name: "严格只看元数据", command: "python vault_tool.py info --vault-file <明确密文路径> --json", purpose: "info、doctor、assess、plan的文本和JSON输出均只读；--vault-file只检查这份目标，缺失或损坏不会回退检查默认库。" },
    ...
    { name: "更换密码", command: "python vault_tool.py passwd", purpose: "本人提供原密码和新密码；VAULT03只更新当前解锁槽，另一槽与原KDF保留。共享密钥文件变化进入rebuild，不用一次打开的层替代整份库。" },
    ...
    {
      "name": "恢复环境虚构自测",
      "command": "python vault_tool.py recovery-check --vault-file <明确库> --self-test --json",
      "purpose": "只用虚构口令和内存载荷检查当前运行环境，不解密真实库；通过不等于真实凭据或新机恢复已经验证。"
    },
    {
      "name": "凭据变更与独立新副本",
      "command": "python vault_tool.py credential-plan --vault-file <明确库> --change keyfile --json; python vault_tool.py rebuild --out <新库路径>",
      "purpose": "先说明改密、共享密钥文件变化、迁移与重建的区别，再由本人明确选择。rebuild输出不得覆盖原库，可用两个已知密码保全两层。"
    },
  ],
  evolution: [
    {
      ...
      "title": "先能携带和按需取回私密文件",
      "commit": "",
      "result": "把选择的文件装入本地加密容器，提供文本查看、搜索、附件提取及旧格式读取；密码、密钥文件和运行环境共同决定能否恢复。",
      "evidence": [
        {
          "date": "2026-08-30",
          "note": "结构检查开始拒绝截断容器；结构有效不等于内容已经解密或恢复。",
          "commit": "c117bd2"
        }
      ]
    },
    {
      "date": "2026-09-03",
      "title": "查看、编辑和备份各自说清结果",
      "commit": "",
      "result": "本地查看与编辑分开，未改成员和未解锁槽位不能被随意丢弃；远端保存要重新读回密文字节，不把命令成功当成可恢复。",
      "evidence": [
        {
          "date": "2026-09-03",
          "note": "追加与维护保留内容、加密参数和未解锁层；本地查看与编辑分开，私人远端保存核对实际字节。",
          "commit": "0b9d4c4 / e67cca1"
        }
      ]
    },
    {
      ...
      "title": "日常维护不拿唯一原件冒险",
      "commit": "57ad50b",
      "result": "新建默认保留输入，改密和追加保留另一个槽位；扩容或共享密钥文件变化先生成独立新副本。导出不覆盖同名冲突，部分写入和恢复失败如实报告。"
    }
  ],
  "readerBoundary": "密码及必要密钥文件由本人保管，遗失没有找回服务。模型只接收状态；本地可查看不等于没有内存、剪贴板或磁盘痕迹，真实私人库恢复仍需独立验证。",
};

export const vaultToolModules = [
  {
    slug: "files-encryption",
    usageEntry: "在已接通本机 vault-workflow 的 AI 对话中说“把这些明确文件追加到这份库”；本人在本地窗口输入凭据。直接 encrypt 是新建或替换路线，不能当追加。",
    usageInputs: ["要加入的文件或目录","现有库或新库目标","追加、替换及同名文件的选择","需要解锁时本人在本地掌握的凭据"],
    productFlow: [{"title":"先核对输入和库结构","detail":"工具检查选定原件、目标库格式和追加或替换的影响；旧成员的同名关系要在本地解锁后才能确认。"},{"title":"本地解锁并装入","detail":"本人只在本机提供凭据，工具打开相应层并处理同名文件，再把明确新文件合并加密；其他打不开的层保持原密文。"},{"title":"核对再交付","detail":"读回新容器中的预期文件并报告成功或失败；原件默认仍在原处，清理要另行明确。"}],
    shortTitle: "文件入库", title: "把明确文件装进库，先分清追加与替换",
    searchAliases: ["文件怎么加密", "文件夹加密", "加密库添加文件", "新文件覆盖旧内容", "加一个文件别丢原来的", "source暂存目录"],
    searchProjection: { intents: ["加密选定文件", "向旧库添加文件", "区分追加与替换"], entities: ["source", "vault.enc", "add_files_mode", "encrypt", "Start-KeyVaultEncrypt.ps1"], relations: ["菜单先合并旧库", "直接加密只处理source", "原路径与暂存副本不同"], failureRecovery: ["同名冲突明确处理", "取消不当成功", "自检失败保原文", "未解锁槽位保留原密文字节"] },
    ...
    readerStates: { pass: "范围明确，合并次序与内容核对正确，密文经过写入和读回验证；原件默认保留，明确选择的清理另报结果。", problem: "同名、新旧版本、另一层或写入结果无法保持时先说明，不悄悄丢文件。", unavailable: "没有文件、路径不可读或本人取消密码输入时停止本次操作，不把空库或取消当成完成。" },
    decisionImpact: ["已有库要追加时用菜单添加；encrypt 和启动脚本不会自动合并。", "source/和复制前的原路径都默认保留；--cleanup-source只处理本次已归档且仍完全相符的文件，不清理新增、变化、占用、硬链接或无法确认的对象。", "菜单追加只修改当前解锁层，未解锁槽位保留密文字节；直接encrypt是另一种重建意图，不把一个密码可打开误认为整库所有层都已验证。"],
    ...
    flow: ["固定文件与目录范围，区分保留原库、追加与整库替换。", "已有库追加时先由本人提供对应凭据，并说明只能打开相应层。", "处理旧库与暂存区的同名关系，再复制本次明确新路径。", "打包、压缩、加密并核对新容器可解回预期内容。", "核对密文写入和读回；默认保留原件，若明确选择清理，另报实际删除、保留与未确认项。"],
    ...
    boundaries: ["任意文件类型可以作为字节入库，不代表任意文件都能在内存文本界面直接阅读。", "没有后台目录监听、全盘扫描或自动上传。", "合并的旧库内容只在内存处理；新输入的source/仍是明文且默认保留。内存合并不等于整个程序或操作系统没有其他内存副本。"],
    failures: [{ condition: "准备区已有同名新版", response: "按明确的新旧优先顺序保留预期版本，并用实际内容回归；不能只验证成员数量。" }, { condition: "取消、没有输入或新库自检失败", response: "返回未完成或失败，保留尚未成功提交时需要的原文与原库，不输出完成证据。" }, { condition: "旧库可能含另一密码对应的层", response: "菜单追加和VAULT03槽位改密保留未解锁槽位的密文字节，不声称已经打开它。共享密钥文件、容量或格式需要变化时生成新副本，不能静默覆盖未知层。" }],
    ...
    readerStatus: "文件打包、追加及同名新版保留已有验证；原件默认保留，真实私人库本轮没有处理。"
  },
  {
    slug: "view-extract",
    usageEntry: "在已接通本机 vault-workflow 的 AI 对话中指定库与要看的文字；使用 LocalView，本人仅在本地窗口解锁。要给外部程序用的文件再明确提取。",
    usageInputs: ["要查看的库和文字线索","若需提取文件，指定成员与落盘位置","本人在本地窗口输入的解锁凭据"],
    productFlow: [{"title":"只打开所需范围","detail":"本人本地解锁后按名称或内容找目标，不一次性导出整库。"},{"title":"选择查看或提取","detail":"支持的文本在本地界面阅读；外部程序必需的文件才按明确名称写到指定目录。"},{"title":"结束时核对残留","detail":"区分内存、剪贴板与已提取文件；中断时报告实际残留，不能称自动清完。"}],
    shortTitle: "查看与提取", title: "只看要用的文字，必须取文件时再明确落盘",
    searchAliases: ["加密文件怎么打开", "加密后怎么取回来", "不解压看文字", "保险库搜索", "只提取一个文件", "明文残留", "剪贴板清除"],
    searchProjection: { intents: ["查看库内文本", "搜索一个成员", "只提取需要的附件", "处理提取残留"], entities: ["decrypt", "--no-disk", "--extract", "decrypted", ":copy", "Ctrl+X"], relations: ["内存查看不同于磁盘提取", "名称搜索不同于内容搜索", "剪贴板独立于正文缓冲区"], failureRecovery: ["错误凭据停止", "强杀可能残留", "旧明文只在明确清理时处理", "重解析点拒绝清理"] },
    ...
    readerStates: { pass: "支持的文本可显示和搜索，筛选提取只交付匹配文件；结束时说明相应缓冲区和目录处理结果。", problem: "冲突、部分导出、格式不支持、清理失败或进程中断时，分别列出已存在、已写入、冲突与保留范围；不覆盖同名文件，不用退出码代替内容完整性。", unavailable: "密码或密钥文件不匹配时不显示内容；来源不存在或格式损坏时不生成空白成功页。" },
    ...
    failures: [{ condition: "图片、PDF、二进制或超出显示限制的文本", response: "说明不能在该文本入口完整显示，改由本人明确选择是否提取，不把部分文本当完整附件。" }, { condition: "进程提前结束或断电", response: "剪贴板计时和退出回调可能来不及执行；明文残留先定位，再用明确的清理入口处理。诊断不会自动删除，不能声称断电当场已清除。" }, { condition: "暂存清理发现链接或删除失败", response: "停止不适合的删除并报告具体范围，不顺着链接清别处文件。" }],
    ...
    readerStatus: "已有本地文本查看、搜索和选定文件提取；冲突不覆盖，部分导出与明文实际落盘分别报告。"
  },
  {
    slug: "passwords-formats",
    usageEntry: "在已接通本机 vault-workflow 的 AI 对话中说“检查这份库为什么打不开”；先用只读 Inspect，真正恢复由本人在本地提供密码及密钥文件。",
    usageInputs: ["打不开的库","本人是否还持有密码和原密钥文件（在本地提供）","具体错误或换机情境"],
    productFlow: [{"title":"先辨认格式","detail":"只读检查库结构、派生参数、密钥文件要求与本机依赖。"},{"title":"本人本地尝试恢复","detail":"凭据齐全后按原格式真实解密并认证；密码或密钥文件缺失时停止，不猜测重置。"},{"title":"交回实际结果","detail":"成功才确认取回内容；结构合格或参数存在只能说明可尝试，不能算恢复。"}],
    shortTitle: "密码与格式", title: "密码、密钥文件和环境齐全，才谈得上恢复",
    ...
    example: "“我记得密码，新电脑却打不开库。”工具先看这份库当初要求的格式、密钥文件和运行环境；必要条件缺失会直接说明，不请本人把密码发到聊天。",
    result: "先得到这份库需要哪些条件的说明；本人在本地提供正确条件并真实解开后，才得到内容或具体失败原因。结构检查不能代替实际解密。",
    ...
    readerStatus: "已有现行格式加密和旧格式读取能力；仍需正确密码、必要密钥文件及兼容环境，没有密码找回功能。"
  },
  {
    slug: "maintenance-recovery",
    usageEntry: "在已接通本机 vault-workflow 的 AI 对话中明确说“改密”“迁移格式”或“清理旧备份”其中一项，并指定库；本人在本地提供旧凭据。",
    usageInputs: ["要维护的库","本次是改密、迁移还是清理","新库或旧备份的保留选择","本人在本地提供旧凭据"],
    productFlow: [{"title":"分清维护目标","detail":"确认当前格式、层次和旧库位置，说明本次会生成哪份新库以及保留哪些旧文件。"},{"title":"生成并验证","detail":"本人提供旧凭据；工具只处理可打开的范围，写出新结果并回读，失败时保留原库或恢复原状。"},{"title":"确认后处理旧件","detail":"逐项交代新库、临时备份和恢复位置；旧备份清理另由本人明确决定。"}],
    shortTitle: "库维护", title: "改密、升级和清理都要说清旧库还剩在哪里",
    ...
    status: "槽位改密、可验证新副本与事务回退已实现；真实私人恢复另验", statusTone: "pass",
    value: "改密码时保留原来没改的内容，容量不够或要换密钥文件时先做另一份经过验证的库。新副本未成功前旧库不动；中途失败会说明原路径和恢复副本各是什么状态，而不是只留一句失败。",
    why: "改密码和升级都会生成新密文。操作中的临时副本、明确保留的旧库和远端备份各有不同去向；把它们都叫“自动备份”，出问题时反而可能找不到能恢复的那一份。",
    example: "“给这份库换密码，先确认原来能打开的文件仍然在；失败时不要损坏旧库。”流程要先验证旧凭据，再核对重建范围与自检，并明确旧库副本是否会在成功后删除。",
    result: "得到经验证的新密码槽位、升级后的库或独立重建副本，并说明保留了哪些层、原库和恢复副本在哪里。冲突、取消和回滚失败单列；未知层的字节保留不等于已经验证其密码。",
    ...
    decisionImpact: ["旧版升级与当前库改密是独立操作，不需要为了普通查看先升级。", "临时备份成功后可能删除；不能把“执行时有备份”当成永久恢复点。", "VAULT03原槽改密保留另一槽；改变共享密钥文件标志或容量必须走rebuild新副本。两密码重建需要两个已知密码分别打开不同槽位，不猜隐藏层是否存在。"],
    ...
    verification: ["真实虚构探针已验证旧密码拒绝、新密码往返，以及 V2→V3 内容相同并留下 .bak。", "9月3日pwbak样例只证明当时实现；2.3.1新增槽位保全、新副本重建和不覆盖旧备份的隔离回归。网页不将历史算法样例改写为本次真实私人库维护。", "本轮没有清理用户任何真实备份或真实维护另一密码层。"],
    relation: "日常内容追加在文件入库模块；双密码容器独立说明两层；远端密文保存与逐字节读回在密文备份模块。",
    readerStatus: "已有只改已打开内容层、生成新副本及失败回退的维护功能；真实私人资料恢复仍需独立验证。"
  },
  {
    slug: "dual-password",
    usageEntry: "在已接通本机 vault-workflow 的 AI 对话中明确提出建立双密码库，并分别指定真实层与诱饵层材料；两套凭据由本人在本地输入。",
    usageInputs: ["两个密码分别应打开的材料","要新建还是从旧库迁移","目标库位置","两套凭据由本人在本地输入"],
    productFlow: [{"title":"确认两套内容","detail":"分清每个密码预期打开什么，并检查原库和可用容量；装不下就停止。"},{"title":"本地创建两层","detail":"工具在保留原库恢复路径的条件下生成双层容器，不替本人保管密码。"},{"title":"分别试开","detail":"两套密码各自解开并比对预期成员；报告历史旧备份仍可能泄露先前状态。"}],
    shortTitle: "双密码容器", title: "两个密码打开不同内容，但不承诺现实中的不可识别",
    ...
    readerStatus: "已有两个密码分别打开不同内容的功能和容量检查；测试证明软件行为，不保证现实胁迫或第三方分析下无法识别。"
  },
  {
    slug: "image-carrier",
    usageEntry: "在本机 vault-tool 使用 hide，把已有密文与封面图片写成新的载体；取回时用 unhide 指定独立输出路径。",
    usageInputs: ["已有密文和封面图片","新载体及提取文件的保存位置"],
    productFlow: [{"title":"选定已有密文和封面","detail":"这是给密文加图片载体，先确认密文本身已生成且另有恢复条件。"},{"title":"写入并取回","detail":"工具把密文附在图片字节后，再从载体提取到独立文件。"},{"title":"比较密文字节","detail":"核对原密文与提取物完全相同；图片能打开不等于密文可恢复，重编码也可能破坏尾部。"}],
    shortTitle: "图片载体", title: "图片后面带一份密文，取回时核对的是密文",
    ...
    readerStatus: "已有在图片尾部携带和提取密文的功能；平台重编码可能破坏载荷，图片能看不代表密文仍完整。"
  },
  {
    slug: "ai-local-interface",
    usageEntry: "在已接通的 AI 对话中调用 vault-workflow，明确说“只检查”“只给建议”“本地查看”或“本地编辑”，并指定精确库；查看/编辑需本人操作本地窗口。",
    usageInputs: ["精确库与这次只查、查看或编辑的选择","要查看或修改的文本成员","实际解锁时本人在本地输入凭据"],
    productFlow: [{"title":"先选效果","detail":"只读检查和建议不弹解锁框；查看或编辑才进入本机界面。"},{"title":"只处理所选内容","detail":"本人在本地解锁，只读取或修改可保真的明确文本成员；格式或容量无法保留时停止。"},{"title":"核对动作状态","detail":"AI 得到取消、查看、保存和自检状态，不取得原文或密码；失败不能写成已经保存。"}],
    shortTitle: "AI 与本地入口", title: "只查状态不打开内容，要读写就明确进入本地操作",
    ...
    result: "只读路线交回结构、环境和建议；本地路线按实际动作报告取消、查看或保存，不把原文和密码发给 AI。若编辑无法保留原库内容就停止；写后自检与恢复都失败时，明确说明目标可能已经变化，并交回实际加密恢复副本。",
    ...
    decisionImpact: ["info、doctor、assess、plan的文本与JSON都只读；--vault-file绑定精确目标。清理、解密、编辑与发布必须走各自明确操作，输出格式不改变副作用。", "ok 表达检查调用是否完成时，不等于库可以解密；具体风险和 decision 仍要看。", "保留另一未知层可能限制编辑容量；容量不足不是让工具悄悄扔掉那一层的理由。"],
    ...
    implementation: [
      ...
      "credential-plan解释原槽改密、共享密钥文件变化、迁移和重建；recovery-check --self-test只做虚构环境往返。CLI、GUI和Skill共用归档、槽位、事务与资源预算原语；VerifyPassword只是一次本地检查，不颁发可复用会话。",
    ],
    ...
    boundaries: ["库元数据通过--vault-file选择精确文件；VaultRoot仍表示工具目录，不把工具位置当另一份库。恢复自测只用虚构口令和内存内容，不认证真实密码。", "GUI 替身与真实加密回归不能冒充本人已在真实桌面输入密码并完成验收。", "Skill、独立 vault-tool 与密码中心各有来源和作用域；修网页不授权读取真实库或改全局规则。", "不因旧接口名字含 prompt 就藏掉现有读写功能，也不为修复增加服务、密钥托管或通用文档编辑器。"],
    ...
    readerStatus: "只查状态与本地查看编辑已有隔离验证；密码由本人本地输入，真实私人库与桌面操作本轮未验收。"
  },
  {
    slug: "private-backup",
    usageEntry: "在已接通本机 vault-workflow 的 AI 对话中明确要求“把这份密文备份到已核准的私人 Key”，或单独要求保护 README；两件事分别确认。",
    usageInputs: ["要备份的密文或要保护的 README","已核准的私人目标和路径","本次只做哪一种动作"],
    productFlow: [{"title":"确认目标与动作","detail":"现场核对仓库私有状态、默认分支、路径及本次究竟上传密文还是处理 README。"},{"title":"写入并远端读回","detail":"按明确选择执行，然后从默认分支核对对应密文字节和分支；效果不明时停在未知。"},{"title":"分开报告恢复条件","detail":"字节一致只证明密文备份；真正取回内容还要密码、密钥文件和实际恢复演练。"}],
    shortTitle: "密文备份", title: "远端保存同一份密文，和真正能恢复分开验",
    ...
    result: "普通上传与README保护都分别报告实际写入和远端回读。README保护还核对当前说明及密文字节，响应丢失时只回读原操作；仍不能确定就保留效果未知。字节一致不证明密码可恢复，也不表示旧历史已清除或既有保险库已合并。",
    readerStates: {"pass":"普通备份必须从私人目标读回同一份密文；单独保护远端说明时，也要核对当前说明与新密文。两者都不代表已经凭密码恢复原文。","problem":"目标或远端内容在操作中改变、写入结果不明或读回不符时，分别说清是否已经写入，不给笼统成功提示。","unavailable":"私人目标、权限或本人本地确认不成立时停止写入；预演和只读检查仍不算发布。"},
    ...
    implementation: [
      ...
      "VerifyRemote的树路径检查仍只证明路径存在，safe_readme_present不是正文安全证明。ProtectRemoteReadme另按完整字节核验受管占位说明，不接受仅含marker；existing_stub_verified不表示检验过已有密文或密码。",
      "ProtectRemoteReadme 是独立精确动作：WhatIf 不读正文、不取密码、不写远端；真实执行由本地辅助进程读取固定 source_head_sha 的 README，要求两次本地密码一致，打包单个 README 并自检。",
      "正文保护写入前重新核对PRIVATE、固定source_head_sha的完整树和目标。截断树不能证明文件不存在；已有vault/vault.enc而README仍为普通正文时拒绝覆盖密文。非强制提交后回读分支头、README与密文并比较SHA-256；响应丢失只核对原结果，不盲重放，无法确定时effect为null。正常换行Base64及固定blob读取均受支持。",
      "恢复还需要对应密码、密钥文件、容器版本与依赖。本地 .bak、远端密文、程序测试和真正打开原文是不同恢复证据。"
    ],
    flow: ["明确普通密文备份，或另行明确远端README保护；不自动合并两种意图。", "现场解析目标、私有状态、默认分支和精确路径，必要时先预演。", "普通备份上传明确密文，再从默认分支读取blob字节和指纹。", "README保护在本地确认后处理固定来源，自检候选，非强制写入后回读README、密文和分支；已经保护、实际写入和效果未知分别报告。", "分别交回写入、字节核对与仍未进行的密码恢复，不把三层合称都已验收。"],
    ...
    readerStatus: "密文上传与远端字节核对接口已有验证；本轮未操作私人远端，真正解密恢复仍要另验。"
  }
];
