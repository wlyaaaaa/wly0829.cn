...
export const vaultToolProject = {
  ...
  slug: "vault-tool",
  title: "vault-tool",
  kicker: "本地文件加密与恢复 · 原件保留、按需取回与可核对维护",
  ...
  dataSources: {
    ...
    rows: [
      { source: "电脑上选定的文件和子目录", data: "文本文档、清单、图片、PDF 或任意二进制文件。引导式入口把指定路径复制到 source/，打包保留相对目录；原路径副本与 source/ 暂存不是同一件事。", result: "得到经过写入和读回验证的vault.enc；source/及原路径文件默认保留。清理输入是另行明确选择，只删除已归档且仍与本次快照完全一致的对象，新增、变化或无法确认的文件保留。" },
...
export const vaultToolModules = [
  ...
  },
  {
    ...
    readerStates: { pass: "普通上传核对默认分支密文；README保护核对固定来源、当前README与密文字节及分支结果。已经是安全说明时仅确认完整受管占位文本相等，不冒充验证过密码或既有密文。", problem: "目标、并发版本、写入或对应路径的核验不成立时说明哪一步失败及是否已经写入，不打印笼统成功。", unavailable: "账号、权限、目标身份或必要本地确认不成立时停止该远端动作；预演与只读仍不冒充发布。" },
