...
export const modules = [
  ...
  },
  {
    ...
    relation:
      "它从规则内核取得方法边界，向授权模块提交精确动作与 scope；需要 Skill 时读取供应模块，最终把运行结果交给证据模块验收。",
  ...
  },
  {
    ...
    problem:
      "多人或多任务同时工作时，最危险的不是缺一个锁，而是把授权、施工责任和事实来源混为一谈。这个模块解决外部影响是否被授权、谁可以写哪个 scope，以及交付失败后责任如何连续。",
    actions: [
      "本机可逆、低风险且在范围内的编辑与测试直接推进；消息、外部修改、发布、付费等 external effect 需要明确对象、内容和动作。",
      "同一目标内的验证、正常重试、发布、回读和 Git 收口由持续授权覆盖；扩大目标、首次公开、付费、force-push 或不可逆迁移重新进入精确门禁。",
      "执行 Owner 通过 revision CAS Claim 最小 scope；同一 holder 扩展用 Add，改派或 whole-project 转换使用正式 transition，写前再用 AuthorizeAction 匹配 action、task、scope 和 revision。",
      "Git 完成把业务结果和仓库收口分开；个人仓库的目标提交必须从真实 default branch 可达，normal push 后再从远端回读。"
    ],
    rationale:
      "授权回答“用户允许什么”，执行 Owner 回答“当前由谁施工”，事实 owner 回答“哪个来源说了算”。分开之后，并发可以只串行真正冲突的写入，任务中断也能通过 checkpoint 和正式交接恢复。",
    boundaries: [
      "Claim whole_project 不会自动产生发布授权、管理员权限或私人正文读取权。",
      "UAC 只提升进程 token，不扩大任务范围；私有目标也不等于可以任意写入。",
      "超时、自报完成或一段 JSON 不能证明旧 Owner 已终止；有残留义务时不能直接 Release。"
    ],
    sources: [
    ...
    relation:
      "能力路由产出精确的执行方法，本模块把方法绑定到授权与 Owner；受保护策略生命周期为重大 effect 提供可信执行入口，证据模块负责最终回读。",
  ...
  },
  {
    ...
    problem:
      "如果保护重大动作的规则本身能被普通工作区修改直接替换，保护就没有可信起点。这个模块解决候选规则如何进入活动状态、运行时如何验证活动版本，以及完整性异常时怎样失败关闭并保留恢复链。",
    actions: [
      "规则按 candidate → protected publish → active generation 流转；工作区内容永远只是候选，不能因文件较新就直接成为活动权威。",
      "固定状态入口一次验证规则集合、签名、单调锚、发布与消费回执、provider 绑定和 owner adapter；模型只接受已验证活动版本。",
      "候选等待发布不会降低当前活动版本，候选暂时不可用也不会删除既有活动能力；但两种状态都不能被用来猜测或恢复候选。",
      "重大 effect 在执行边界重读现场 reference 与 resolution，核对动作、可见性、前置条件、回滚和 read-back；正式 receipt 与真实回读缺一不可。"
    ],
    rationale:
      "把候选和活动状态彻底分开，允许规则正常演进，也让运行时有一个独立于工作树的可信答案。异常时只冻结受影响的重大动作，普通低风险工作不被误判为攻击。",
    boundaries: [
      "普通文件编辑、可回退重构、测试或候选差异不会仅因位置敏感自动变成重大动作。",
      "仓库里的 manifest、自签状态或截图不能证明生产激活；动态代际、hash 和 ledger 也不适合作为长期文档内容。",
      "保护合同不拥有授权和执行 Owner 语义，只验证重大 effect 所需的活动策略与执行链。"
    ],
    sources: [
    ...
    relation:
      "规则内核定义哪些语义需要保护，授权模块提供精确目标与步骤能力，本模块保证重大 effect 使用的确实是活动策略；完成证据模块再独立确认产品结果。",
  ...
  },
  {
    ...
    actions: [
      "唯一供应清单记录 canonical source、类型、是否安装和退役项；用户目录只作为 discovery，不承担源码事实。",
      "事务 installer 管理 discovery junction，先验证目标、来源和已有占用，再切换并回读，避免普通复制形成多个真相。",
      "Skill 只描述精确触发条件和安全入口；Plugin 可以把 Skills、MCP server 与 App 打成一个本地 bundle，但不会因此获得额外权限。",
      "验证明确区分 source、install、current、fresh task 和端到端能力；某一层 PASS 不能替代另一层。"
    ],
    rationale:
      "重复需求值得一个可发现、可维护的窄入口；一次性需求不值得制造长期 Skill。把 canonical source 与 discovery 分开，可保持安装简单，同时让升级与回滚仍有唯一依据。",
    boundaries: [
      "Skills 不是项目，也不与 `.agents` 强制归属；本网站的 Skills 目录只是现行供应的只读介绍。",
      "Skill 是建议性方法，不能制造授权、提权或人类确认；插件账号也不会扩展用户原本没有的权限。",
      "已有 provider 只缺一小段能力时，优先修复原路线，不以重装公共插件或静默切换账号代替。"
    ],
    sources: [
    ...
    relation:
      "能力路由在真实需求出现时选择 Skill 或 Plugin；规则内核限制它们不能自升为门禁，授权模块控制外部 effect，上下文与证据模块检查 fresh task 是否真正可用。",
  ...
  },
  {
    ...
    problem:
      "长任务容易在压缩、交接或外部状态变化后丢掉关键决定；同时，测试通过、部署成功和用户真的能用常被混成一个 PASS。这个模块解决需要读多少上下文、怎样恢复任务，以及什么证据足以支持完成结论。",
    actions: [
      "上下文按问题渐进获取：先读项目规则与最小元数据，只有事实会改变决策时才进入相应 owner，不做周期性全量扫描。",
      "复杂或长程工作把目标、硬边界、关键决定、当前实现和验证状态保存在正确项目产物或宿主持久任务状态中，便于压缩后重建。",
      "证据健康区分 source、test、install、publish、fresh task、E2E 和用户可见验收；每层说明观察时间、来源与真实缺口。",
      "完成时既检查产品路径，也检查 Git 和外部发布回读；缺失或陈旧证据保持 unknown 或 blocked，不用内部回执补成绿色。"
    ],
    rationale:
    ...
    boundaries: [
      ...
      "没有某层证据不等于失败，但不能升为 PASS；历史记录也不能冒充当前现场。",
...
export const ideas = [
  {
    ...
    basis: "过程可以反复变化，结果是用户持续判断价值的坐标。把证据放在结果之后，不会降低严谨度，反而能暴露“内部 PASS、产品未完成”的落差。",
  ...
  },
  {
    ...
    title: "验证要能推翻结论",
    summary: "好的验证不是寻找一个 PASS，而是主动检查哪些事实会证明当前判断错误。",
    ...
    basis: "可证伪验证迫使结论与真实世界连接，也让 unknown 保持诚实。独立证据层不是增加流程，而是减少错误完成。",
  ...
  },
  {
    ...
    definition: "只有实际重复、稳定变化点、外部 provider 差异或明确 owner 边界能降低总变更成本时，才建立适配层和复用结构。其他情况下保持直接实现。",
    problem: "面向假想扩展的架构会增加文件、配置和测试，却没有现实 consumer；真正变化到来时，它往往还需要重写。",
    use: [
      "未来模型或 provider 差异用窄适配器隔离，不建立全能平台。",
...
export const skills = [
  ...
  ["codex-local-remote-control", "查看 Codex Local Remote 状态或进行已授权的只读归档核查。", "为冻结运行时保留一个不会误启动、重启或接管服务的只读入口。", "用户询问该运行时状态，或明确授权查看本地归档时使用。", "已冻结能力最需要清楚的“不做什么”，避免诊断动作意外恢复旧服务。", "永不打开、关闭、重启或调用冻结运行时；恢复是另一个明确启动的项目。"],
  ...
  ["google-workspace-direct", "通过固定 provider 读取或精确操作 Gmail、Drive 与 Calendar。", "让 Google Workspace 请求复用同一账号与 provider 边界，并支持原生 Docs 导出。", "任务需要当前 Google 邮件、云端文件、文档或日历事实时使用。", "连接器能力和账号状态会变化，需要窄入口先确认 provider 实际暴露的动作。", "默认只读；只有 provider 真实支持且用户授权的精确写入才执行。"],
  ["local-secret-broker", "管理 PCConfig 托管的密码、密钥、Token、登录和其他凭据。", "用 SecretRef 完成查找、注入、备份、恢复、更新与退役，尽量不把明文交给模型或日志。", "任务确实需要凭据的库存、注入、恢复或生命周期操作时使用。", "凭据操作的最安全路径通常是盲填和引用，而不是复制明文。", "普通任务不展示明文；明文 reveal 需要用户明确的单项请求与最高权限因子。"],
  ...
  ["control-plane-doctor", "对 `.agents` 及其协作控制面执行明确范围的健康、漂移、迁移和恢复检查。", "按用户点名 scope 运行结构化验收，并把事实 owner 之间的差异说清。", "用户明确要求控制面健康、漂移、恢复或跨仓库验收时使用。", "这类检查跨越多个 owner，需要严格限制读取范围与结论来源。", "不做周期性全扫，不接管业务项目，也不因一处异常自动修改其他控制面。"],
