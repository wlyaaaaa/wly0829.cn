...
export const skills = [
  ...
  skill({
    ...
    sourceBytes: 6674,
    sourceSha256: "bcee060ff85c99ff2bfff24f34153cb77ba5599644233be5d28c0f3338d56029",
    name: "google-workspace-direct",
    title: "Google Workspace 固定入口",
    status: "已安装 / 固定 Google Workspace 服务入口已配置",
    maturity: "A-",
    summary: "查自己的Gmail和Google Drive时，它用既有同一账号把查找、实际读正文或文件，以及已获授权的修改连起来，不让我在每一步重复说继续。验证码请求会在原有邮件窗口里有界等待并核对服务和时间。Calendar（日历）当前冻结，不在这条本机入口中；连接成功也不等于邮箱、文件和写操作都已验收。",
    useWhen: ["按条件查邮件并读取实际正文或同一线程","查找、读取或导出Drive文件","本人明确要找刚刚发来的Gmail验证码","需要原生Google文档导出或普通TXT/DOCX/PDF正文","已明确授权的发送、回复、上传、修改或分享，而且现有Provider确实提供该动作"],
    avoidWhen: ["切换其他账号","静默改走浏览器、公共 Connector 或 rclone","Provider 没有暴露目标写操作","Google Photos、Admin 或支付","调用、启用或维护当前冻结的Calendar；本机入口限制不外推到其他客户端"],
    inputs: ["窄查询、对象 ID 或时间范围", "写操作时的精确对象、内容和动作授权"],
    outputs: ["最小必要邮件正文、线程或文件结果；验证码注明服务、收件时间及未解决的冲突，不把命中候选当作已验证有效", "原生文档导出", "写操作的 Provider 回执与 read-back", "普通存储文件经固定 Provider 的 DriveDownload 下载后的原生读取结果"],
    flow: ["已知任务直接走对应窄入口；gmail_read.py search按实际请求取得正文，不把只含ID/主题的搜索页当成读过邮件，也不先枚举所有工具。","选择固定 Provider 的精确 operation","读取实际需要的正文和上下文后完成当前请求；写操作仅在用户确切授权及已暴露能力内执行，同一线程回复、附件与最终对象分别回读。","原生文档必须走 Drive export","写入前核对 Provider 能力和授权","按同一 Provider read-back","Drive已知名称先准确找；只有当前完整搜索结果确实未命中才扩大条件，不能把第一页或最多五个候选当成所有文件。","“Gmail验证码”沿现有gmail_read入口看最近10分钟、相同服务及当前上下文；首次未见时每5秒查看、至少等待120秒并做最后一次回读，不重复算旧码，也不把过期邮件混成新验证码。","发生投递不确定时回读原对象和回执，不盲目重新发送；验证码候选须由当前任务读正文判明用途，不自动完成账号登录或真人验证。"],
    boundaries: ["不落中央库","不自动同步","不换账号或第二 Provider","删除、发送和公开分享需要精确授权；冻结Calendar不会因为一句邀请需求被启用。","MCP 只暴露元数据或没有 drive_download，不等于固定 Provider 没能力；普通文件可走已登记 CLI DriveDownload","普通文件用 files.get?alt=media，原生 Google Docs 必须 files.export；不能混用或切换第二 Provider","本机Codex实际取用普通个人参考前锁屏，取用档案性/私密资料前走已登记因子；先保护再读，不先查库决定是否验证。独立获准的电脑MCP沿自己的私密访问授权，不重复本机因子；本机Codex借MCP不属例外，共享冻结对全部入口生效。 邮件主题、正文和验证码本身属于所需私密内容；Drive按实际文件内容选保护，不按扩展名降级。","不因这个Skill存在而启动轮询服务、同步库或第二账号连接；有限等待只发生在本次明确验证码请求内。"],
    dependencies: ["PCConfig 的 google-workspace.primary Provider", "已登记 OAuth 绑定"],
    tests: "当前规范Skill与已发布PCConfig入口合同已读；Calendar冻结与Gmail/Drive零网络配置校验不证明OAuth有效、邮件正文可读或写操作完成。本轮没有读取真实邮件、验证码或文件，也未发送或上传。",
  ...
  skill({
    ...
    status: "已安装 / 正式环境回归通过",
    maturity: "A",
    summary: "把中文和微信录音转成可搜索文字，按需要附时间位置、匿名说话人或本人声音线索。定位已选录音片段先复用已有结果；明确允许上传时，同一个阿里云入口可做重要录音交叉验证，也可复核普通转写质量，二者不会混同证据等级。 已有稿子只想给配音对时，可明确选择只对齐而不再识别；它不证明稿子就是录音说过的内容。",
    useWhen: ["需要中文录音全文，或明确选择带时间位置的模式","需要区分匿名说话人 cluster","需要判断精确句段是否支持 person:self","重要专业录音需要双链验收","普通本地转写存在影响理解的疑点，且选定录音/任务已明确授权云质量复核","已有文稿和对应音频，只需字词时间用于配音/字幕，不请求重新识别正文","需要在本机应用里实时中文听写，或排查Win+H桌面听写"],
  ...
  skill({
    ...
    useWhen: ["查询、补充或纠正本人的基本情况、真实经历、生活重点、通用价值取舍与偏好","希望吃喝、购物、旅行、娱乐、工具或审美建议能结合本人已有经历，并解释为什么","某个领域任务需要共同本人背景，或产生有依据、值得以后使用的新认识或更正","用户明确要求更新偏好、按最新资料理解自己或准备投递增量","希望在手机或云端使用、补充和纠正电脑上的同一份本人背景"],
    ...
    sourceBytes: 7022,
    sourceSha256: "2298ddfb8709745f6114979946f736fbb9ad6b7177f296db70203af4615667e4",
  ...
  skill({
    ...
    flow: ["当前任务先确定事实、行动意图、沟通目的和关系分寸", "首次拟写或处理纠正前，先满足当前保护再读reply-guide.md；普通个性化参考先屏锁，展示历史通信或私密事实先因子，正常恋爱协助沿领域例外。已读未更新且保护仍有效可复用，指南更新后重读。", "需要样本时运行 expression.py scenes，再用 reply --scene 读取一份相关场景，不另开模型改写", "缺少匹配样本时保留指南、说明参考有限，继续依据既定意思拟稿", "保留必要原消息与已知媒体语境，不移植旧样本的人名、日期、地点或事件", "纠正先解决当前稿，再按项目 AGENTS 合并教训或按有界窗口补语料，校验和重读；发送留在调用方的独立动作与授权边界"],
    boundaries: ["不自行改变策略，不增承诺、道歉、邀约或亲密程度；普通措辞自主决定，只有会改变意思或结果的疑问才问","允许改善旧说法，不追求复刻；普通办事消息不套恋爱口吻","本人原话、本人修订与 AI 草稿保持不同来源，AI 草稿即使被发送也不自动成为本人样本","只在当前用途与已有范围允许时由现有微信/材料入口补读，不扫描全账号","反馈和语料维护由当前 AI 完成，不建自动学习服务；格式校验不能证明作者身份或表达合适","本人认可、真实媒体特别是动画语境下的效果，以及跨任务无提示路由仍须实际验收","表达库不再参与操作者身份判断，不能先读样本决定是否验证。保留author、source与语境，区分本人原话/修订、AI代拟、他人、引用和转贴；待验证或冻结期间不把当前说法、反馈或改稿当作新本人样本。独立MCP日常沿既有授权，共享冻结仍生效。"],
    ...
    sourceBytes: 3072,
    sourceSha256: "0b44b76edaf160c05617192f7cd9ab9008f9f7357457476f5db91a4d9fab7d1e",
  ...
  skill({
    ...
    sourceBytes: 9547,
    sourceSha256: "b2aaf14da468941eaf115114222b52f0706080e02133344503a681385d901ca8",
    name: "local-secret-broker",
    title: "本地凭据 Broker",
    status: "已安装 / 受保护",
    maturity: "A",
    summary: "需要查某项凭据是否存在、让指定程序或浏览器使用它，或维护备份恢复时，它走本机 Password Center（密码中心）的固定入口。普通查询只返回候选元数据与状态；账号或密码明文只在本人验证后的本地窗口显示，模型不接收。远程会话不便显示时，可使用遮盖内容的复制面板把指定字段放到这台电脑的剪贴板；是否同步到手机或另一台电脑，要另行确认。 它也管理每次私人资料访问的屏锁、因子与共享冻结：取消必需验证后，其他入口也不能沿用私人内容，但普通工作继续；这不是磁盘保护。",
    useWhen: ["查询某个账号或 SecretRef 是否存在","需要精确获取一个凭据字段","让浏览器或程序 blind fill","执行凭据备份、恢复或轮换","一个已登记本地载体需要在原环境缺失时独立恢复","本人明确查看一项账号/密码，或一次查看一整张银行卡","维护凭据、导入正式Google密码快照，或核对需保留的来源与副本","核对当前共享个人环境及本次资料访问范围","普通个人参考需要本地屏锁，或私密资料需要四选一因子","明确冻结个人环境，或用对应冻结代恢复任务/全局限时访问"],
    ...
    outputs: ["仅元数据的查找结果与候选名称，只有status=pass、found=false才证明未找到","精确程序注入或浏览器盲填结果，不含密码、银行卡值或访问令牌","本人验证后的本地窗口可显示一个指定账号/密码字段或一整张银行卡记录，明文不进入模型、stdout或JSON","创建、更新、导入、退役、恢复和加密备份各自的真实回读及失败状态","当前共享active/frozen状态、允许的资料范围和到期信息，或需要的精确恢复入口；不返回私人正文","普通隐私操作与独立设备保护分别回读，不把因子成功当成磁盘已解锁或全部账号操作获准"],
    flow: ["网站登录先复用有效会话，或由 browser-control-continuity 选择本地 Chrome 已保存账号；不要因看见登录页就覆盖已填表单或调用密码中心","Password Center 凭据确实需要时才走登记 Browser Bridge 盲填，模型不接收密码且桥本身不提交；该桥不因此获得云端浏览器访问能力","从已安装的C:\\ProgramData\\PCConfig\\AuthorityHost\\tools\\Invoke-SecretBroker.ps1进入；E盘同名入口只是兼容转发，不绕开安装manifest和固定运行时","账号是什么、有没有这项等问题先Lookup，不先List/Discover/Audit；不唯一时只返回最多五个候选名","沿所需reference选择单一动作，复用仍有效的当前身份，不反复预检","Reveal -LocalDisplay需明确字段和一次已登记真人因子；银行卡号、有效期、CVV作为一个加密对象一次显示，不分三次验证","远程会话不能显示普通查看窗口时，仍走Reveal -LocalDisplay并使用-CopyOnly遮盖面板；账号、密码或银行卡字段按明确选择复制到Broker所在电脑的剪贴板，不改走Copy/AgentCopy，不向模型返回明文","程序用AgentSecretRef注入登记目标；浏览器用AgentLogin或精确HTTPS页面的AgentCardFill，Bridge不提交、不向模型回读字段值","精确Set/Update的新值已在当前任务时仅用匿名继承stdin与Runtime能力，不落argv、环境、文件、剪贴板或日志；人工新值用本地界面","变更原子回读、备份与审计；只有Invalid/Revoked/Expired表明需更新，超时、限流或权限不足不证明凭据错误","精确载体恢复由现有RecoverRegisteredCarrier准备请求和新运行绑定，先验证备份再处理所需因子；恢复成功但备份部分失败时仅Backup该carrier_id，不重放恢复","必要保护判断走 ApplyProtectedJudgment：允许、独立锁屏或本人验证由合格主体按具体语境选择，不按动作类别自动隔离","仅独立的设备/入侵保护邀请，才在正式邀请前用StatusVerificationReadiness检查四类软件，PrepareVerificationInvitation后从真实可见邀请起10分钟，显示Passkey/TOTP/Recovery/Account并按正式RecoverDeviceTrust恢复；普通隐私访问不用此入口。","独立入侵保护中的新软件故障以RecordVerificationFault保留实际起止并补足一次；普通隐私窗口另为五分钟，不继承补时，也不借重试重置窗口。","StatusPersonalEnvironment只读当前共享状态；本机Codex缺本条Hook投影时，先CheckPersonalDataAccess -PrivacyLevel none按实际root/人类输入回读，再决定是否需要screen或factor。","普通LockPrivacyScreen与VerifyPersonalEnvironment直接使用真实当前绑定，不先派高档模型；显式LockPersonalEnvironment冻结共享环境，不调用设备恢复或磁盘隔离来代替。","同一根等待复用同一请求；VerifyPersonalEnvironment默认当前任务、可明确选择全局限时访问。期限从PCConfig现行配置读取，不因查询、重试或压缩延长，重启后的旧授权不能自动复活。","四类因子任一已登记方式成功仅恢复其准确冻结代与范围；共有冻结更晚发生时，旧回调不能解冻。自愿OptionalPrivacyWaiver取消不制造新冻结，不能与必需因子取消混用。"],
    boundaries: ["秘密值不进入公开面板","等待验证本身不冻结已有合法访问；必需验证明确取消、拒绝或可见邀请五分钟超时才共享冻结。冻结停止私人取用、复用、披露与受保护动作，但不锁盘、不删数据或重置设备信任，通用工作与精确恢复继续。","管理员权限令牌不扩大授权","device untrusted（设备不可信）时失败关闭","载体 ID 只从 PCConfig 登记读取，恢复不依赖旧 C Policy/CoreGoal，也不覆盖仍可读旧备份","替换设备上的恢复码按备份验证，不要求事先已有本地密码数据库；本网站不执行或接收任何秘密","银行卡盲填只支持一个标准autocomplete字段组和本次精确HTTPS目标，填入成功不证明付款或提交","Microsoft默认密码同时是Windows登录密码，标为never_inject，不自动填入浏览器或系统界面","Google密码管理器是独立来源；正式CSV仅按已绑定账号导入增改，移除记录为pending_removed，不自动删除本地密文或回写Google；成功与E/G回读后处理精确导出清理","BitLocker全部恢复项只在专用本地窗口经一次真人因子显示，不循环Reveal、不复制全部、不向模型返回；生产隔离与只读恢复查看是独立能力","同主体官方更新、Bridge失联或超时本身不是设备不可信；device_untrusted仍阻止普通秘密动作，恢复由独立受保护入口负责","查看与复制是不同结果；普通远控客户端可看和点遮盖面板，复制仅写Broker主机剪贴板，远控剪贴板同步仍需另证，不承诺跨设备Codex/ChatGPT投递","合理身份疑点不要求50%阈值；锁屏与最高权限验证独立，普通Hook超时或组件故障不会自动把设备变为不可信","不同运行框架可由自己的适配器核验真实消息或工具调用判断产物；缺少Codex专用字段不阻断无关工作，也不产生最高权限身份","本机Codex实际取用普通个人参考前锁屏，取用档案性/私密资料前走已登记因子；先保护再读，不先查库决定是否验证。独立获准的电脑MCP沿自己的私密访问授权，不重复本机因子；本机Codex借MCP不属例外，共享冻结对全部入口生效。","普通屏锁、任务/全局隐私因子、秘密明文Reveal和设备保护是不同授权范围；隐私全局访问不免除密码或账号明文所需真人因子。"],
    dependencies: ["PCConfig Password Center（密码中心）", "KDBX 或相应秘密权限来源", "活动保护策略"],
    tests: "保留既有盲填、银行卡、CopyOnly与载体恢复的各日期证据；本次读取E144、规范Skill和authority-and-trust引用，区分五分钟隐私窗口、十分钟独立保护、共享冻结及重启失效。未提示本人因子、锁屏、锁盘或读取真实凭据。",
  ...
  skill({
    ...
    useWhen: ["不确定当前 checkout（本地检出目录）属于哪个仓库","准备恢复、迁移、推送或发布","需要确认仓库是公开还是私有、默认分支是否正确","多个 Git worktree（工作树）和同步状态会影响实施","进入项目维护或准备PUBLIC内容，需要定位已有私有配套文档"],
  ...
  skill({
    ...
    sourceBytes: 6089,
    sourceSha256: "524acb5fc1df38eb5af3638c06750c8ba15f78442f8db0c0a5e76c2e693e058c",
    ...
    outputs: ["0 到 10 的直属代理判断", "模型家族、effort、scope 和 fork 边界", "当前已分类家族或未分类的真实型号/effort组合；非OpenAI同代同谱系Flash与明确允许的OpenAI子会话各按当前目录、根/父边界核对。"],
    flow: ["root 的 UserPromptSubmit 或 child 的 SubagentStart 在 0–10 判断前注入可信身份与当前 E identity","只有旧 root 完全没有 Hook 或注入时，才把同一任务中已有的自然语言 model/effort 确认规范化后写入并回读耐久 thread binding；child 不继承","上下文压缩、E identity改变或加载不确定时完整重读当前经济路由节；不固定条目数量。E换代刷新代际快照，不要求重复确认仍成立的型号选择。","代理自己按任务语义、净收益、冲突和 slots 评估 0–10、模型家族与 effort","真实 spawn 前由 PreToolUse Hook 复核 TOCTOU、家族上限、effort 上限和参数","root 继续负责方向、集成和最终验收，不因派出 child 原地空等","封闭、独立可验的工作优先考虑 Luna Max；状态、依赖和故障因果优先考虑 Terra High；业务语义与冲突取舍优先考虑 Sol High；Astra 用于少量方向判断和最容易返工的关键点。具体型号与强度仍按任务选择，不机械用满根上限","未分类根保留同一精确ID与思考上限；非OpenAI还可选择目录明确证明的同代同谱系Flash，Full可下派Flash但不反向升级。歧义退回已证明的自身，不按价格、版本或菜单位置猜强弱。","用户明确允许跨入OpenAI的型号/档位或范围时，从真实同一根原话建立或复用许可；已注入合法组合直接用，不重复绑定。非OpenAI根实际调用openai_child，而不是给spawn_agent加一个agent_type。","openai_child一次传完整agent_type、model、reasoning_effort、task_name、message，不传fork_turns；创建受理、实际模型启动与任务结果分别回读，后代不得扩权。"],
    boundaries: ["Hook 只验证身份、E identity 与参数，不选择模型家族或代理数量、不创建 child，也不制造授权","耐久 thread binding 只用于完全没有 Hook 或注入的旧 root，只落 canonical ID，child 不继承","E release 换代不撤销已经成立的用户确认；规则明确撤销该路径或身份冲突时才失败关闭","PreToolUse 只是 spawn 前复核，不能代替判断前的可信身份","协作者创建所需Hook或有效绑定缺失时，只关闭委派，普通任务继续；Codex真实新输入缺少个人保护Hook另按共享个人环境规则处理，不能借普通委派边界放行私人读取。","不依赖 Stop Hook；app version（应用版本）和 versioned path（带版本路径）不是准入条件","委派只能收窄 scope、授权、家族和 effort，root 始终保留最终责任","已分类OpenAI组合按活动家族/effort上限比较；未分类只在真实自身或有证据的同代Flash与明确许可范围内选择，不制造第五档或永久价格排名。","同型号后代思考强度不升级；跨型号按组合等级与根、直接父双上限比较。Luna 可至 Max，ultra 按 xhigh 比较上限但保留真实身份和 fork 边界；允许上限不是实际默认档位","task name 以实际模型与启动 effort 开头，只为可读性，不证明身份；槽位满本身不授权创建顶层任务","不能从电脑MCP借用、编造CODEX_THREAD_ID或把模型自报当真实宿主身份；没有可信委派身份时不启动子代理，普通调查与实现继续。","这里的无委派身份不免除本机Codex私人资料访问前的共享状态与隐私Hook要求；不同门控制不同能力。"],
