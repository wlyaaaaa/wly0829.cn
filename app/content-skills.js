...
export const skills = [
  ...
  skill({
    ...
    sourceBytes: 10683,
    sourceSha256: "ce87f29cb7da0e22615897b9864eb913070184de495122105390ef135269f04d",
    sourceState: "当前 Skill 与恢复 helper 已从 .agents PRIVATE main 回读；9f794bb 补充没有专用文件选择器接口时的原生对话框处理，当前页面保留此前配置恢复边界",
    ...
    evidenceBasis: "当前 .agents b4f2bba 中浏览 Skill 与 helper 字节、已安装个人供应和官方 source/Host/setup 合同；原有真实上传恢复只作为历史事实，不把当前注册存在或 status=ready 当执行健康",
  ...
  skill({
    ...
    sourceBytes: 9070,
    sourceSha256: "b6382dddebaae3ba6239f5ba7cc7b3abcf6dd0f41c194e5d535d7df3f4f6e5da",
  ...
  skill({
    ...
    sourceBytes: 6636,
    sourceSha256: "ebc8e567b7c9e2d0ead1078e3b2c21a2306c1c859791b4111d4a1400ecc58014",
    ...
    useWhen: ["批量加密明确文件或目录", "验证加密 bundle 而不落地明文", "解密到明确目标", "中断后继续已授权任务", "在本地查看明确文件包内的一份UTF-8文本", "编辑一个文本成员并另存为新文件包"],
    ...
    tests: "既有6项测试保留为历史；f524c18已发布本地查看/编辑路由与参数、保留原包和结果字段约定。当前网页只核对源码与供应，真实本地窗口和本人因子不在本轮执行。",
    sourcePath: "E:\\.agents\\skills\\authorization-file-broker\\SKILL.md",
    endToEndState: "既有受保护加解密有历史证据；新增查看/编辑的真实本人操作仍由密码中心单独验收",
  }),
  skill({
    slug: "vault-workflow",
    sourceBytes: 6440,
    sourceSha256: "2f9e935cb8ed3e3b90eb13e8b9e2c658482a2f071d1033055e46fe75de9167bf",
    name: "vault-workflow",
    title: "本地文件加密、查看与备份",
    status: "已安装 / 隔离回归已验",
    maturity: "A",
    summary: "把明确文件加密成库、把库导出到指定目录，或只在本地查看和编辑支持的文本；也能验证密码、为VAULT03换密码及把密文装入或取出图片载体。密码和必要密钥文件在本地输入，原件和未选槽位保留。仅检查不解密，私人密文备份另走发布与回读，不与密码中心混用。",
    useWhen: ["不打开原文地检查库结构与环境", "在本机查看或编辑指定库里的支持文本", "核对精确密文的私人备份", "明确要求保护指定远端 README 正文", "将明确文件或目录加密为新.enc，或导出到明确目录", "验证一个库的密码或更换VAULT03当前解锁槽的密码", "将密文装入图片载体或从图片取回密文"],
    ...
    outputs: ["不含原文的元数据、建议与实际效果状态", "本地窗口中的支持文本，或明确的 saved／未保存／取消结果", "普通发布的真实写入和远端密文字节回读；README保护只有单提交API结果，不混同", "保留源文件的新加密库、仅写到明确目录的导出文件、或新的图片载体/取回密文"],
    flow: ["先分清仅检查、本地读写还是远端动作", "检查模式只读结构和环境，不自动执行建议", "本地读写明确目标，本人提供所需凭据；不捕获会打印明文的CLI查看输出", "保存前检查成员、原KDF（密码派生参数）和槽位容量；不能保真则停止", "普通密文发布核对默认分支字节；README保护只有本地候选自检与单提交API结果，后续字节回读另列未实现", "Encrypt不删输入；DecryptExport只写明确目录并保留同名冲突", "ChangePassword只支持VAULT03，保持当前KDF并只重加密本次密码解锁槽；其他槽不变", "VerifyPassword只验证一次并丢弃内存原文，不建立可复用授权会话；下一次需要凭据的操作重新在本地输入", "Hide/Unhide仅封装或取回密文，不解密，不弹密码或最高权限因子"],
  ...
  skill({
    ...
    sourceState: "当前 personal-panel-refresh 规范字节已从 .agents PRIVATE main b4f2bba 发布回读；共享入口与网站均允许 Astra High 或以上完成网页语义工作，Astra Max 只是默认",
    ...
    evidenceSourceCommit: generatedPanelFacts.sourceCommit,
    evidenceBasis: "已发布 Skill b4f2bba、活动 E128、本人 High 及以上更正和本轮精确源码范围；没有用历史 Pages 或旧 active Owner 接受回执证明本候选发布",
    sourcePath: "E:\\.agents\\skills\\personal-panel-refresh\\SKILL.md",
    sourceBytes: 8549,
    sourceSha256: "220e3382a39a0871e999d4541501243122f85018bb3a80bcfbcf8bd16512c224",
    endToEndState: "历史派发/发布路线保留；当前任务实际身份、独立内容验收、Pages 与公网回读各自核对，High 及以上均符合 Astra 网页资格"
  }),
  skill({
