...
const workDeliveryProject = {
  ...
  usageExamples: [
    { ask: "我有立项说明、评审纪要和执行计划，先建一个可持续更新的交付包。", effect: "建立稳定 package，只冻结三份明确来源并交回覆盖、冲突和当前结构。", moduleSlug: "package-sources" },
    { ask: "哪些事实能正式采用？这个金额、日期和表格行列能不能回到原文？", effect: "查看结构化事实、证据定位、审阅理由和质量门，缺 actor 的边界也保持可见。", moduleSlug: "evidence-quality" },
    { ask: "确认后只生成 PRD；评审和执行表下一轮再要。", effect: "从同一当前 build 生成 PRD.md、manifest、traceability 与 DOCX，不启动 PPTX/XLSX。", moduleSlug: "consistent-deliverables" },
    { ask: "执行计划换成新版了，告诉我哪些事实和旧文件真的过期。", effect: "唯一原文自动重新绑定，消失或不唯一的事实与引用变化的旧 build 标记 stale。", moduleSlug: "source-change-next-version" },
    { ask: "现在到底完成到哪一层？失败后能从哪里继续，是否已经证明省时间？", effect: "分开功能、安装、合成 E2E、真实工作、时间基线和数据库恢复状态，并交回最近可靠恢复点。", moduleSlug: "value-state-recovery" }
  ],
  evidenceLayers: [
...
const workDeliveryModules = [
  ...
  commonModuleShape({
    ...
    example: "预算保存为 amount=605000.00、currency=CNY，试运行保存为 date=2026-10-12；执行计划的一行证据定位到 CSV 第几行、第几列。关键冲突没解决时只生成醒目草稿。",
  ...
  commonModuleShape({
    ...
    example: "执行计划更新后，一行原文仍唯一存在，证据自动指向 version 2；另一行被删除，对应事实 stale，引用它的旧 build 也 stale，其他事实保持 fresh。",
