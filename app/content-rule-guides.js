    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          item("最小充分架构只禁止无依据方案", "先问“用户是否要求这个结果”，再比较同一完整功能和真实质量下的实现总成本。现有入口足够就用，有当前具体未满足项才补最小增量；不停止用户目标，不限制自主修复、重建和委派。"),
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          item("新增实现只补当前真实缺口", "新增模块、服务、任务、数据库、状态机、Owner 或验证层须对应具体未满足项；未来猜想、惯例、最佳实践、额度或审查偏好不能举证。无依据只禁止该增量，回到简单路径继续，不把反膨胀变成整个用户目标的停工门。"),
