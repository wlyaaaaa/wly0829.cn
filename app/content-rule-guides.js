    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          item("4. 每个父代理 0 到 10", "0 合法但不能惯性。Luna Max 优先封闭、读重和确定性工作；Terra Max 适合强耦合实现、深调试、架构和强审查；Sol Max 适合复杂实现、战略与终审；Astra gpt-6-astra 面向关键路径最高难度、最易返工或最快正确收敛，须判断质量/返工/墙钟收益是否覆盖费用。比较总交付成本，低 effort 不自动更便宜，也不固定一家通用默认。"),
          item("5. 家族和 effort 只能向下", "四档依次为 gpt-5.6-luna、gpt-5.6-terra、gpt-5.6-sol、gpt-6-astra；每档只能派自己或已知更低档。其它 verified 型号是 unclassified（未分类），仍可派相同精确 ID 的后代，不能猜为更强或转派便宜型号。所有档位取宿主实际可用集，Luna/Terra Max 是常用质量基线而非硬下限；后代型号与 effort 同受根/直接父上限，effort 绝对不超过 ultra。每次真实创建仍核验低档与未分类型号。"),
