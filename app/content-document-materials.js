...
const documentMaterialsProject = {
  ...
  summary: "当我已经知道要做什么、交给谁，也有了必要原件，就用这个项目把它们整理成合同、说明、申请、通知、回复、售后材料或附件包；只改单个 Word/PDF 时直接绕过，只找原件时进入个人材料查找。它从当前事项和最小原件开始，生成同源、按用途中文命名的 DOCX 与 PDF，按屏幕或打印用途安排版式，核对内容、附件、签名资产、页码和版本，并提供逐页渲染结果。非打印PDF用真正帮助阅读、定位的彩色层级，不以省墨黑白版代替；打印版才另验灰度。最终签名是否清楚、位置是否正确仍须看实际页面，不能只看图片数量；已生成、本人已签、可递送、已递送、已收件、已处理与对方签回分别记录。",
  ...
  productPrinciples: [
    ...
    { title: "电子版和打印版来自同一内容", detail: "DOCX、PDF与附件包共享同一确认内容，但电子与打印版可以采用不同排版配色，不要求每次都做两版。非打印PDF必须有帮助阅读与定位的彩色层级、文字标签和足够对比度，不只是给标题上色；打印版才采用纸面与灰度验收。原件和已签署、递送历史不重着色。" },
