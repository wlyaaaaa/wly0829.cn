...
const sourceMainCommit = "4f149223eb5de2be56e9d800929365b8a96b7a23";
const observedAt = "2026-09-14T03:18:00Z";
const wrapperCommand = "pwsh -NoProfile -File .\\daily-preferences.ps1";

const dailyPreferencesSnapshot = createProjectSnapshot({
  observedAt,
  label: "共用本人理解、按提示词更新微信与新导出、受影响认识复核；手机和云端经电脑 MCP 使用同一原库，107个来源实例保留9月9日观察，当前新增完整卡读取与同字节定位修复",
  boundary: "本人已确认电脑 MCP 接通，手机和云端使用原库现有入口；两份 Google Drive 文件保留为历史快照，停止日常维护。命令执行、AI 实际阅读与理解是否有用分别判断；来源和媒体缺口仍保留。本轮仅核对4f14922公开安全源码与合同，未读私人记录、重采库存、操作手机或重跑历史测试。107来源实例是9月9日观察，48认知卡及数据总量是9月8日观察，不随源码日期更新。",
  metrics: [
  ...
  facts: [
    ...
    { label: "当前源码观察", value: `2026-09-14本地干净HEAD与新读远端main均为${sourceMainCommit}；profile --key --reader完整保留详卡并去重展开引用，repair-artifact-locators按大小/SHA-256核对同字节迁移，预检后显式事务修复定位，不重新导入或改写认识。新代码观察不继承旧库存、旧测试或真人效果验收。`, hero: false },
    { label: "9月9日历史源码验收", value: "2026-09-09 回读 PRIVATE main=1aae7f7，工作树干净，解析版本仍为 daily-preferences.v0.12.0、schema 6。微信发送者筛选、按账号/会话读取位置、实际阅读范围和引用缺口说明已经进入 main；当前合同包括电脑 MCP 同库使用与操作者可信核对的按需只读背景；后者不触发增量或回写。该次18项微信增量与6项导出合成测试通过；185 项 Python、14 个子测试与 7 项 Node 仍是 2026-09-07 基线，不冒充本次完整回归或实际手机使用。", hero: false },
...
const dailyPreferencesProject = {
  ...
  evidenceLayers: [
    ...
    { layer: "PRIVATE 源码 main", proves: `当前main=${sourceMainCommit}已从远端回读；原有本人理解、认知卡、可靠回写、原文候选、已核引用、退款与B站集合语义保留，新增reader与同字节原件定位修复有当前代码/合同证据；没有重跑旧测试。`, doesNotProve: "源码和测试不证明私人材料已充分阅读、理解已覆盖全部经历或每个领域均完成实际采用。" },
...
const dailyPreferencesModules = [
  ...
  commonModuleShape({
    ...
    implementation: [
      ...
      "云端按需读取当前 Skill、AGENTS 与 DOMAIN_CONTRACT，不维护第二套手机规则、画像、数据库或专用接口。命令优先实际执行，缺少某个 Codex 专属工具不使普通命令整体不可用。",
