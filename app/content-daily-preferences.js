...
const dailyPreferencesSnapshot = createProjectSnapshot({
  observedAt,
  label: "共用本人理解、按提示词更新微信与新导出、受影响认识复核；手机和云端经电脑 MCP 使用同一原库，107 个来源实例已只读核对",
  boundary: "本人已确认电脑 MCP 接通，手机和云端使用原库现有入口；两份 Google Drive 文件保留为历史快照，停止日常维护。命令执行、AI 实际阅读与理解是否有用分别判断；来源和媒体缺口仍保留。本页未另行操作手机，各项证据按实际观察时间展示。",
  metrics: [
    { label: "按需更新", value: "微信 · 新导出" },
    { label: "认识读取", value: "概括 · 详述 · 原文" },
    { label: "来源实例", value: "107 个" },
  ...
  facts: [
    ...
    { label: "当前数据状态", value: "2026-09-08 08:16:40 UTC 只读聚合：190,266 条 current 记录、232,496 条总版本、42,230 条历史版本；schema 6、quick_check=ok、外键问题 0。记录、明示与认知卡的最后更新时间分别为 9/7 06:05:24、9/8 04:47:34、9/8 07:38:38 UTC，均早于本批截止；未展开记录正文，不把数据量当作已理解的材料量。" },
    { label: "当前来源", value: "2026-09-09 03:40:09 UTC 回读 107 个来源实例：12 个 acquired_verified（来源已取得并核对）、95 个 snapshot_only（只有快照），涵盖 16 类逻辑来源。微信原生消息、ASR 派生和附件各保留出处；来源实例数量不等于独立经历、完整阅读或新事实。" },
...
const dailyPreferencesProject = {
  ...
  evidenceLayers: [
    ...
    { layer: "电脑 MCP 与按需导出", proves: "3bd9c8e 的当前合同记录本人确认接通电脑 MCP，并改为同库读取、回写和按需增量；旧 Drive 文件停止日常维护。本轮 6 项虚构导出测试通过，完整文件导出能力仍保留。", doesNotProve: "用户确认接通与源码合同不等于本页执行了新的手机会话；合成测试不证明每次连接、理解质量或未来平台状态。历史文件与新导出都不是全库恢复包。" },
