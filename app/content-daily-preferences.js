...
const sourceMainCommit = "4d2503983e1610574f6486af120ea6edbf352815";
const observedAt = "2026-09-18T18:58:17Z";
const wrapperCommand = "pwsh -NoProfile -File .\\daily-preferences.ps1";

const dailyPreferencesSnapshot = createProjectSnapshot({
  observedAt,
  label: "共同理解要能直接用于帮助；先保护资料、合理推定与及时回写各有边界",
  boundary: "当前4d25039已发布源码与E144明确先保护再取用、理解库不用于身份判断，以及领域新认识的阶段回写。未读取私人记录、重采库存、操作手机或重跑历史测试；107来源实例仍是9月9日、48认知卡及数据总量仍是9月8日观察，不随规则更新变新。",
  metrics: [
    { label: "按需更新", value: "微信 · 新导出" },
    { label: "认识读取", value: "概括 · 详述 · 原文" },
    { label: "来源实例（9月9日）", value: "107 个" },
  ...
  facts: [
    ...
    { label: "来源实例（9月9日观察）", value: "2026-09-09 03:40:09 UTC 回读 107 个来源实例：12 个 acquired_verified（来源已取得并核对）、95 个 snapshot_only（只有快照），涵盖 16 类逻辑来源。微信原生消息、ASR 派生和附件各保留出处；来源实例数量不等于独立经历、完整阅读或新事实。" },
    ...
    { label: "当前源码观察", value: "2026-09-18只读回读main=4d2503983e1610574f6486af120ea6edbf352815。profile --key --reader完整读取、按大小/SHA-256核对的原件定位修复仍保留；业务入口显式--privacy-level none|screen|factor，先由PCConfig核对真实访问范围，再返回数据。源码新鲜度不继承旧库存、旧测试或真人效果。", hero: false },
    { label: "9月9日历史源码验收", value: "2026-09-09的PRIVATE main=1aae7f7、daily-preferences.v0.12.0/schema6记录了微信发送者筛选、按账号/会话阅读位置、范围和引用缺口；当日18项微信增量与6项导出合成测试通过。185项Python、14个子测试、7项Node仍为9月7日基线，不冒充当前完整回归或实际手机使用。旧身份辅助用途已由当前Owner与E144明确取消。", hero: false },
...
const dailyPreferencesProject = {
  ...
  repositoryUrl: null,
  statusTone: "mixed",
  cardStatus: "共用本人背景、按提示词吸收微信与新导出、修正认识；手机与云端通过电脑 MCP 使用原库，107 个来源实例",
  ...
  repositoryNote: `PRIVATE（私有）仓库与稳定标识继续使用 daily-preferences，当前 main=${sourceMainCommit}；v0.12.0 / schema 6 在原有来源、退款、平台集合与按需增量上增加本人背景、认知详卡、原文候选、已核引用与可靠领域回写。只有一份当前共同本人理解，独立领域继续拥有专业状态。本轮只读代码与产品合同；聚合保留原观察，不复制私人原话、账号、财务组合、行程或健康个体值。`,
  ...
  result: "我得到能直接用于当前问题的理解与建议：哪些经历和取舍相关、依据是什么、哪里是模型最倾向的判断、什么反例会改变结论。需要深入时可以回到完整详卡和原文；形成新认识后，摘要、详述、别名与证据一起更新并回读。手机或云端 AI 通过已接通的电脑 MCP 使用同一原库，当前更正沿相同入口保存，无需搬运两份背景文件；确实需要独立材料时再完整导出。明确要求更新来源时，仍逐来源交回真实变化与缺口；普通推荐保留熟悉、相邻和新鲜候选，最后由我决定。",
  readerStates: {
  ...
  methodCanvas: {
    ...
    steps: [
      ...
      { actor: "动态材料", title: "从微信实际读到的地方接着理解", detail: "按请求检查账号和会话变化，取齐选定消息并读必要上下文；将新认识和反例纳入原详述，修订后保存实际阅读范围。新导出也沿相同的阅读与复核原则处理。" },
  ...
  productPrinciples: [
    {"title":"先保护资料，再用于理解，不拿背景辨人","detail":"普通个性化参考先满足本机锁屏，档案性或私密资料先通过已登记因子；不能先读背景或样本来决定是否验证。独立已授权电脑MCP沿自己的私密访问授权，不重复本机因子；本机Codex借MCP不属例外。共享冻结后所有入口都停止取用、复用和披露私人内容，普通工作与最小恢复继续。 本库不再参与操作者身份判断；待验证或冻结期间不把当前新说法写入本人基准，正常恢复后继续按来源和语境处理。"},
    { title: "行动与认识分开判断", detail: "当前明确决定约束行动；喜好、外部事实和稳定价值判断分别比较语境、长期取舍与反例，不把最新自述或行为设为全部认识的最高证据。current/historical 是版本状态，不等于真假。" },
    {"title":"形成能用的理解，也允许新证据纠正","detail":"来源、原话、时间和记录身份必须准确；日常理解可以综合生活过程、已过时间和后续行为提出最倾向、可修正的判断，不因一份记录未独立证明结果就永远悬置。推断与直接记载分开，无反证不能证明任意事情；专业领域仍按具体用途核验。"},
    { title: "不把过去倒填成现在，也不编造转折", detail: "表达时间、事情发生的时期、当时选择与后来反省分别看；声称“开始”或“不再”需要前后依据。模型误读引起的反复纠错不是本人执着，岗位职责也不能自动解释成私人善意。角色、时期或反例会改变含义时，概括和详述都保留。 摘要保留会改变当前帮助的理由、冲突和长期主题，不能让最近一次纠错抹掉仍然成立的其他认识。" },
    ...
    { title: "接入不是偏好结论", detail: "材料保全和 ingest 只是中间步骤；还要判断偏好语义、重建相关快照并回验普通问题，或明确证明语义无变化。 领域产生新共同认识时，由产生结果的当前任务在阶段交付前合并回写并回读；不依赖本人抽查或父库未来扫描。" },
    { title: "解析器清单不是来源全景", detail: "每个已承接偏好领域都要检查真实行为源或明确缺口；退役 PersonalOS 只作一次性发现线索，发现后的来源必须转成独立现行入口或缺口。" },
    { title: "Steam 的选择与游玩是不同强度的依据", detail: "已确认账号按 AppID 合成同一个人的库；玩过可以增强行为依据，已分类但未玩或仅安装仍保留较弱的选择意义。不能用没玩反推不喜欢；本人已明确非免费游戏由自己购买，但具体历史取得方式仍不能从当前商店分类猜出。" },
    { title: "手机与电脑共用原库，文件只在需要时导出", detail: "手机或云端通过电脑 MCP 使用现有读取、回写与增量入口，不维护另一份当前档案。普通命令先实际执行，真实失败再准确说明并接续；离线材料或转交需求才完整导出，需要云文件才另做实际更新和回读。" },
  ...
  responsibilities: [
    ...
    "维护已核对的 107 个来源实例、16 类逻辑来源与 5 项尚未取得来源；明确拒绝的两类退出待办，来源数量不冒充阅读或理解质量",
    "按提示词检查微信动态材料和新导出，保留账号、发送者、分页、实际阅读范围与未读媒体边界，并复核受影响的旧认识",
  ...
  exclusions: [
    ...
    "不把 Steam 库内选择冒充实际游玩，不把应用或类型未知项当成游戏，也不从当前商店分类猜历史取得方式",
  ...
  operatingFlow: [
    ...
    { title: "按问题补证", detail: "吃喝、购物、支付、出行、Steam 游戏与哔哩哔哩按专门轴；Steam 区分库内选择、实际游玩和应用，B 站问题只取 B 站证据，其余扩展域使用明示、通用证据或明确缺口。" },
  ...
  evidenceLayers: [
    {"layer":"资料保护与理解用途","proves":"当前PRIVATE main 4d25039与E144明确：业务取数前按实际任务保护，理解库不再提供操作者判断辅助。当前合同还要求合理、可修正的理解，以及领域在阶段交付前完成可靠回写。","doesNotProve":"规则与源码回读不证明本轮实际读取私人档案、完成真人因子、形成正确认识或让其他领域真正采用。"},
    { layer: "PRIVATE 源码 main", proves: "2026-09-18T18:58:17Z正式回读PRIVATE main 4d2503983e1610574f6486af120ea6edbf352815；现有完整卡读取、可靠投递、同字节原件定位和退款/集合语义保留，新资料访问与理解/领域回写合同已读。没有重跑历史测试、刷新私人数据或修复来源程序。", doesNotProve: "源码和测试不证明私人材料已充分阅读、理解已覆盖全部经历或每个领域均完成实际采用。" },
  ...
  evolution: [
    ...
    { date: "2026-09-05—09-09", commit: "7e5fe1b—3bd9c8e", result: "v0.12.0 在原有偏好证据上形成共用本人理解、完整详卡、已核引文与可靠领域回写；认识按时间、关系角色、反例和真实反馈修正。手机使用由两份云端阅读文件转为电脑 MCP 直接共用原库，旧文件停止日常维护，独立导出按需保留；局部取用与修正证据不扩大为全量理解质量证明。" }
  ],
...
const dailyPreferencesModules = [
  commonModuleShape({
    ...
    status: "v0.12.0 / schema 6 已实现索引、详卡、角色引用和可靠回写；9月8日的48张认知卡只是旧状态聚合，本轮未读取私人卡片或重采数量",
    statusTone: "mixed",
    ...
    result: "得到可直接交流的认识及其适用范围，必要时可展开完整经历、推理、反例、原话和来源位置；长卡可以完整阅读而不重复展开同一引用。原件仅换位置时，按同字节证据修复定位，让已有认识继续回到原件；内容改变则复核引用。相关领域真正接收并使用后的结果另行回读。",
    readerStates: {
  ...
  commonModuleShape({
    ...
    teaser: "直接说“结合最近微信和新导出，更新一下对我的理解”：先取本轮需要的新材料，再阅读、补充或纠正原有认识，最后保存实际读到哪里。可直接取得的来源在当前对话处理，需要新数据包的来源仍由本人提供。",
    status: "107 个来源实例：12 个已取得并核对，95 个保留快照；微信按需更新并记录实际阅读位置，人工来源按所提供材料更新",
    statusTone: "mixed",
    ...
    example: "我可以说：“结合最近微信和这次新导出，更新一下对我的理解。”AI 先看指定账号哪些会话有新内容，从实际阅读位置接续；阅读必要原话、引用和上下文，说明哪些认识补充了、需要改或仍然不确定。若还要求刷新偏好行为，再处理 Chrome、Steam 和 B 站；没有新包的 Google Play、航空和 12306 维持原快照。",
    ...
    decisionImpact: [
      ...
      "原观察5项未取得来源是 bank_transactions、jd_orders、pinduoduo_orders、meituan_orders、cainiao_logistics；youtube_activity / maps_activity 已明确排除，来源缺材料与用户拒绝分开。",
    ...
    sources: [
      ...
      { path: "tools/wechat_update.py", role: "串起微信变化发现、固定范围、分页取得和实际阅读完成，保留发送者、引用缺口和阅读位置。" },
      { path: "tests/test_wechat_update.py", role: "验证范围、分页、来源身份和阅读位置；测试源码与真实消息阅读结果分开。" },
  ...
  commonModuleShape({
    ...
    searchProjection: {
      intents: ["按自然问题查偏好证据", "只取B站相关证据", "分开吃和喝", "区分Steam已玩与未玩库内选择", "过滤无关AI对话", "召回较老中文记录"],
      ...
      failureRecovery: ["快照缺失继续证据查询", "B站问题不混入Steam或全库噪声", "未玩游戏保留较弱依据，应用不当游戏", "历史取得方式保持Unknown", "无匹配保留Unknown", "只读连接不修改数据库"]
    },
    teaser: "普通推荐不需要把 232,496 条记录版本塞进上下文；先用与问题相关的现行理解，必要时再取明示、行为事实和原文候选，避免噪声挤掉重要经历。",
    status: "查询先按问题缩小候选范围，再校验相关认识；08:16 聚合为 190,266 条 current 记录、48 张有效认知卡，旧专项自然回读保留其观察日期",
    statusTone: "pass",
    ...
    result: "得到与问题相关的明示、有效认识、证据候选和出处；返回数量及是否还有未展示内容一并说明，AI 再判断事实与推测。少量命中不会被当成已经读过所有材料。",
    readerStates: {
    ...
    verification: ["构造 1,001 条较新占位与第 1,002 条旧中文目标，确认旧目标仍返回。", "合成同类近期与六年前成功记录，确认近期更靠前、旧记录仍返回且 repeat_count=1。", "退款、关闭与 refunded_full 链在 facts 保留交易事实，在 evidence 和快照支持中退出；partial/Unknown 不传播。", "Steam 合成回归证明启动时长不加分、current_free 只低 0.5 分，已分类未玩游戏保留且较弱；游戏查询排除 application。", "2026-09-04 Steam 聚合为 118 已玩游戏、6 已玩应用、8 未玩游戏和 33 其他应用；本批未重新采集。", "当前 B 站自然问题只返回该平台记录与明示，不再引用任何已失效推定；AI/开发与出行问题也分别命中正确域。"],
  ...
  commonModuleShape({
    ...
    example: "我可以说：“先看看我库里有哪些没玩的游戏，再给熟悉、相邻和新鲜三类选择。”系统分开实际游玩和较弱的库内选择，按题取证，再说明玩法或节奏的联系；没玩不等于不喜欢，新的推荐仍标出推测，累计启动时长不参与评分。",
    ...
    boundaries: ["不冒充 Python 推荐算法。", "Steam 已分类未玩/仅安装是较弱选择证据，不是游玩或喜欢证明；应用不当游戏，当前商店分类不证明历史取得方式。", "非成功和 refunded_full 不进入普通偏好、模型上下文或复购计数；partial/Unknown 不猜。", "认知是否继续成立由 AI 复核相关来源与反例；stale 不是本人改变偏好的结论。", "久远、低频、长期未买或无记录都不自动等于过期/不喜欢。", "不承诺实时最低价。", "不替用户下单付款。", "不把历史变成白名单。", "不为扩展域虚构专门来源能力。", "用户未要求单选时保留选择权。"],
    failures: [
    ...
    verification: ["全新自然更新对话没有点名 Skill、命令或路径，能够自行刷新三类自动来源并完成语义闭环；推荐结构仍由 Skill/AI 判断。", "2026-09-04 Steam 事实层为 118 已玩游戏、6 已玩应用、8 未玩游戏与 33 其他应用；当前实现允许已分类未玩游戏作较弱依据，不能用旧专项观察收窄能力。", "回归证明极长启动时长不增加偏好分，当前免费只在其他条件相同时低 0.5 分。", "核对每个新鲜项都有邻接依据和推测标签。", "核对搜索接力只给相关平台与可复制词，不冒充最低价或执行付款。"],
  ...
  commonModuleShape({
    ...
    why: "只靠定期导出的手机文件，更正之后还要重复生成、上传和核对，旧副本也容易被当作现状。电脑 MCP 已接通后，复用原库入口即可让读取与回写共用同一版本；真正需要离线文件时仍保留完整导出。",
    ...
    readerStates: {
      pass: "原库命令完成所需读取，AI 根据实际材料回答；有依据的新内容写回后回读版本和结果。普通调用不先导出，不要求每次读完全部背景。",
      problem: "发现旧理解、人物关系或时期语境有误时，修正拥有该认识的源卡并回读；版本冲突先核对现状再合并，不在手机端维护第二份当前结论。",
      unavailable: "先实际执行所需命令；真实连接或命令失败时保留具体步骤、必要报错与已完成内容，在既有授权内修复。确需电脑端接续时，用户带着原目标和报错反馈，从现有结果继续，不重做已完成工作。"
    },
    decisionImpact: [
    ...
    implementation: [
      ...
      "云端按需读取当前 Skill、AGENTS 与 DOMAIN_CONTRACT，不维护第二套手机规则、画像、数据库或专用接口。命令优先实际执行，缺少某个 Codex 专属工具不使普通命令整体不可用。 独立已授权MCP沿其既有私密访问，不重复本机Codex因子，但共享冻结仍阻断取用和旧上下文复用；本机Codex借MCP不能豁免本机保护。",
