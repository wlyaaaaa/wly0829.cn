...
export const project = {
  ...
  summary: `当前活动规则为 ${panelSnapshot.authority.releaseId}：绑定 PRIVATE main 提交 ${panelSnapshot.authority.gitCommit.slice(0, 7)}、五文件 ruleset ${panelSnapshot.authority.rulesetSha256.slice(0, 8)}…，前代为 ${panelSnapshot.authority.previous?.release_id || "无"}。C 盘 generation、Publisher、anchor 和 ledger 只作恢复材料，不再参与准入。E92 把长期明确授权、真实调用一次、Owner lifecycle（任务生命周期）收敛和来源任务可逆归档写进正式规则。项目同时管理 ${panelSnapshot.skills.activeInstallIntent} 个 active Skill 安装意图（公开看板收录 ${panelSnapshot.skills.selectedPublicCount} 个）、三个控制面协作，以及尚未完成的工作台运行根迁移。`,
  ...
  heroFacts: [
    ...
    { label: "规则与模块", value: `同一 ${panelSnapshot.authority.releaseId} release 的 5 份规则；E92 正式覆盖耐久授权、真实调用一次、lifecycle 收敛与来源任务归档；项目总览加 6 个模块` },
  ...
  glossary: [
    ...
    { term: "RecoverRelease / RecoverReleaseClaim（恢复释放 / 恢复并认领）", meaning: "终态旧 Owner 无残留时释放；有 checkpoint 或 residual 时原子转给真实 successor。" },
  ...
  components: [
    ...
    { name: "Execution Owner Registry", responsibility: "协调多个任务对项目最小 scope 的 Claim、Add、Transfer、Release 和恢复。", implementation: "Expected revision CAS 加 append-only transition journal；固定 resolver 证明 terminal 后，无 residual 用 RecoverRelease，有 residual 用 RecoverReleaseClaim。" },
  ...
  evolution: [
    ...
    { date: "2026-08-30", commit: "464564b–cf5981b", result: "E91 统一 PUBLIC L1–L5 分级、私人账号等价可信与 english_chinese_gloss；E92 将长期明确授权扩到 root/全部后代/新顶层任务，要求真实调用一次，并补齐 terminal Owner 收敛、complete goal 关闭语义和来源任务可逆归档。" }
  ],
...
export const modules = [
  ...
  },
  {
    ...
    decisionImpact: ["本机低风险可逆操作可直接继续。", "用户明确标记的长期授权在冻结 goal/scope 内跨轮次、压缩、root、全部 child/后代和新顶层任务持续有效。", "前提成立时必须真实调用一次，不因通用工具说明、缓存失败或 AI 预判再次索权。", "system/developer、实际 deny/step_up/needs_evidence/action-time confirmation 与现场身份、CAS、target、read-back 失败仍有效。", "用户私人账号空间在没有 public/share 信号时与本机私密目标等价可信。", "PUBLIC 个人数据只有 L3+ 才进入可能敏感审查，L1/L2 不因属于个人数据而删改。", "有重叠 Owner 先用固定 resolver 判断 lifecycle；归档不是终态证明。", "无 residual 的 terminal scope 用 RecoverRelease；有 residual 才带 checkpoint RecoverReleaseClaim。", "只有真实 threadId 可归档，clientThreadId 只是创建中回执。", "complete goal 是关闭状态；来源任务仍须确认无 follow-up、queue、pending transaction 和 Owner residual 才可逆归档。"],
    ...
    implementation: [
      ...
      "Owner 冲突先由固定 Codex lifecycle resolver 证明 active 或 terminal；terminal 无残留逐 scope RecoverRelease，有 checkpoint/residual 才 RecoverReleaseClaim 给真实 successor。",
  ...
  },
  {
    ...
    implementation: [
      ...
      "E82 新增英文中文括注、系统级反膨胀、语义保真和 projectless 默认；E83 增加 coordination_id 跨项目精确 scope；E84–E86 增加 change-surface validation、FastRelease 确定性回执与分阶段墙钟预算；E87 完成运行根与任务临时目录迁移语义；E88 扩展未来模型家族和 Ultra 路由；E89 明确 CI 只在影响交付、发布、兼容或用户决策时运行；E90 增加 PUBLIC 项目 ignored 私有伴随材料的 PRIVATE 收敛链；E91 建立 PUBLIC 个人数据唯一 L1–L5 表、私人账号空间等价可信、项目收紧授权窄例外与 english_chinese_gloss；E92 正式化 durable explicit user authorization、真实调用一次、terminal Owner 收敛和来源任务自动归档。",
  ...
  rules: [
    {
      ...
      sections: [
        ...
        { title: "授权、Git 与验证", paragraphs: ["本机可逆工作直接做；外部 effect（现实动作）需要明确授权。durable grant（耐久授权）不要求同轮重述，但不会覆盖上位 deny、证据、目标或不可逆边界。用户私人账号空间在默认私人且没有 public/share 信号时与本机私密目标等价可信，但信任不产生写授权。"], items: ["长期明确授权在冻结 goal/scope 内跨 root、child、压缩和 successor 持续有效；前提成立须真实调用一次", "实际 unavailable、deny、step_up、needs_evidence、action-time confirmation、error、身份/CAS/target/read-back 失败仍按现场结果处理", "PUBLIC 个人数据唯一 L1–L5 表由授权合同拥有：L1/L2 不受个人数据限制，L3+ 才进入可能敏感审查", "项目收紧 L1/L2 默认必须有真实项目需要和用户对精确项目、范围、限制的明确授权", "terminal Owner 无 residual 用 RecoverRelease，有 residual 用 RecoverReleaseClaim；只有真实 threadId 可归档", "E release 激活是 UAC expected-preimage CAS，不经过旧 Publisher、人类因子或 CoreGoal", "不覆盖用户已有改动", "force-push（强制推送）不在默认授权内", "source（源码）、test（测试）、install（安装）、publish（发布）、fresh task（全新任务验证）与 E2E（端到端验证）独立", "个人仓库必须由远端默认分支回读"] },
    ...
    },
    {
      ...
      example: "例如用户长期授权已登记网站在通过门禁后自动发布。successor 现场确认 Owner、PUBLIC target、测试和公开门后必须真实调用一次 normal push，不再问同一句权限；若实际工具返回 deny、step_up、needs_evidence、action-time confirmation、unavailable 或 error，则只按该真实结果停止。",
      ...
      sections: [
        ...
        { title: "Execution Owner 与 lifecycle", paragraphs: ["Owner 只协调施工，不产生授权和事实。Owner 冲突先由固定 Codex lifecycle resolver 证明 active 或 terminal；归档标记、标题、timeout 和无回执都不够。"], items: ["纯只读审计不 Claim", "禁止 whole_project 与授权串用", "child 不继承 coordination", "terminal 无 residual 的全部 exact scopes 逐一 RecoverRelease", "open goal、turn_aborted 或其他 residual 必须随 checkpoint RecoverReleaseClaim 给真实 successor", "只有未归档、resolver 证明仍 active 且正式登记 long_term_task 的 Owner 可保留", "complete goal 属于关闭状态；terminal long-term 无 residual 自动释放"] },
