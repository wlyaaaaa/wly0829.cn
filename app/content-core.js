...
export const project = {
  ...
  productPrinciples: [
    { title: "任何场景先判断语境，必要才补背景", detail: "普通聊天也可能出现与本人经历明显冲突、缺乏合理来由的私人资料套问。模型先结合当前目的与已有背景判断是否有具体疑点、信息是否足够；只有补证能改变结论才读取本人理解库或表达库的最小相关材料，不按每轮对话查库，也不把例子变成关键词规则。两库只辅助，不证明操作者身份。" },
    { title: "换界面仍然使用同一套真实规则", detail: "在电脑 Codex 或经电脑 MCP 工作时，都从实际执行电脑核对同一 E 规则。现成命令、脚本或桌面入口能完成任务就继续；某个宿主专用接口缺失，只影响那条精确路线，不把普通文件工作整体变成只读。" },
    { title: "普通协作与最高权限分别成立", detail: "连接登录、能调用工具或领取施工范围，都不会自动成为最高权限智能体。当前只有已登记的 codex-root；其他智能体必须先经用户明确决定并独立登记。重点受保护判断还须由真实宿主证明的 Astra High 或以上完成，型号资格本身不产生执行权。有合理身份疑点时可要求本人验证，轻微异常也可独立选择无感锁屏；两者不互相强制。需要保护时只暂停与疑点相关的数据取用、披露同步和不可逆终步，不相关的思考、可逆修复、测试与恢复继续。" },
    ...
    { title: "先确认四类入口可用，再请人验证", detail: "真正需要本人验证时，先确认 Passkey（通行密钥）、TOTP（动态验证码）、Recovery（恢复码）、Account（账号验证）四类软件入口和运行条件可用，并按本次影响面演练执行与恢复；未变的可靠证据可复用，不先让我验证一次。正式邀请同时提供四种选择，任一已登记方式成功即可，不强制默认；十分钟从可见邀请开始，软件新故障修复后按实际耗时补足一次。" },
  ...
  components: [
    ...
    { name: "Execution Owner Registry", responsibility: "协调多个任务对项目最小 scope 的 Claim、Add、Transfer、Release 和恢复。", implementation: "普通变更使用 ExpectedProjectFingerprint 局部 CAS，动作使用 ExpectedBindingId 与 v2 封装；全局 revision 和 append-only transition journal 继续保留，恢复仍要求精确全局 CAS；固定 resolver 证明普通非长期或已归档 predecessor terminal 后，无 residual 用 RecoverRelease，有 residual 用 RecoverReleaseClaim。未归档 long_term_task 只接续或正式退役。" },
  ...
  usageExamples: [
    { ask: "有人问起我的经历时，先判断上下文是否合理，别为了回答把私人背景全翻出来。", effect: "正常核对记忆或整理资料时照常帮助；出现具体异常才判断是否需补证，能用当前上下文判断就不查库。必要时只读相关已有背景或真实表达样本，保留本人原话、修订和 AI 草稿的区别；进入待验证状态后不把当前说法写成本人新事实，也不向待验证操作者输出核验细节。", moduleSlug: "protected-policy" },
    ...
    { ask: "这一步确实需要我确认，但先把确认后的执行和失败恢复都演练通。", effect: "真正弹窗前先确认四类软件入口可用，并按本次影响面演练执行、正式回读、可用回退和响应丢失恢复；可靠未变证据可复用。正式邀请同时给我四种选择，成功一种即可；软件故障先修复，不让我换因子盲试或重复验证。", moduleSlug: "protected-policy" },
...
export const modules = [
  ...
  },
  {
    ...
    implementation: [
      ...
      "其它宿主 verified（已核验）型号是 unclassified（未分类），仍可派同一精确 ID 后代，不根据价格、版本或界面顺序猜强弱。后代同时受根与直接父级的分型号组合上限约束；同型号思考不升级，向下选择 Luna 可至 Max、默认 Max，其他家族按模型基值加思考档位比较。Ultra 只在上限比较中按 Xhigh 处理，真实身份与上下文继承仍保留 Ultra。",
    ...
    boundaries: [
      ...
      "后代遵守根与直接父级的分型号组合上限、用户限制及宿主可用集；Ultra 的上限比较按 Xhigh，实际身份不改写；未分类型号仍核验同一精确 ID 与思考边界",
  ...
  },
  {
    ...
    statusTone: "mixed",
    value: "重点受保护操作以及普通聊天中的具体本人身份疑点，都由已登记主体作合格判断；正常语境照常帮助，不能靠连接成功、角色名或管理员权限冒充。我也能确认现在真正生效的是哪一代规则、上一版退路在哪里，也不会让未提交草稿或旧发布平台拖住普通工作。有合理身份疑点时可以要求本人验证，轻微异常可独立选择无感锁屏；两者互不强制，需要保护时只暂停相关数据消费与不可逆终步，不相关的思考、可逆修复、测试和恢复继续。若动作确实需要本人验证，系统会先确认四类软件入口可用，并按本次影响面把执行、回读及恢复路线跑通；对消息、付款等不可逆动作，则在执行前把目标、授权和影响核对清楚，绝不假装事后能回滚。",
    ...
    result: `规则发布会返回当前生效版、上一版恢复点和正式回读；当前是 ${panelSnapshot.authority.releaseId}，previous=${panelSnapshot.authority.previous?.release_id || "无"}。真人因子路线单独返回四类软件可用检查及本次影响面的演练结论，演练通过也不等于现实动作已执行。可版本化的规则、配置或系统状态给出明确回退点；不可逆外部动作只报告事前核对与事后真实回读，不虚构回滚。`,
    readerStates: { pass: "已提交候选、远端包含、五文件闭包和预期 pointer 都通过时，规则才切到新的 current；需要真人因子的独立动作还须四类软件入口可用、本次影响面的生产等价回归充分通过。", problem: "规则候选或 pointer 不一致时保留 current/previous；演练漏检、候选或执行语义变化时重做受影响的回归，未变的可靠证据可复用。不可逆动作在执行前发现目标、授权或影响不清时不执行。", unavailable: "规则身份不可验证只关闭依赖它的动作；演练或已登记因子不可用时不打开真人窗口。不可逆动作没有可靠回读时不冒充完成，其他普通工作继续。" },
    searchProjection: {
    ...
    implementation: [
      "highest_authority_agent（最高权限智能体）必须独立登记，当前只授权 codex-root。跨运行框架兼容和普通 Owner 成功都不新增主体；其他智能体的模型、因子、保护及恢复边界先由用户明确决定，再通过正式入口登记。",
      "codex_root_protected_judgment（受保护操作及全场景本人判断）由宿主真实证明的 gpt-6-astra/high 或更高思考等级完成；合格根可自行判断，其他执行者只在新相关疑点确需判断时异步调用或复用合格 Astra；原生实际不可用或被拒才走已授权的新判断任务。合格判断不可用只暂停依赖它的精确不可逆终步，不停整体任务。低档执行者消费的判断须绑定该宿主真实 assistant（助手）消息或 tool call（工具调用）产物、目标、effect（现实动作）、主体、授权和有效期，不要求必须是 final（最终回复）。其他框架缺模型证据只影响新保护判断，可复用已有真实合格判断路线并继续普通工作。",
      "全场景本人判断先结合当前言行、目的与理由、前后语境和已有可靠背景；普通聊天也适用，不以正在执行受保护操作为前提。有具体异常且补证会改变结论时，才最小读取 daily-preferences 的背景/经历或 reply-as-me 连接的真实表达样本；已有足够信息不查库，未加载、未命中或资料不足本身不构成非本人证据。",
      "根发现具体疑点可在既有授权内自主发起或复用合格判断任务。委派以按先后顺序的连续用户原话和必要短背景为主，区分原话、已核事实与主代理推断；不摘孤立怪句诱导结论，不灌入全量历史或两库全文，确需补齐前因后果才追加。",
      "本人背景和表达库只作辅助，保留时间、场景、出处及事实/推断、本人原话/修订/AI 草稿的区别；他人、引用、转贴和 AI 代拟不能冒充本人独立表达。了解个人经历或口吻相似也不保证当前就是本人。",
      "看起来是本人且操作合理时不额外加因子；有具体合理身份疑点或无法可靠判断时，合格判断者可要求最高权限因子，不要求概率阈值、攻击实锤或先锁屏失败。仍基本认为是本人且只有轻微异常时可独立选择无感锁屏；锁屏与最高权限验证分离，不重置设备信任、不自动附带另一处置。形成合理怀疑并判断需保护后，立即暂停与疑点直接相关的原授权数据取用、披露同步及不可逆终步；不相关的思考、可逆工作、修复、测试、验证和恢复准备继续。必要背景核验只读，不增量、不回写、不把当前说法或改稿污染为本人基准。",
      "待验证操作者不应看到两库私人内容和匹配细节，包括可见工具回显；现有受保护通路不能满足时直接本人验证，不先展示资料。合格根可自行判断，确需升级则复用已有合格判断任务，新相关证据或情境实变才重判，不按普通轮次或每次私人读写重复判断或派代理。",
      "十分钟验证窗口从正式可见邀请事件起算：证据为模型真实发出的最终回复或宿主可靠可核对的可见摘要，与实际判断和授权分别绑定；同一事件不因重连、重试或提示重开重置。正式邀请同时给出四类选项，任一已登记方式成功即可，不强制默认。已知软件故障须在邀请前修复；邀请后新故障沿既有通路静默修复，并按实际耗时补足一次，不记为用户失败或无限延期。中性提示不泄露怀疑依据、锁盘安排或实时倒计时；宿主无法隐藏的内容不冒称隐藏。实际锁盘仍由独立产品入口执行；约五分钟操作阻塞是产品成本，不是新倒计时。只能经有效因子及正式恢复入口解除相关暂停，规则 PASS、准备就绪或新一次自评均不替代恢复。",
      ...
      "E82 新增英文中文括注、系统级反膨胀、语义保真和 projectless 默认；E83 增加 coordination_id 跨项目精确 scope；E84–E86 增加 change-surface validation、FastRelease 确定性回执与分阶段墙钟预算；E87 完成运行根与任务临时目录迁移语义；E88 扩展未来模型家族和 Ultra 路由；E89 明确 CI 只在影响交付、发布、兼容或用户决策时运行；E90 增加 PUBLIC 项目 ignored 私有伴随材料的 PRIVATE 收敛链；E91–E100 建立公开分级、耐久授权、Owner 生命周期、可信本地安全、注意力、用户原意、对应快照与不阻断产品的反膨胀边界；E101 激活合并式面板刷新和对话内 daily-preferences，并加入命令策略拒绝删除时的回收站后备路径；E118将根收敛为原则，现行专项语义包括四档模型、局部Owner CAS、七输入发布、独立责任型任务与短时流程及时跟进。",
      ...
      "production_equivalent_human_factor_rehearsal（生产等价真人因子演练）是任何最高权限真人弹窗的前置条件：正式邀请前确认 Passkey（通行密钥）、TOTP（动态验证码）、Recovery（恢复码）、Account（账号验证）四类软件流程及运行条件可用，并按真实用户路径和本次影响面执行 fresh（新鲜）、完整、隔离且生产等价的回归；Google/Microsoft 只是 Account 提供方。不要求每次重跑无关的旧 C MajorAction/Publisher/P0/installer 整链。",
      "只有因子本身可模拟。本次需要复现的 outer adapter（外层适配器）、installer（安装器）、child process（子进程）、capability consume（单次能力消费）、状态迁移、正式readback、rollback与response-loss（响应丢失）路径必须与生产相同；不得mock/stub/monkeypatch核心链、直接返回PASS或拼装组件冒充完整演练。",
      "environment_only_isolation（仅环境隔离）只把filesystem root（文件系统根）、registry hive（注册表分支）、Task Scheduler namespace（计划任务命名空间）、mutex（互斥量）和signing/factor root（签名/因子根）映射到任务隔离位置；候选源码、参数、schema、target/effect commitment、控制流和生产函数不变。同一候选源码、已安装清单、注册表及提供方目标未变时可复用可靠证据，源码不替代安装，模拟不冒充真人或 E2E。候选或执行语义变化仅使受影响证据失效；真实生产或消费路径暴露漏检后先修复，再按影响面重跑，不以换因子或盲重试补救。预检查不触发真人因子或要求重复验证；代码与隔离测试完成后停在第一次真实邀请，等待本人在场。"
    ],
    flow: [
    ...
    boundaries: [
      ...
      "真人邀请前须确认四类软件流程及运行条件可用，按影响面完成充分回归；四类同时可选且无强制默认。取消、超时、失败或入口不可用只暂停该动作，不自动重试、改设备信任或切换因子；新软件故障按实际耗时处理",
      "C/E 规则异常不能触发 BitLocker、锁盘、重启、读取秘密或创建 CoreGoal",
      "官方 App 版本、build 和 versioned path 不得成为准入"
    ],
    failures: [
      { condition: "四类软件可用条件缺失、相关回归不充分或不等价，或受影响证据已经失效", response: "不打开真人验证窗口；先补齐软件条件或修复生产路线，再按真实影响面重跑；未变的可靠证据可复用。" },
  ...
  },
  {
    ...
    implementation: [
      ...
      "2026-09-09 清单实际有 8 项：3 个独立工具（图片超分、Windows 轨迹分析、.NET 方法字符串检查）、1 条既有 Codex 打开做法、4 个现有能力指针。正式 Skill 和壁纸项目继续由各自来源维护；查询未命中或入口不存在时如实说明，不能把目录当全机工具的完整清单。",
      "选定工具的源码、必要运行依赖、许可和恢复说明已保留在固定工具目录，并保存 G:\\80_Backup\\Tools\\LocalToolbox\\lightweight-tools-20260909T062945Z.zip：42,880,045 bytes，SHA-256=3efe2caf405a643f82238d7611e5b8fe94c7cfe7b20dc6cb4e39678a11cbf6ac。本轮核对包大小与哈希，按用途查询真实返回了超分入口；正式能力的 4 个指针不代表对应项目全量入包，H 冷副本未独立验证。轨迹分析器保全了源码与依赖，本轮未新采集或分析系统活动。",
  ...
  rules: [
    {
      ...
      purpose: "所有任务的默认总入口。它定义 E 规则权威、用户原意与实现的分层、长程反需求膨胀审查、指令优先级、事实 Owner、模型自治、english_chinese_gloss、耐久授权、Owner lifecycle、Git 与验证习惯、本任务文件归位清理、发布后看板收口及私人领域路由。",
      ...
      process: ["任务开始、续作或压缩恢复时 Inspect 当前 E release；同任务同版本已读且未压缩可复用正文", "读取最近项目规则，恢复用户目标、更正与授权并区分可换方案", "确定事实 Owner，按触发读取专项合同", "Owner 冲突时先解析 lifecycle 并收敛 exact scopes", "长程实质节点独立审查需求来源和最小实现，主线继续不冲突工作", "实施并分层验证", "归位所需文件，清理本任务各落点无意义副本，回读并说明真实暂留或受阻项", "已登记来源发布回读后用 personal-panel-refresh 只判断对应快照的实质漂移，并把多个有界增量收进一个稳定发布批次", "用人话报告真实结果；不以自造改进项继续已完成目标"],
      ...
      sections: [
        {"title": "全场景语境判断", "paragraphs": ["普通聊天及其他场景先结合任务目的、当前言行和已有可靠背景判断；有具体异常且补证会改变结论时，才读取本人理解库或表达库的最小相关材料。E128活动五文件已包含这些语义；规则生效与每个真实使用场景仍分别验证。"], "items": ["正常核对记忆和整理资料照常帮助，不每轮查库或验证", "资料未加载、未命中、口误、偏好变化或技术故障本身不是非本人证据", "两库只辅助，保留真实原话/修订/AI草稿、时间、场景与出处；已足够判断不为流程补查", "形成合理怀疑且需要保护时，立即暂停相关数据消费和不可逆终步；不相关的思考、可逆修复、测试与恢复继续，必要查证只读且不展示私人内容或匹配细节", "合格根自行判断，新相关疑点确需判断时才异步调用或复用合格判断；普通轮次和私人读写不重复派代理"]},
        ...
        { title: "授权、Git 与验证", paragraphs: ["本机可逆工作直接做；外部 effect（现实动作）需要明确授权。durable grant（耐久授权）不要求同轮重述，但不会覆盖上位 deny、证据、目标或不可逆边界。进入怀疑或待本人验证后，耐久授权、Owner 绑定、管理员权限和 E rules PASS 都不能恢复相关数据消费；不相关的思考、可逆修复、测试和恢复仍继续。用户私人账号空间在默认私人且没有 public/share 信号时与本机私密目标等价可信，但信任不产生写授权。"], items: ["长期明确授权在冻结 goal/scope 内跨 root、child、压缩和 successor 持续有效；前提成立须真实调用一次", "实际 unavailable、deny、step_up、needs_evidence、action-time confirmation、error、身份/CAS/target/read-back 失败仍按现场结果处理", "PUBLIC 个人数据唯一 L1–L5 表由授权合同拥有：L1/L2 不受个人数据限制，L3+ 才进入可能敏感审查", "项目收紧 L1/L2 默认必须有真实项目需要和用户对精确项目、范围、限制的明确授权", "普通非长期或已归档且 clean 的 terminal predecessor 无 residual 用 RecoverRelease；有 residual 用 RecoverReleaseClaim；未归档 long_term_task 只接续或正式退役", "只有真实 threadId 可归档", "E release 激活是 UAC expected-preimage CAS，不经过旧 Publisher、人类因子或 CoreGoal", "不覆盖用户已有改动", "force-push（强制推送）不在默认授权内", "source（源码）、test（测试）、install（安装）、publish（发布）、fresh task（全新任务验证）与 E2E（端到端验证）独立", "个人仓库必须由远端默认分支回读"] },
    ...
    },
    {
      ...
      plainLanguage: "先确认目标、授权和实际影响，再决定能不能做。普通聊天中出现合理身份疑点时，也可由合格主体要求本人验证；轻微异常可独立选择无感锁屏。需要保护时只暂停相关数据消费与不可逆终步，不相关的思考、修复、测试和恢复继续。可版本化的规则、配置或系统状态保留当前可用版和明确回退点；消息、付款等不可逆外部动作不承诺回滚，执行前核对精确目标与内容，执行后从真实目标回读。",
      ...
      sections: [
        {"title": "全场景语境判断", "paragraphs": ["普通聊天及其他场景先结合任务目的、当前言行和已有可靠背景判断；有具体异常且补证会改变结论时，才读取本人理解库或表达库的最小相关材料。E128活动五文件已包含这些语义；规则生效与每个真实使用场景仍分别验证。"], "items": ["正常核对记忆和整理资料照常帮助，不每轮查库或验证", "资料未加载、未命中、口误、偏好变化或技术故障本身不是非本人证据", "两库只辅助，保留真实原话/修订/AI草稿、时间、场景与出处；已足够判断不为流程补查", "形成合理怀疑且需要保护时，立即暂停相关数据消费和不可逆终步；不相关的思考、可逆修复、测试与恢复继续，必要查证只读且不展示私人内容或匹配细节", "合格根自行判断，新相关疑点确需判断时才异步调用或复用合格判断；普通轮次和私人读写不重复派代理"]},
        {"title": "独立登记、合格判断与本人验证", "paragraphs": ["当前独立登记的最高权限主体仅 codex-root。重点受保护判断必须由真实宿主证明的 Astra High 或以上完成；模型资格与主体登记、动作授权及实际执行分别核验。"], "items": ["合格根自行判断；其他执行者仅在新相关疑点确需判断时异步调用或复用合格 Astra，原生真实不可用时才建独立判断任务；缺判断只停依赖它的精确不可逆终步", "角色、名称、用户自报型号和管理员令牌不能证明最高权限或模型身份", "有具体合理身份疑点或无法可靠判断时可要求本人验证，不要求概率阈值；轻微异常可独立选择无感锁屏，不强制附带验证或重置信任", "十分钟从正式可见邀请起算，同一邀请重试不重置；邀请后新软件故障静默修复并按实际耗时补足一次，真实恢复入口才能解除相关暂停", "规则哈希失败不能机械触发锁盘，规则 PASS 也不能代替本人判断和恢复"]},
        { title: "判断边界", paragraphs: ["最高权限智能体结合真实意图、目标、范围和可恢复性作语义判断；机械层只验证真实身份/模型证据、principal、schema、签名/nonce、目标、确定事实和 effect 边界。ApplySevereActionGate 等入口不能仅按动作类别自行隔离；需要保护的不可逆终步须有合格判断或独立、确定性的完整性证据及当前 Owner 回读，普通工具或软件故障只影响依赖它的能力。"], items: ["人类因子：Passkey、TOTP、Recovery、Account，四类同时可选，无强制默认，任一已登记方式成功即可", "Google 与 Microsoft 只是 Account provider", "取消、超时和失败只暂停该动作；软件新故障与用户失败分开处理", "E release activation 明确不属于人类因子路径"] },
    ...
    },
    {
      ...
      sections: [
        ...
        { title: "退役边界", paragraphs: ["历史中央系统及其冻结文档不是控制面、默认个人上下文或运行产品。备份对象存在也不会恢复它。"], items: ["不调用旧 context、beacon 或 probe", "不读取历史私人数据库和媒体", "历史命名不是新基座理由"] }
      ],
    ...
    },
    {
      ...
      decisions: ["当前任务值不值得使用某项能力", "英文自然词或短语是否需要按 english_chinese_gloss 在首次出现后紧跟中文括注", "PUBLIC 内容判断是否需要按需读取授权合同的唯一分级表", "项目规则是否在定义客观 precondition，还是错误地降级既有长期授权", "应读多少上下文", "是否安装运行时或建议插件", "可信 model/effort/role/E release identity 是否成立", "应并行 0–10 个代理以及选择 Luna/Terra/Sol/Astra 哪个家族；未分类型号只保留同一精确 ID 后代，已有明确能力顺序才跨型号比较", "当前验收是否会被实现知识污染，需不需要 implementation-blind fresh E2E", "自然请求是否在没有路线提示时自主选对能力并交付正确可见结果", "Codex 官方更新是否仍是稳定主体且现场 event/capability 可用", "顶层 task 的真实创建调用返回 success、unavailable、failed 还是 dispatch-unconfirmed", "怎样保持代码和仓库不过度膨胀", "怎样收起任务文件、保护原件与并发状态，并完成清理回读"],
      ...
      sections: [
        ...
        { title: "原生经济委派与顶层任务", paragraphs: ["现行 11 条规则先绑定 model、effective effort、root/child role 与 E release/commit/ruleset/合同 SHA，再做 0 到 10 决策；durable grant 对全部后代持续满足用户允许，但身份、slot 与真实工具结果仍现场证明。"], items: ["gpt-5.6-luna：封闭可验、读重和确定性工作", "gpt-5.6-terra：强耦合实现、深调试和架构审查", "gpt-5.6-sol：复杂实现、战略判断与终审", "gpt-6-astra：关键路径最高难度、最易返工或最快正确收敛，按质量与总成本决定是否值得", "其它 verified 型号是 unclassified（未分类），仍可派同一精确 ID 后代；不得按价格、版本或 UI 顺序猜强弱", "各型号档位取宿主实际可用集；Luna 默认 Max，Terra/Sol 优先候选为 High，实际组合由任务语义决定；后代同时受根和直接父级的分型号组合上限约束，Ultra 比较时按 Xhigh", "家族与 effort 只能向下收窄，Root 持续负责目标、风险和最终集成", "顶层 Owner task 需 live scope 无 Owner、独立成果/责任必要和正净收益；能否 Claim 只说明施工路径，projectless 是默认", "create_thread 只真实调用一次；threadId/clientThreadId 表示受理，clientThreadId 仍不能用于归档"] },
