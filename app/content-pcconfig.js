...
const pcconfigSnapshot = createProjectSnapshot({
  observedAt: "2026-09-07T20:16:52Z",
  label: "主工作站配置与独立恢复入口可读；H 首次冷备已有回读，目前离线；G 当前副本与恢复任务本轮就绪",
  boundary: "本轮合入已发布 PCConfig 912b1af 的恢复、GPU 调度与 Windows 运维增量；H 在 9 月 5 日已有首次备份验收，9 月 6 日因介质不可用正常跳过。旧硬件、P0、授权文件和副驾驶证据仍保留原观察时间，不把源码、安装或历史冷备冒充新机恢复",
  metrics: [
    { label: "配置地图", value: "v6 · 15 项目 · 157 键" },
    { label: "恢复任务", value: "11 Ready · H 当前离线" },
  ...
  facts: [
    ...
    { label: "恢复任务", value: "2026-09-07 只读回读：11 个核心恢复任务均 Ready、最近结果 0；14 组 G 来源存在、9 个必需集合 fresh（新鲜）；H 首次冷备已登记，当前离线" },
    ...
    { label: "核心恢复任务", value: "2026-09-07T20:16:52Z 的 Inspect（只读检查）为 ready、warnings=[]：11 个任务均 Ready、最近结果 0，14 组 G 来源存在，9 个 required（必需）集合 fresh；另有 2 个独立凭据/保护数据 Owner（责任源）。H 当前不可见；最近 Cold（冷备）于 9 月 6 日 23:30（本机时区）因 H_unavailable 返回 skipped，没有写 G、H 或云端。", hero: false },
    { label: "秘密恢复回读", value: "Secret 恢复 10/10 项在三条路线一致；G 路 20 份、PRIVATE 路 65 份快照，覆盖 33 天。公开回执不返回秘密原文。", hero: false },
    { label: "稳定机器投影", value: "稳定机器投影当前为 v6，登记 15 个项目路径关系；旧开发存储验收覆盖17个锚点；现已登记18个，新增OLED源码只是待稳定版的历史恢复点。C盘用户配置inventory（清单）有26项，三者都是恢复导航而不是应用可用性的替代证明。", hero: false },
  ...
  gaps: [
    ...
    "H 在 2026-09-05 已完成回盘与首次冷备验收，登记容量为 2,048,593,489,920 bytes（约 1907.90 GiB）；当前介质离线与最近 H_unavailable 跳过不撤销该历史结果，也不证明之后 G 的增量已进入 H。新机恢复和恢复后重建 Hot/Cold 仍未验收。",
...
export const pcconfigProject = {
  ...
  usageExamples: [
    ...
    { moduleSlug: "recovery-backup", ask: "把 G 的资料和照片备份到 H，也必须先做加密 Carrier 吗？", effect: "普通 G 副本保持原生格式；凭据与 P0–P7 载荷各走正式入口。14 组集合已有 H 首次备份历史，当前 H 离线，接回并由本人解锁后才检查本轮增量和来源新鲜度。" },
...
export const pcconfigModules = [
  ...
  },
  {
    ...
    status: "2026-09-07 Inspect 为 ready：11 个任务、14 组 G 来源、9 个必需集合 fresh；9 月 5 日 H 首次备份已验收，现在离线。新机恢复、WEPE 识盘/网络和 BIOS 保存重启仍未验收",
    statusTone: "mixed",
    searchAliases: [
    ...
    decisionImpact: [
      ...
      "G 是在线 Hot（热备）来源，H 是人工解锁窗口中的 Cold（冷备）来源。14 组集合含普通 PersonalData、PersonalMedia、AI 课程与 RecoveryKit；只复制既有 G 字节，不读取 E 的活动个人库。9 月 5 日首次 H 备份已登记，当前 H 离线，新的 G 增量仍需下一次冷备回读。",
    ...
    flow: [
      ...
      "依赖齐全后，按 9 阶段/56 任务重建计划任务，并对 21 个 live 启动项逐个检查；只恢复现役且前置条件满足的项，不从旧 XML 复活退役任务。",
    ...
    boundaries: [
      ...
      "H 已有 2026-09-05 首次备份和卷身份验收；当前离线不能读取其最新副本，G 新鲜回执与源码测试也不证明 H 已追上后续增量或新机恢复完成",
    ...
    verification: [
      ...
      "本轮已发布来源为 PCConfig PRIVATE main=origin/main=912b1af985938acab08c806cc0e1e9f03f0e2611，远端 main 同值。源码工作区另有 stable_machine_projection.json 的并发候选，本页不将其替换为当前稳定投影。Windows 小工具与热备脚本现在由 PCConfig 本身持有，不依赖已移除的独立脚本仓库。",
      ...
      "scheduled_task_rebuild_plan.json 当前为 9 个阶段、56 个任务；Get-StartupInventory 当前观察 21 个启动项。两者是重建导航与现场清单，不单独证明自然启动",
      ...
      "backup_media_structure.json 在已发布 d4775b1 记录 2026-09-05 的 H first_backup_verified（首次备份已核验），包含 CoreRecovery 与 Password Center 的独立回执引用。当前 H 不可访问；历史备份、当前可读、新机/换板恢复和恢复后 Hot/Cold 重建是不同证据，后两项仍未完成。",
