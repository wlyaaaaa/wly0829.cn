...
export const pcconfigModules = [
  ...
  },
  {
    ...
    "implementation": [
      ...
      "两端静态 Codex OAuth 客户端分别为 codex-secondary-laptop 与 codex-main-desktop，连接名分别为 main_desktop 与 secondary_laptop；回调登记 http://127.0.0.1/callback，只允许操作系统临时端口变化，换码仍绑定原完整回调与 PKCE S256。issuer（令牌签发方）与回调 iss 须精确匹配所选入口及末尾斜杠，DCR/CIMD 关闭。副机 main_desktop 已更新为主机新域名，但重新登录未完成；ChatGPT 主机新地址与备用地址也仍需独立授权，配置存在不等于客户端连通。",
      ...
      "2026-09-11 16:29（北京时间）已完成受控故障演练：主动停止主机 Cloudflare 隧道，主公网返回 530 / 1033；通用 MCP 客户端强制走副机公网地址，连续三次命令均返回 WLY、Cloudflare 已停止，随后恢复主入口。备用连接用于已确认的主入口连接故障，不能恢复关机的主机，也不自动切换或重放结果未知的命令。该演练不包含 ChatGPT 新入口的 OAuth 登录与刷新。",
