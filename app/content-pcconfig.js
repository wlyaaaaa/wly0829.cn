...
const pcconfigSnapshot = createProjectSnapshot({
  observedAt: "2026-09-03T16:14:09.4849149Z",
  label: "主工作站配置与恢复可读；G 热备范围已对齐，H 尚未返回，副驾驶现场仍待目标机回读",
  boundary: "本次只刷新 G/H 备份范围、个人副本回执与核心恢复只读检查，其余机器事实保留各自原观察时间；14 组是 H 复制计划，不是已完成的 H 副本或首次恢复。副驾驶仍仅有主机限定账本与恢复源码证据，设备健康、U 盘世代和 NVMe（固态硬盘）外置接管待目标机验收",
  metrics: [
    { label: "配置地图", value: "v6 · 15 项目 · 157 键" },
    { label: "恢复任务", value: "11 Ready · H 未返回" },
    { label: "秘密恢复", value: "10/10 三路一致" },
    { label: "设备角色", value: "1 主工作站 · 1 副驾驶" }
  ],
  facts: [
    { label: "配置地图", value: "2026-09-01 原快照：PRIVATE main=3fae514；稳定机器投影 v6、44 份 Registry、15 个项目、157 个配置键、89 个环境变量、64 段 PATH、11 个受管软件、21 个 live 启动项、87 个任务" },
    { label: "开发存储", value: "V 盘 299.9 GiB、Z 盘 12 GiB；17 个恢复锚点的 5/5 检查通过" },
    { label: "恢复任务", value: "2026-09-03 只读回读：11 个核心恢复相关任务均为 Ready、最近结果 0；14 组 G 来源存在，CoreRecovery 3/3 检查通过，H 尚未返回" },
    ...
    { label: "2026-09-01 配置地图", value: "原快照中，源仓库 PRIVATE（私有）main 与 origin/main 同为 3fae514305862a0eafefdcbedb233436094c3efe，worktree clean；配置地图覆盖 44 份 Registry、15 个项目、157 个配置键、89 个环境变量、64 段 PATH、11 个受管软件和 87 个任务；Get-StartupInventory 同次回读 21 个启动项（20 enabled、1 disabled）。这些 Git/运行态结论仍绑定原观察时刻。", hero: false },
    { label: "任务登记后续投影", value: "后续 tasks.json 的 scheduled_tasks 有 89 项，来源文件 LastWriteTimeUtc=2026-09-02T01:59:29.1431081Z，generation_id=task-scan-20260902t015928757-b662d93c249541e8；对应任务重建计划更新时间为 2026-09-02 09:59:29 +08:00。本次只读取已有来源投影，没有重跑扫描或任务，不能据此刷新原 87 项快照的运行结果。", hero: false },
    { label: "开发存储回读", value: "开发存储回读 V 盘 299.9 GiB、Z 盘 12 GiB；17 个恢复锚点的 5/5 检查通过。", hero: false },
    { label: "核心恢复任务", value: "2026-09-03 的 Inspect（只读检查）为 ready、warnings=[]：11 个任务均 Ready、最近结果 0，14 组 G 来源存在，9 个 required（必需）集合的新鲜度均为 fresh；另有 2 个独立凭据/保护数据 Owner（责任源）。3/3 区域检查通过不等于 H 实体复制或应用恢复。", hero: false },
    { label: "秘密恢复回读", value: "Secret 恢复 10/10 项在三条路线一致；G 路 20 份、PRIVATE 路 65 份快照，覆盖 33 天。公开回执不返回秘密原文。", hero: false },
    ...
    { label: "P0 当前状态", value: "P0 current 为 revision 68、normal、active=LKG、trusted=true、recovery_status=null；最新自然启动为 46984 ms、deadline_met=true，低于 180 秒门。", hero: false },
    { label: "Drift 现场", value: "2026-09-01T02:10:34Z fresh Drift 返回 6 pass、1 warn、0 block，证据为 6 pass、0 fail、1 unknown；唯一 attention 是普通权限任务观察不完整，runtime_health 仍 pass 且 failure_last_result_count=0。", hero: false },
    ...
    { label: "个人数据当前副本", value: "PersonalDataReplica-Hot-Daily 已启用，状态 Ready、最近结果 0；最新 personal-data.replica-receipt.v1 完成于 2026-09-03T15:45:26.813169Z，五映射均 complete/post_verified=true、errors=[]、payload_content_read=false。E→G 当前副本按各映射及其排除规则同步覆盖、重命名和删除，不提供历史版本；Media/Packages 重复树已排除，专用 PersonalMedia/Packages 保留。回执只证明完成时刻，不证明两次任务之间持续相同。", hero: false },
    { label: "Password Center", value: "Password Center 独立安装态为 current：9/9 文件与锚点一致，retired_c_policy_used=false。银行卡三字段可原子保存与盲填，但真实支付页提交始终由用户决定。", hero: false },
    { label: "授权文件", value: "授权文件实现已包含在上述 9/9 current 安装态中：authorization_file_broker.py 的 source/installed SHA-256 同为 5eb7b3e59099ccde45804824d3edef03ced8abfab3ff8a3cbc1252db65123854，manifest anchor 匹配且 Inspect 零写入；独立 6/6 合成测试通过。", hero: false }
  ],
  gaps: [
    ...
    "H 当前仍未返回，返还盘容量与卷身份未观察；14 组 G→H 计划、源码及隔离测试不证明真实 H 副本。最近冷备为 H_unavailable 跳过，没有当前 Codex 对话 H closure；首次 H 复制、独立恢复和恢复后在新系统重建 Hot/Cold 均尚未验收。",
    ...
    "PersonalDataReplica-Hot-Daily 的最新五映射回执绑定 2026-09-03T15:45:26.813169Z；该结果不证明随后新增或修改的数据已在下一次自然运行前同步，也不证明 H 冷备或手机端恢复已经完成。",
...
export const pcconfigProject = {
  ...
  visibility: "私有仓库",
  statusTone: "mixed",
  cardStatus: "主工作站配置可用；protected-policy 依赖分类需修复，副驾驶笔记本现场状态待在该机回读",
  ...
  summary: "PCConfig 是我两台 Windows（微软操作系统）电脑的机器事实总账、配置地图和恢复中心。它管着 WLY 台式主工作站的磁盘、运行时、计划任务、备份、重大机器动作和明确选中文件的授权加密，也登记副驾驶笔记本的远控、轻量开发、独立灾备与故障接管边界。平时想查“为什么这个任务没启动”“把这些文件加密并确认可恢复”“离开主机时笔记本能做什么”或“台式机坏了怎样继续工作”，直接问即可。",
  ...
  plainExample: "比如台式机突然开不了机，而原 NVMe（固态硬盘）完好：副驾驶方案会让笔记本用自己的 Windows 启动，把原盘装进已验收的硬盘盒后只读认出 E/V 工作区，再继续必要的 Git、文档和轻量开发。它同时说清哪些来自 Git、哪些来自原盘、哪些账号要重新登录、哪些重型能力不能继承，以及修好台式机后怎样交回；这条笔记本与原盘接管路线仍须真机演练，不能把源码或离线测试当成已经接管成功。",
  ...
  components: [
    ...
    { name: "个人数据当前副本", responsibility: "把五个精确 E 源按当前状态镜像到 G，供本机故障后的快速恢复。", implementation: "PersonalDataReplica-Hot-Daily 由 Scripts 项目拥有；覆盖、重命名和删除直接传播，不保留历史版本。Media/Packages 由专用恢复包路径负责，不再重复镜像；只有五映射按各自排除规则 complete/post_verified 的业务回执才能证明完成时刻一致。G→H 则是独立的加法复制，不沿用 E→G 的镜像删除。" },
  ...
  usageExamples: [
    ...
    { moduleSlug: "runtime-startup", ask: "本地模型、OCR 和语音任务会不会一起抢显卡？", effect: "先告诉我显卡现在由谁占用、重任务能否排队，再让所有重型工作走同一个串行入口；机器条件不满足时一个新重任务都不启动，也不会结束正在工作的其他任务，普通文档和 CPU 工作照常继续。" },
    ...
    { moduleSlug: "recovery-backup", ask: "系统损坏后把 Codex 对话恢复回来。", effect: "先确认恢复点完整、每个对象都能对上哈希，再从已经闭合的 G 热备或 H 冷备恢复；原始对话不会上传 GitHub。H 当前尚未返回，因此这次不能把冷备恢复说成已经完成。" },
    { moduleSlug: "recovery-backup", ask: "把 G 的资料和照片备份到 H，也必须先做加密 Carrier 吗？", effect: "普通 G 副本不要求加密 Carrier；真正凭据恢复集与 P0–P7 载荷走正式入口。14 组计划已登记，但 H 尚未返回，不能称已复制或已恢复。" },
...
export const pcconfigModules = [
  {
    ...
    value: "把散落在磁盘、环境、项目文件和本机服务里的配置，收成一张一目了然又能核验的机器地图。除了“登记了多少项”，我还能看到主工作站的处理器、内存、显卡、Windows 与开发工具精确版本，以及这些结论是哪次观察留下的。TimeAudit 若发现持续硬件压力，只会送来一份有界异常摘要，PCConfig 再决定是否值得重读稳定机器事实，不让一次短暂卡顿改写整台电脑的基线。",
    ...
    implementation: [
      ...
      "本页精确机器配置来自 stable_machine_projection v6：observed_at_utc=2026-08-30T11:27:53.0247541Z，generated_at_utc=2026-08-30T11:27:58.3567618Z，canonical_sha256=sha256:e34001b8a8b66b88a213646faa7500de2eeee41c9b8dede6c81a8da60a742eaf；补齐字段不表示重新采集。",
  ...
  },
  {
    ...
    status: "2026-09-01 原快照有 21 个启动项、87 个任务，10 个核心恢复任务 Ready 且最近结果 0；2026-09-02 后续任务来源投影为 89 项，未据此重采运行态",
    statusTone: "mixed",
    searchProjection: {
    ...
    value: "不再停在“软件装着”这一步，而是看正确版本能不能用、启动入口还在不在、计划任务上次怎样结束，以及重装后该按什么顺序接回。主工作站有三条彼此独立的连续性链：ToDesk 在无人登录时维护机器侧远控服务，微信在登录后只补到两个健康实例，WeFlow 保住第二路持久配置与可读接口。出问题时会落到具体任务或组件，而不是一句“电脑环境坏了”。",
    ...
    implementation: [
      ...
      "Get-StartupInventory 本轮 live 回读 21 个启动项，其中 20 enabled、1 disabled；版本化 startup_snapshot 仍是维护基线，不能覆盖当前现场。",
      "2026-09-01 原快照的 tasks Registry 有 87 项；10 个核心恢复任务当时均为 Ready、最近结果 0，并由 CoreRecovery 3/3 验收分层回读。",
      "后续 tasks.json 的 scheduled_tasks 数组已登记 89 项：文件 LastWriteTimeUtc=2026-09-02T01:59:29.1431081Z，task-scan generation=task-scan-20260902t015928757-b662d93c249541e8；scheduled_task_rebuild_plan.json 同批更新时间为 2026-09-02 09:59:29 +08:00。此次只读投影，没有调用 scan_tasks 或触发任务，不能把 89 项来源登记反推成最新运行态验收。",
      ...
      "WeChat AutoStart 以当前交互用户 / Limited（受限权限）在登录后延迟 30 秒，经 wscript.exe → Invoke-PowerShellHidden.vbs → Scripts\\Invoke-WeChatDualLaunch.ps1；只启动 C:\\Program Files\\Tencent\\Weixin\\Weixin.exe，忽略子进程与零线程/零句柄幽灵，补到两个顶层实例后连续稳定 15 秒。桌面与开始菜单的 微信双开.lnk 使用同一幂等入口。",
    ...
    flow: [
      ...
      "恢复微信官方客户端和 Scripts 启动器后，按双开安装器回读任务与快捷方式；账号由用户重新确认。恢复 WeFlowBridge 与原持久 profile 后，再验证 16000 绑定、health/sessions/contacts 和资料库身份",
    ...
    verification: [
      ...
      "Get-StartupInventory.ps1 -Json 当前覆盖 21 个启动项",
  ...
  },
  {
    ...
    status: "2026-09-03 CoreRecovery 只读检查 ready：11 个任务、14 个 G→H 计划集合、2 个独立 Owner，区域检查 3/3 PASS；H 尚未返回，真实 H 副本、首次恢复、新机端到端、WEPE 识盘/网络和 BIOS 保存重启仍待验收",
    statusTone: "mixed",
    searchAliases: [
    ...
    value: "遇到 Windows 重装、换电脑、系统盘故障或只能进入 PE（预安装环境）时，恢复从手里真正可用的来源开始，并按“不伤原盘”的顺序推进：先认机器和磁盘，再处理主板设置、Windows 与驱动，接回三个控制面、运行时、项目、用户配置、任务和启动项，最后恢复登录与受保护数据并在真实应用里验收。日常备份则把 G 盘现有普通资料、照片和视频的原生字节纳入 H 计划；普通资料不必先做成加密 Carrier（恢复载体）。",
    ...
    result: "交付的不是一句“文件拷回来了”，而是一张按层可续作的恢复清单：这次属于哪种故障场景，F/G/H/Git 与 P0–P7 各提供了什么，哪些设置和数据已现场回读，哪些应用仍要登录，哪些只是文件存在或用户确认，哪些真机验收尚未发生，以及失败时应停在哪个检查点。当前 H 尚未返回，真实 H 副本、首次恢复、新机端到端、WEPE 识盘/网络与 BIOS 保存重启都不能写成已完成。",
    readerStates: {
    ...
    decisionImpact: [
      ...
      "G 是在线 Hot（热备）来源，H 是人工解锁窗口中的 Cold（冷备）来源。当前 14 组 H 计划包含普通 PersonalData（个人资料）、PersonalMedia（照片、视频、音频等媒体）、AI 课程和 RecoveryKit（恢复材料包）；只复制既有 G 字节，不去读 E 上的活动个人库。H 尚未返回，计划不是已完成副本。",
      ...
      "G 的个人副本会随 E 当前内容同步覆盖、重命名和删除；H 不按 G 的删除清单清除旧路径，但同名更新不提供上一版文件的版本历史。个人资料和媒体必须有 36 小时内的完整五映射回执，Hot 闭合上下文须在 48 小时内；目录存在或任务退出码为 0 不能代替这些证据。重复 Packages（恢复包目录）已从日常来源排除，专用手机包、云候选和精选分类保持原分工。",
    ...
    implementation: [
      ...
      "计划任务恢复投影有 9 个阶段、54 个现行任务；当前 live 启动清单为 21 项。先恢复项目、解释器、账号入口和配置，再按 rebuild_required、optional、manual_after_install、hold_for_user 与 vendor-owned 边界重建；任务 identity 使用 task_path + task_name，并保存 exact preimage。",
      ...
      "RecoveryKit 整树从 G 进入 H 的 90_人工保留区，但 CORE_RECOVERY 只走已验证上下文闭包，避免重复；Codex 对话只复制不可变 cold-payload（冷备对象），由其 Owner 逐对象 SHA-256 回读 H closure 后再发布 H current.json。首次 H 恢复还须调用现有 Invoke-PasswordCenterColdBackup.ps1 正式入口处理 RecoverySets，通用 Cold 不作 raw copy（直接字节复制）。",
      "Scripts 的现役 PersonalDataReplica-Hot-Daily 仍只维护五个 E→G 当前映射；Media 映射排除 Packages，保留专用 G/PersonalMedia/Packages，防止重复目录再次生成。两份本人明确不要的旧历史包和冗余树已由来源工程退役，不复活；现有手机恢复包、云候选与精选分类未改。"
    ],
    flow: [
      ...
      "依赖齐全后，按 9 阶段/54 任务重建计划任务，并对 21 个 live 启动项逐个检查；只恢复现役且前置条件满足的项，不从旧 XML 复活退役任务。",
    ...
    boundaries: [
      ...
      "H 尚未返回，实际容量与卷身份未知；14 组计划、G 新鲜回执、源码和隔离测试都不是 H 实体副本或首次恢复完成证明",
      "永久删除和秘密恢复始终保留精确人工门禁"
    ],
    failures: [
    ...
    sources: [
      ...
      { path: "E:\\PCConfig\\registries\\scheduled_task_rebuild_plan.json", role: "9 个阶段、54 个现行任务、恢复决定与授权边界" },
      { path: "E:\\PCConfig\\registries\\startup_snapshot.json", role: "版本化启动维护基线；当前 21 项仍由 live Provider 回读" },
      ...
      { path: "E:\\Projects\\Tools\\Scripts\\Sync-PersonalReplicasToG.ps1", role: "五映射 E→G 当前副本及 Media/Packages 排除规则" },
    ...
    verification: [
      ...
      "本次备份/恢复来源为 PCConfig PRIVATE main=d7ff1b3c2a0c3bfea2c640bca5ea0d764d8fdaa5，Scripts PUBLIC main=da6153c476685e88fee3a42bdb03701e5e04381b；两者已独立远端回读。Scripts 业务修复为 540627dfebc0b219335c69a358780c6271aa7276，后续仅补 UTF-8 BOM（编码标记）",
      ...
      "来源 Owner 已报告现役 PersonalDataReplica-Hot-Daily 连续两次 complete/post_verified；本次独立读取最新回执为 2026-09-03T15:45:26.813169Z、五映射均 complete/post_verified=true、errors=[]、payload_content_read=false，并只读确认重复 Media/Packages 不存在、专用 PersonalMedia/Packages 存在。它不证明下一次运行或 H 恢复",
      ...
      "scheduled_task_rebuild_plan.json 当前为 9 个阶段、54 个任务；Get-StartupInventory 当前观察 21 个启动项。两者是重建导航与现场清单，不单独证明自然启动",
      ...
      "来源工程已通过隔离复制、只读目标替换、硬链接保留与源变化测试；core_recovery_maintenance.test.ps1 同时覆盖 Inspect、Hot、Cold、closure、时效、卷身份和中断回执。本轮只读复核对应源码与测试范围，没有重跑备份任务、读取私人原件或制造真实 H 验收",
  ...
  },
  {
    ...
    shortTitle: "秘密与服务入口",
    title: "Password Center（密码中心）与固定 Workspace Provider（办公服务入口）",
    ...
    decisionImpact: [
      ...
      "Gmail 可找邮件、读指定正文和附件、管理草稿、发送及精确标签；Drive 可找文件、下载/导出、上传/更新及对指定用户分享；Calendar 可查日历、创建/修改/删除事件和另行邀请。当前快照只证明固定绑定已配置，不证明这些远端动作在本次都已验收。",
    ...
    implementation: [
      ...
      "PRIVATE Git 新设备恢复独立于已退役 C Policy、Publisher、MajorAction 和 CoreGoal；完成后重新发布 PRIVATE Git 恢复集，G 在线才额外同步热副本，G 缺失不阻断这条恢复。它和普通 RegisteredCarrier 的恢复应用不是一条可混用的旧授权链，也不能由文件存在直接判定恢复成功。",
      ...
      "Drive 读取为 DriveSearch、DriveGet、DriveListPermissions、DriveDownload、DriveExport；写入为 DriveUpload、DriveUpdate、DriveShare、DriveUnshare、DriveTrash、DriveRestore、DriveDelete。普通文件走 files.get?alt=media；Google Docs 走 files.export，格式限定 txt/md/pdf/docx 与匹配扩展名，写本地输出使用同目录临时文件和原子提交，不获准 Overwrite（覆盖）就保留已存在目标。",
      ...
      "固定 HTTP 超时 90 秒、单请求传输上限 67108864 bytes（64 MiB）；Gmail 附件和 Drive 上传/下载超限就明确失败，不自动搭建续传平台。Google Docs export 另受上游 10 MiB 响应上限约束，exportSizeLimitExceeded 映射为 drive_export_response_oversize，不误报权限不足。",
      ...
      "每个写动作的 -DryRun 都只校验本地参数，不读取 DPAPI、不刷新 token、不联系 Google；实际写超时或上游结果歧义统一 effect_unknown，不能自动重放。OAuth token exchange（令牌交换）本身也只尝试一次。",
    ...
    boundaries: [
      ...
      "单请求上限 64 MiB，Google Docs 导出另有 10 MiB 上限；大文件失败不会引入无边界上传平台",
    ...
    verification: [
      ...
      "独立 Password Center 9/9 安装一致性和现有 10/10 恢复覆盖保留原快照证据；干净新机恢复、每种办公写入、真实网页登录/支付和 CSV 实际导入均不因本次说明补齐而升级为已验收。",
  ...
  },
  {
    ...
    value: "给几个重要文件加密时，最怕处理到一半中断后全部重来，或解密时把目标目录里正在使用的同名新文件冲掉。这个模块只碰我点名的范围：按文件保存进度，完成后核验密文，换目录恢复时先查冲突；正文和密钥都不用交给模型，也不用把普通文件硬塞进凭据库或 Vault。",
    ...
    result: "正常时得到一份可续作、可校验、可恢复且不会覆盖不同内容的加密包；中断后只继续未完成文件，恢复前还能发现密文是否被改动。技术层保留 manifest（清单）、key envelope（密钥信封）、分块密文、加密索引和不含正文/密钥的回执；来源、密文或恢复目标有问题时返回精确的部分完成、冲突或篡改状态。当前没有独立预览动作，也没有用真实用户文件跑最高权限 E2E，不能把合成测试写成正式加密载体已经完成。",
  ...
  },
  {
    ...
    teaser: "过去的万能管理员旧平台已经退役；规则发布、BitLocker、密码中心与各类受保护数据动作如今各归自己的窄入口，不再共用一把通吃的钥匙。",
    status: "protected-policy status=retired 且 6 个退役任务缺席；现行 Registry 37 个依赖，但当前分类 validator 因 allowlist 漂移失败",
    statusTone: "mixed",
    searchProjection: {
    ...
    value: "遇到高影响机器操作，我能先知道现在该找谁：规则发布归 .agents，BitLocker、Password Center 与 P0–P7 各走独立产品入口；Codex Home 只保留已闭合的机器落点事实，迁移工具已经退役。旧 C policy 继续留作历史材料，但不会因为曾有管理员能力就重新变成万能执行器。",
    ...
    implementation: [
      ...
      "现行 protected_policy_retirement Registry 有 37 个 source dependency；固定状态入口仍返回 retired、mutations_performed=false。当前 Test-ProtectedPolicyRetirement 因 independent_product_installer 不在 classification allowlist 中返回 protected_policy_retirement_dependency_classification_invalid。",
    ...
    flow: [
      ...
      "核对当前 37 个 source/machine dependency（源码/机器依赖）的 disposition（去向分类）；历史 36 项通过结果不代表新增分类已获当前校验器接纳",
    ...
    verification: [
      ...
      "Test-ProtectedPolicyRetirement 当前 FAIL：37 个依赖中的 independent_product_installer 未被 validator allowlist 接纳；固定 retired 状态、6 个退役任务缺席和独立产品保留仍有各自证据，但不能合并称整体验收 PASS",
      "机器收敛回执证明 6 个目标任务 absent、无匹配 service/worker，P0 boot recovery、Password Center 和 BitLocker 未改",
      "PCConfig 固定 source cutoff 为 3fae514305862a0eafefdcbedb233436094c3efe；main 与 origin/main 一致、worktree clean。4d17554 已退役完成使命的迁移工具；本轮只读回读确认 C:\\Users\\10979\\.codex LinkType=Junction、Target=E:\\Data\\AppData\\Codex，且 E 目标存在。",
