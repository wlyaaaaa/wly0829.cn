...
export const pcconfigProject = {
  ...
  usageExamples: [
    ...
    { moduleSlug: "runtime-startup", ask: "登录后微信只开了一个，或者第二路 WeFlow 读不到了，怎么恢复？", effect: "微信启动器只补到两个健康顶层实例；WeFlow 再核对持久 profile（配置目录）、16000 端口和 health/sessions/contacts。配置目录缺失就停，不新建空库；两个进程或接口 200 都不能替代账号与 active-library（当前资料库身份）确认。" },
    ...
    { moduleSlug: "secrets-providers", ask: "找出这个项目现在该用哪条凭据，并把我指定的旧凭据替换掉。", effect: "先按名称、站点、项目、来源和健康状态找安全元数据，显示来源 Owner、冲突与最后验证；目标唯一且可写才进入精确替换，保留 preimage（变更前像），加密写入、消费者试用和恢复集回读完成后才报成功。来源没扫到或临时离线不会自动退役。" },
    ...
    { moduleSlug: "secrets-providers", ask: "原电脑坏了，G 盘不在，只靠私有 Git 恢复集还能重建密码中心吗？", effect: "独立 RecoverFromPrivateGit 会在干净新设备读取登记 PRIVATE 分支当前完整恢复集，核对远端提交、逐资产哈希与 epoch（状态代际），用一个有效因子恢复并生成新设备自己的密钥；不复制旧私钥，不依赖退役 C Policy、Publisher 或 CoreGoal。当前安装和恢复集就绪不等于本次已做真实新机演练。" },
    { moduleSlug: "secrets-providers", ask: "找这封邮件和云盘文件，再把我确认的回复、文件与日程改动交给固定 Google 账号执行。", effect: "邮件可搜索、读指定正文/附件、起草和发送；云盘可搜索、下载/导出、上传和精确分享；日历可查询、创建、修改和另行邀请。只读 MCP 不提供这些写入，写入须走同一 Provider 的类型化 CLI（命令行入口）、明确对象内容并回读；超时返回 effect_unknown（效果未知）时先查是否已发生，不能盲重发。" },
    ...
    { moduleSlug: "protected-actions", ask: "Codex Home 现在到底在哪里，迁移入口还需要保留吗？", effect: "现场回读 E 是唯一物理根、C 是兼容 junction；已完成迁移的专用脚本和测试已经退役，不再显示为可重复调用的当前能力。未来若要再次迁移，必须重新建立目标、preimage、回滚和验收。" },
    ...
    { moduleSlug: "protected-data", ask: "我的恢复因子丢了一个，能否换一个，同时保留最后的恢复入口？", effect: "P1 用另一个仍有效因子授权新登记或替换，先证明新因子可打开候选密文再提交；撤销时重新封装，不允许删除最后一条有效恢复路径。当前 P1 v1 支持集只有 recovery-code-v1，四类完整适配属于后续候选，不冒充已安装。" },
    { moduleSlug: "protected-data", ask: "打开安全文件库，找到我指定的文件，预览后导出一份。", effect: "候选入口通过 P0 拉起同一个安全文件库 Browser（浏览器控件），按目录、搜索和排序查看，再按用户点名预览/导出；密钥不进界面。导出到普通目录后离开保护范围，用户导出不会被 AI 按时自动删除。当前 normal-open（正常打开）换代与正式安装仍待验收，改名/移动/删除等完整日常写入是产品目标，不能靠只读候选宣布已可用。" },
    { moduleSlug: "protected-data", ask: "主入口、GUI 或目标授权服务坏了，只剩一个恢复载体和一个有效因子，还能读出文件吗？", effect: "恢复设计先在全新隔离目录核验完整载体和选定因子，建立 pre-root（重建正式控制面之前）的只读浏览与按需导出；后续设备登记失败不撤销这份已验证读取能力。但 RecoveryFactorHost 和侧车仍是候选，不能把设计、源码或 P0 正常启动当成真实单载体恢复成功。" },
    { moduleSlug: "protected-data", ask: "怎样把明确选中的旧文件导入加密库，并确认热备和冷备各自能恢复？", effect: "P5 设计只让 Vault V2 从精确清单流式导入并回读唯一 current index（当前索引）；P7 只复制同一已提交密文快照到热/冷副本，再分别在独立目录恢复并解密代表对象验证。旧明文先只读保留，删除另需明确授权；当前 P5–P7 仍只完成合成复制样例，没有执行正式导入、备份或恢复。" }
  ],
  evidenceLayers: [
...
export const pcconfigModules = [
  {
    ...
    value: "把散落在磁盘、环境、项目文件和本机服务里的配置变成可导航、可验证的机器地图。除了知道“登记了多少项”，我能直接查到主工作站的处理器、内存、显卡、Windows 和开发工具精确版本，以及它们是哪次观察的结果。用户不用靠记忆猜路径，也不会因为看到一个旧值就直接改错地方。TimeAudit 发现持续硬件压力时，PCConfig 还能按自己的游标消费一份有界异常摘要，再决定是否值得重新读取稳定机器事实。",
  ...
  },
  {
    ...
    teaser: "把工具版本、环境元数据和任务拆成独立运行链，并说清主工作站登录前 ToDesk、登录后微信双开与 WeFlow 连续性分别怎样恢复。",
    ...
    value: "让我知道“软件装着”之外，正确版本是否可用、启动入口是否仍在、计划任务上次怎样结束，以及重装后要按什么顺序恢复。主工作站还有三项具体保障：ToDesk 在无人登录时保持远控服务，微信在登录后补齐两个实例，WeFlow 保住第二路持久配置与可读接口。故障会落到具体任务或组件，而不是一句笼统的“电脑环境坏了”。",
    why: "运行时路径会变化，环境变量可能双作用域，登录启动和计划任务又是两套机制。只看文件是否存在会漏掉错误 Action（执行命令）、错误身份、被禁用任务或非零结果；只看任务 Ready（就绪）也不能证明业务成功。",
    example: "例如我问“台式机重启后还没登录，能否用 ToDesk 连回去；登录后为什么只开了一个微信”。前者查开机即运行的 SYSTEM 服务与本次启动回执，后者查延迟 30 秒的交互用户任务，并只补缺少的健康微信实例。第二路 WeFlow 读不到时，再查它自己的持久配置目录和接口，不用微信进程数猜账号或资料库。一般后台任务仍先比较 Task Scheduler（Windows 计划任务服务）、LastTaskResult（最近运行结果）和业务回执；任务成功与业务成功分开。",
  ...
  },
  {
    ...
    value: "重装 Windows、换电脑、系统盘故障或只能进入 PE 时，我能从当前手里真正可用的来源开始，按不破坏原盘的顺序恢复：先认机器和磁盘，再处理 BIOS/UEFI、Windows 与驱动，接回三个控制面、运行时、项目、用户配置、任务和启动项，最后恢复登录与受保护数据并做真实应用验收。日常备份则把 G 中已存在的普通资料、照片视频等原生字节纳入 H 计划；不要求先把所有媒体和资料做成加密恢复载体。",
  ...
  },
  {
    ...
    teaser: "从找对凭据、处理来源冲突、更新和恢复，一直讲到邮件、云盘、日历的具体读写；秘密默认盲用，真实结果按各自入口回读。",
    ...
    value: "我能找到真正该用的凭据，分清它来自哪个项目或密码库，处理冲突、导入新增、替换失效项，并在换机时重建访问；不是只有一个“把密码藏起来”的接口。程序可以盲用账号、API（程序接口）和银行卡字段，固定 Google 账号也能完成明确的邮件、云盘和日历读写。需要人工验证时只处理那个精确目标，不因此获得批量明文能力。",
    why: "同名账号可能来自不同来源，旧 CSV 少了一项也不代表用户要删除它；直接覆盖或把网络失败当成密码失效会破坏仍可用的登录与恢复线。另一方面，把 Provider 做成可临时换账号、网址和任意请求体的通用网络入口，会让一次具体请求扩散到其他对象。这里保留来源、目标、回滚和效果回读。",
    example: "例如我说“找出这个项目现在使用的凭据，把我指定的旧项换掉，并确保新机能恢复”。先列安全元数据、来源 Owner（责任源）和冲突，唯一命中后才按精确目标更新；加密回读、消费者试用和恢复集更新都成功才完成。若我说“在当前支付页填这张银行卡但不要提交”，则只对唯一 HTTPS 页面和标准字段原子盲填，提交仍由我决定。办公请求可说“找这封邮件、把回复存为草稿，再把确认的日程改到明天”；读取、草稿、修改与邀请各自是独立动作，不静默换账号。",
    result: "查找返回名称、用途、来源、健康、冲突、更新时间和恢复覆盖，不返回秘密；变更返回新增/保持/待复核、精确回读和恢复集结果，失败可取消、回滚或明确 partial（部分完成）。登录、盲填与办公动作另返回各自业务结果。写请求超时可能已经生效，必须先查询真实对象再决定下一步，不能把无响应当成没有执行。",
    readerStates: {
  ...
  },
  {
    ...
    result: "正常时得到一份可续作、可验证、可恢复且不会覆盖不同内容的加密包；中断后可以只继续未完成文件，恢复前还能证明密文没有被改动。技术层用 manifest、key envelope、分块密文 objects、加密 index 和不含正文/密钥的 receipt 保存证据；来源、密文或恢复目标有问题时返回精确 partial/conflict/tamper 状态并保留双方。当前没有独立 preview/dry-run 动作，也没有用真实用户文件跑最高权限 E2E。",
  ...
  },
  {
    ...
    value: "我能知道一项高影响机器操作现在该走哪里：规则发布归 .agents，BitLocker、Password Center、P0–P7 各走独立产品入口；Codex Home 当前只是一项已闭合的机器落点事实，迁移工具已经退役。旧 C policy 只保留历史，不再因为拥有管理员能力就成为万能执行器。",
    ...
    example: "例如历史上的 Codex Home 切换曾先保存 preimage、最终增量、ACL/链接清单和回滚副本；写入仍活跃时保持 waiting_for_codex_exit，退出后才原子切换并回读。现在 E 是唯一根，C 只是一条兼容 junction，迁移工具已经退役；另一个例子是当前规则发布，它只走 .agents 的 E release。",
  ...
  },
  {
    ...
    teaser: "从保住最后恢复因子、安全文件库日常使用，到主入口失效后的单载体读取、正式导入和热冷副本恢复，分清目标、候选、安装和真实数据证据。",
    ...
    value: "这套产品要同时解决“安全存放”和“故障后仍能拿回来”：日常由安全文件库查找、预览和明确导出，因子更换不能删掉最后恢复路径，应用升级保留最后可用版，主入口坏了也能设计为用一份完整 Carrier（恢复载体）加一个有效因子先恢复只读访问。当前已验证的 P0 运行结果、P1 有限支持集和后续文件库/恢复候选各自列明，不能把整套目标都当作已安装。",
    why: "普通应用升级失败可以重装，但加密数据升级失败可能同时损坏入口、索引、密钥封装和恢复线；如果必须先修好原 GUI、目标授权或全部设备才能解密，恢复就形成死循环。必须保住独立载体、至少一个有效因子和主入口之外的只读路径，同时先证明新版本可读写、可重开、可恢复再切换。",
    example: "日常可以说“打开安全文件库，找这份文件，预览后导出到我指定的目录”。候选界面用同一个目录/搜索/排序控件，导出前说明副本将离开保护范围；它目前不代表完整写操作已经上线。若改成“主入口坏了，我只有一份完整载体和一个有效因子”，恢复设计会先在独立目录校验并开放只读浏览，后续设备登记失败也保留已验证访问。应用升级仍由 P0 把候选放入不可变 slot（版本槽），经切换前后健康检查才取代 LKG（最后确认可用版本），失败退回旧版。",
    result: "用户应收到明确可用的版本或恢复入口：升级成功返回正常选择器、新 LKG 与回退版；因子事务返回新代际与最后路径仍可用的证明；文件库返回指定文件或用户确认的导出；单载体恢复返回隔离只读会话与完整性回读。P5–P7 则要分别证明导入后唯一索引、热副本和冷副本的独立恢复。当前只有 source（源码）或 fixture（合成样例）证据的部分只称 candidate（候选），不以这些结果替代安装、真实因子、数据或新机验收。",
    readerStates: {
