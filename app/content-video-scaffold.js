...
export const videoScaffoldProject = {
  ...
  plainExample: "我可以提出：“按这份已经确认的脚本做一条视频。每段旁白一场，念到‘核心结论’时再让重点数字出现；先把所有场景放进预览页给我看，确认后再渲染 4K60，并生成成片、封面和章节，不要自动上传。”脚手架负责把这些阶段串起来，选题、文案事实和最终审美仍由我确认。",
  ...
  usageExamples: [
    { moduleSlug: "project-bootstrap", ask: "给下一期视频建一个干净项目，先确认这台电脑能不能跑。", effect: "向空目录复制通用运行时与测试，先执行 test 和本地 doctor；非空目录拒绝覆盖，Fish 网络不在普通体检中偷偷调用。" },
    { moduleSlug: "voice-timing", ask: "按这几段已确认旁白生成声音和词级时间，不要因为重跑把旧时间轴套到新音频。", effect: "脚本、配音配置、音频、时长与识别配置逐层绑定；身份漂移时停止并提示明确重建，不静默沿用旧产物。" },
    { moduleSlug: "scene-authoring", ask: "念到“核心结论”时数字出现，素材只用我给的这一张图。", effect: "提示词带入真实说词与素材 URI；审阅后的 SVG 用 data-cue 绑定，找不到词或素材缺失时不进入长渲染。" },
    { moduleSlug: "preflight-preview", ask: "先把所有场景放在一页里让我看，文字越界就别继续。", effect: "build、lint 和 preview 先给出编号、cue、硬布局与动态视觉结果；人工未确认时不启动完整 4K 渲染。" },
    { moduleSlug: "deterministic-render", ask: "昨晚渲染中断了，输入没变就接着跑；我换了图片就别混用旧分片。", effect: "同一身份下只补缺失或错误分片；场景、file:/// 素材、背景、时长或编码变化会清掉受管旧分片后全量重渲。" },
    { moduleSlug: "delivery-verify", ask: "把旁白、可选音乐、封面和章节补齐，检查通过再告诉我能不能投稿。", effect: "生成 final_output.mp4、cover.png、chapters.txt 并执行 verify；仍明确要求真实观看，脚手架不上传平台。" },
    { moduleSlug: "recovery-reuse", ask: "哪些文件可以删，哪些应该留下让我明天继续？", effect: "默认只清理可再生分片、拼接临时物和 rendered/，保留脚本、音频、时间轴、场景 HTML、最终交付与身份；无法证明同源的旧产物不继续使用。" }
  ],
  evidenceLayers: [
...
export const videoScaffoldModules = [
  {
    ...
    example: "“在 D:\\Videos\\my-next-video 建一个新项目，别带上一期脚本；先跑自身测试和不联网体检。”初始化遇到非空目录直接拒绝，不为了方便覆盖现有文件。",
  ...
  },
  {
    ...
    example: "“把三段已确认文案生成旁白，念到‘核心结论’时给场景 cue；我刚改过第二段，不要复用旧 audio_02 或 srt_02。”普通运行发现身份不符会拒绝，确认要重配后再明确 force。",
  ...
  },
  {
    ...
    example: "“第二场只用我给的 hero.png；念到‘三项变化’时画三条线，最后数字滚动到 82。”prompts 会带入真实 file:/// URI 和可 cue 词，审阅后的 fragment_02.svg 才进入 build。",
  ...
  },
  {
    ...
    example: "“先把 12 个场景都放到预览页；任何标题越界或 cue 缺失都别渲染。”build 核对索引，lint 在每场接近末尾的一个稳定帧检查画布，preview 再生成可缩放动态网格。",
  ...
  },
  {
    ...
    example: "“昨晚完成了前 20 个分片，今天继续；但我把 hero.png 换了，如果旧分片用了旧图就全部重做。”同 URI 素材字节变化会改变身份，受管旧分片被清理后再渲染。",
  ...
  },
  {
    ...
    example: "“旁白为主，bgm.mp3 只在空隙明显；封面用本期标题，章节从开场开始。全部验证后再告诉我是否值得观看。”merge 不会在没有旁白时生成最终静音交付。",
  ...
  },
  {
    ...
    example: "“明天接着做：脚本没变的旁白继续用，第二场图换了就重渲；清理今天的临时分片，但保留时间轴和场景 HTML。”不同阶段按自己的身份与可再生边界处理。",
