...
export const cacbProject = {
  ...
  plainExample: "假如我要检验 AI 会不会真修代码，而不是只会说“已经解决”：给它一个小项目、失败现象和明确目标，再检查它留下的代码、诊断说明和测试结果。这个筛选展示会说明怎样让每次尝试拿到同一份输入、待在自己的目录里，以及缺哪项证据就必须写未完成；它不在网页上启动新评测，也不拿旧分数冒充当前排名。",
  ...
  usageExamples: [
    ...
    { moduleSlug: "identity-evidence", ask: "某个模型现在到底能不能在这条执行路线里用？", effect: "模型证据卡会把模型、提供方、版本、执行方式和当前可用条件分开核对；它只回答身份与资格，真实任务有没有做成仍看独立基准结果。" },
...
export const cacbModules = [
  ...
  },
  {
    ...
    example: "我想比较“一个根智能体带几名帮手”和“一个智能体独自完成”哪种更靠谱。页面会展示每名帮手拿到的独立任务、交回的结果，以及根怎样解决冲突并完成总体验收；两种执行方式不会混成一个成绩。",
