...
export const cacbProject = {
  ...
  plainExample: "例如我要检验 AI 能否修好一段代码：给它一个本地小项目、几段失败命令的输出和明确验收要求，让它补齐故障诊断函数并运行测试。CACB 为每次尝试准备同一版本的输入和独立目录；结束后检查实际代码、逐条诊断报告和测试结果，保留失败原因与可复查证据。没有完整产物或验证时就写未完成，不用一句“已解决”替代结果。执行路线、身份、独立审查和费用观察各自的详细边界仍在下方模块中说明。",
