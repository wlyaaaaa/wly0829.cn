...
export const skillOutcomes = {
  ...
  "timeaudit-diagnostics": {
    value: "让我在卡顿、发热、掉帧、磁盘/网络或睡眠问题已经过去后，快速取得一段可判断覆盖质量的历史证据；不用临时拼 SQL，也不会把缺数据或相关信号误写成确定根因。",
  ...
  "work-delivery": {
    value: "让我把一个会持续变化的工作事项做成真正一致的交付包：需求、会议记录、规则和表格中的事实只确认一次，PRD、项目计划、评审 PPT、执行表、周报和汇报都从同一事实版本生成；来源变化时只处理受影响部分。",
    why: "手工分别改 PRD、PPT 和跟踪表，很容易出现日期、负责人、范围、指标和验收口径互相矛盾。更常见的问题是来源改了，却没人知道哪些旧成品已经过期。",
    example: "例如我明确选择需求说明、会议纪要和一张指标表，要求做 PRD、项目计划、评审 PPT 和执行表。它先指出三份来源里的事实、冲突和缺口；确认后从同一事实版本生成各类成品。后来会议纪要更新时，它只告诉我受影响的事实和成品。",
    result: "我会得到来源覆盖、已确认事实、未解决冲突、跨产物一致的 PRD/计划/评审/执行/周报或汇报、追溯清单和变更影响；若关键事实或验收条件缺失，只得到明确标记的待确认草稿和一个需要决定的问题。",
