...
export const skillOutcomes = {
  ...
  "timeaudit-diagnostics": {
    ...
    example: "例如我问“过去一小时为什么偶尔卡”。它先确认 3590 个样本覆盖到窗口末端，再检查 CPU/GPU、内存、磁盘、网络和有效游戏帧；若没有游戏帧就明确写正常 `no_game_frames`，不会凭空生成掉帧原因。",
    result: "我会得到先结论、后证据的诊断摘要：查询时间窗、覆盖质量、关键聚合、阈值出现次数、因果限制、未覆盖时间和下一项安全检查。逐行进程或窗口细节不在 v1 输出中，但这些字段类别不是天然禁止公开。",
    readerStates: { pass: "schema、owner、status、窗口和 coverage 都正确时，使用聚合结果辅助当前诊断，并明确它只能证明相关性。", problem: "存在空档、rejected frame、阈值 warning 或交叉状态重叠时保留这些质量信息，不把部分结果包装成全绿。", unavailable: "provider 不存在、超时、stale、empty 或结构异常时停止使用它，回到普通诊断；不反复重试，也不自动控制 TimeAudit。" },
