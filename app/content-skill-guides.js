...
export const skillGuides = {
  ...
  "personal-litigation": guide([
    ["Case card（唯一现状卡）", "本人一项私人事务当前状态、时间线、原件位置、已确认事实、未知、期限和下一步的唯一可编辑责任源。"],
    ["ready_for_upload（可提交成品）", "正式工具链已完成构建、完整审阅、必要签署与允许清单验证；只证明成品可用，不证明已经提交。"],
    ["presumed completed（推定本人已操作）", "只有用户明确决定继续、已经取得 verified ready_for_upload（已验证可提交成品），且后来没有 cancellation（取消）、non-submission（未提交）、failure（失败）、withdrawal（撤回）或 deferral（延期）反馈时，才可按惯例避免重复动作；它仍不是平台或机构回执。"],
    ["Reality read-back（现实状态回读）", "从真实平台、邮件、电话、机构记录或用户一手说明确认后来实际发生了什么，而不是读取生产日志猜现实。"]
  ], [
    ["现状卡与原件冲突", "不静默改写，也不把工具读取状态写进正式文书。", "核对精确原件和用户确认事实，只把真正不确定的时间、原话或原因保留为未知。"],
    ["只有可提交成品，没有平台回执", "本人是否已经操作保持 Unknown（未知），也不写成机构已受理。", "先做现实状态回读；只有明确继续、已验证可提交成品、且没有取消、未提交、失败、撤回或延期反馈时，才可按惯例避免重复动作。"],
    ["需要提交、付款、撤回或联系外部机构", "没有精确授权就停止外部动作。", "先完成文书和本地验收；用户明确对象、内容与动作后再执行并回读。"]
  ]),
  "documents": guide([
...
export const skillOutcomes = {
  ...
  "personal-litigation": {
    value: "让我处理本人私人事务时始终围绕一份现状卡和真实原件工作，并把文书生产、签署、本人操作、平台回执与机构状态分开，避免重复动作或把内部回执误报为现实结果。",
    why: "同一事务可能同时存在草稿、签署件、可提交包、本人已经操作的判断、平台记录和机构处理。把这些层合在一起，最容易重复提交、漏掉期限或把文件生成当成现实进展。",
    example: "例如我问“这件事现在到哪一步了，下一份文书怎么准备”。它先读唯一现状卡，再只核对会改变判断的原件；不能确认的事实单列，文书通过正式工具链完整审阅。",
    result: "我会得到一条当前结论、理由和下一步，以及可以实际使用的文书或材料包；哪些只是生产完成、哪些已由本人操作、哪些有平台或机构回执会分别说明。",
    readerStates: { pass: "现状卡、必要原件、期限与目标渠道一致时，形成可用文书和明确下一步；外部动作另按精确授权执行。", problem: "原件冲突、时间不清或只有部分回执时，保留已确认事实并把真正未知单列，不重复操作。", unavailable: "现状卡、关键原件或正式工具链不可用时停止受影响结论和文书，不从旧对话、缓存或内部日志补猜。" },
    changes: ["默认只读唯一现状卡的当前层。", "需要时只打开会改变当前判断的最小原件。", "文书生产、签署、本人操作、平台和机构状态逐层区分。", "不能确认的事实变成具体待核查项，不在正式文书中补猜。", "提交、付款、撤回和外部联系必须有精确授权。"]
  },
  "documents": {
