...
export const skillGuides = {
  ...
  "work-delivery": guide([
    ...
    ["首轮或正式 PRD 超过时间门", "5 分钟内没有来源覆盖/事实/冲突/结构，或 20 分钟仍没有合格 PRD，就判本轮产品路径失败，不用“更专业”掩盖等待。", "立即缩小交付物或退回直接能力；事实已确认后的 PRD.md + DOCX 若超过 15 秒，也要定位确定性构建瓶颈。"]
  ]),
  "documents": guide([
...
export const skillOutcomes = {
  ...
  "work-delivery": {
    ...
    changes: ["只有用户明确选中的来源进入交付包。", "事实、假设、冲突和未知分开确认。", "PRD、项目计划、评审 PPT、执行表、周报和汇报绑定同一事实修订版。", "普通 2–5 份来源先在 5 分钟内给覆盖/事实/冲突/结构；正式 PRD 目标 10–15 分钟，20 分钟仍不合格即失败。", "事实确认后的 PRD.md + DOCX 确定性构建目标不超过 15 秒；只需要 PRD 时不等待其他产物。", "来源变化后旧构建标记过期，并只列受影响事实。", "一次性单文件编辑绕过本项目。", "未授权的发送、审批、邀请和外部修改不执行。"]
  },
  "documents": {
