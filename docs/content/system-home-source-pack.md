...
| PCConfig | 吴乐阳个人维护的 Windows 配置与恢复控制面 | “为什么任务没启动”“迁移到 V 盘”“重装后怎样恢复”。它把通用工具能力接到这台机器的路径、运行时、任务、备份和回滚 | PRIVATE main `5bb66e9`，另有 3 个迁移候选文件；完整可见 Drift 为 Registry 87 / Windows 88、2 新增/1 移除；BIOS 核心记录已验证。Codex Home 尚未正式切换 | `/projects/pcconfig/` |
