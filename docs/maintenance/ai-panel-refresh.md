...
- 来源项目正式发布后，由 `personal-panel-refresh` 把项目 ID、远端回读提交和实质影响理由交给一个全新网站任务。
...
7. 运行内容合同、构建、公开边界和本地预览；发布仍需单独授权。
...
当前首屏 JavaScript gzip 预算为 256 KiB。第 8 个项目启用前必须把项目详情改为 Registry lazy-load：首页只加载轻量卡片 manifest，搜索使用由 AI 正文派生的轻量索引。派生索引不得成为第二份语义正文；40+ 项目不能沿用全量 eager import。
