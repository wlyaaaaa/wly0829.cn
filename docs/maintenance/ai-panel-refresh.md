...
3. AI 读取受影响页面的新旧产品语义，再判断 System 的真实场景、职责卡、定时协作、项目版图、规则故事、能力家族或验证层是否会让读者形成错误理解。
...
`config/final-project-order.json` 是 32 个保留独立项目、价值顺序和施工状态的唯一清单，使用修订后的 1–33 名次：rank 15 留空，用户本轮删除 Playbook 后明确将后续编号减一。Registry 只登记完整内容包，实际数量从清单读取。Gemini 按最新交付情况选择最低未建名次，并尊重用户明确暂缓的项目；下一项不等待 Astra 的验收、设计、修复、整合或发布。Astra 在互不冲突的范围并行推进，最后合并验证，本轮网站只保留本地候选。Scripts 的能力由所属项目承接，不再生成独立项目；`md-triple-tactics-talent-solver` 只作为 System 历史总账资产。新增项目进入 Registry 时提供唯一内容包、轻量卡片信息、Owner collectors、impact sources 和独立 observedAt/fingerprint，并沿用清单中的固定名次。
